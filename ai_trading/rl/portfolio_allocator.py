...
class PortfolioAllocator:
    ...
    def allocate(
        self, action_weights, symbols, prices=None, volatilities=None, returns=None
    ):
        """
        ...
        if isinstance(action_weights, np.ndarray):
            action_weights = {
                symbol: float(weight) for symbol, weight in zip(symbols, action_weights)
            }

        # Vérifier que nous avons des poids pour tous les symboles
        assert set(action_weights.keys()) == set(
            symbols
        ), "Les poids d'action doivent correspondre aux symboles"
        ...
        if self.method == "equal":
            ...
            ), "Les volatilités sont requises pour la méthode 'volatility'"
            allocation = self._volatility_allocation(action_weights, volatilities)
        elif self.method == "momentum":
            assert (
                returns is not None
            ), "Les rendements sont requis pour la méthode 'momentum'"
            allocation = self._momentum_allocation(action_weights, returns)
        elif self.method == "smart":
            assert (
                volatilities is not None and returns is not None
            ), "Volatilités et rendements requis pour la méthode 'smart'"
            allocation = self._smart_allocation(action_weights, volatilities, returns)
        else:
            logger.warning(
                f"Méthode d'allocation '{self.method}' non reconnue, utilisation de 'equal'"
            )
            allocation = self._equal_allocation(action_weights)

        # Limiter le nombre de positions actives si spécifié
        if self.max_active_positions is not None and self.max_active_positions < len(
            symbols
        ):
        ...
        if total_allocation > 0:
            normalized_allocation = {
                symbol: weight / total_allocation
    ...
    def _equal_allocation(self, action_weights):
        ...
        active_assets = {
            symbol: weight for symbol, weight in action_weights.items() if weight > 0.1
        }
        ...
        return {
            symbol: equal_weight if symbol in active_assets else 0.0
    ...
    def _volatility_allocation(self, action_weights, volatilities):
        ...
        for symbol in action_weights:
            if symbol in active_assets:
                allocation[symbol] = action_weights[symbol] * inv_volatility.get(
                    symbol, 0.0
                )
            else:
    ...
    def _momentum_allocation(self, action_weights, returns):
        ...
        for symbol in action_weights:
            if symbol in active_assets:
                allocation[symbol] = action_weights[symbol] * positive_returns.get(
                    symbol, 0.0
                )
            else:
                allocation[symbol] = 0.0
