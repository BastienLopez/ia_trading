...
class AccumulatedSACAgent:
    ...
    def update(self, batch_size):
        ...
        for _ in range(self.accumulation_steps):
            ...
            states, actions, rewards, next_states, dones = self.agent.buffer.sample(batch_size)
            ...
            with torch.no_grad():
                ...
                value_target = rewards + self.agent.gamma * (1 - dones) * (q_next - self.agent.alpha * next_log_probs)
        ...
        for target_param, param in zip(self.agent.target_q1.parameters(), self.agent.q1.parameters()):
            target_param.data.copy_(self.agent.tau * param.data + (1 - self.agent.tau) * target_param.data)
            
        for target_param, param in zip(self.agent.target_q2.parameters(), self.agent.q2.parameters()):
            target_param.data.copy_(self.agent.tau * param.data + (1 - self.agent.tau) * target_param.data)
        ...
        avg_alpha_loss = total_alpha_loss / self.accumulation_steps if self.agent.auto_entropy else 0
    ...
    for name, system in trading_systems.items():
        ...
        for episode in range(config["episodes"]):
            ...
            while not done:
                ...
                next_state, reward, terminated, truncated, info = system._env.step(action)
            ...
            writer.add_scalar(f"{name}/portfolio_value", info.get("portfolio_value", 0), episode)
...
def main():
    """Fonction principale."""
    parser = argparse.ArgumentParser(description="Entraînement RL avec accumulation de gradient")
    parser.add_argument("--gpu", action="store_true", help="Forcer l'utilisation du GPU si disponible")
