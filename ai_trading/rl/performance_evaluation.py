...
def compare_strategies(df, strategies, initial_balance=10000, window_size=50):
    ...
    for name, strategy in strategies.items():
        ...
        results[name] = {"portfolio_values": portfolio_values, "metrics": metrics}
    ...
    metrics_df.to_csv(
        os.path.join(VISUALIZATION_DIR, f"metrics_comparison_{timestamp}.csv")
    )
...
def plot_strategy_comparison(results, title="Comparaison des stratégies"):
    ...
    if "buy_hold" in results:
        plt.plot(
            results["buy_hold"]["portfolio_values"],
            linestyle="--",
            label="Buy & Hold",
            color="black",
        )
    ...
    output_path = os.path.join(
        VISUALIZATION_DIR, f"strategy_comparison_{timestamp}.png"
    )
