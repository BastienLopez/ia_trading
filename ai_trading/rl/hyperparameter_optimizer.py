...
def calculate_sharpe_ratio(returns, risk_free_rate=0.0):
    ...
    if std_return == 0:
        return 0
    return (mean_return - risk_free_rate) / std_return * np.sqrt(252)  # Annualisation (252 jours de trading)
...
class HyperparameterOptimizer:
    ...
    def __init__(
        ...
        logger.info(f"HyperparameterOptimizer initialisé avec {len(self._get_param_combinations())} "
    ...
    def grid_search(self):
        ...
        logger.info(f"Démarrage de la recherche par grille avec {total_combinations} combinaisons")
        ...
        if self.n_jobs > 1:
            with ProcessPoolExecutor(max_workers=self.n_jobs) as executor:
                ...
                for i, params in enumerate(param_combinations):
                    future = executor.submit(self._evaluate_params, params, i, total_combinations)
            ...
            for i, params in enumerate(param_combinations):
                score, params, metrics = self._evaluate_params(params, i, total_combinations)
                self._update_best(score, params, metrics)
    ...
    def _evaluate_params(self, params, index, total):
        ...
        # Entraîner l'agent
        train_metrics = self._train_agent(env, agent, params, self.n_episodes, self.max_steps)
    ...
    def _train_agent(self, env, agent, params, n_episodes, max_steps):
        ...
        for episode in range(n_episodes):
            ...
            while not done and (max_steps is None or step < max_steps):
                ...
                if step % params.get('train_frequency', 1) == 0 and agent.memory.size() >= agent.batch_size:
                    agent.train()
            ...
            if self.verbose > 1 and episode % 5 == 0:
                logger.info(f"Épisode {episode}/{n_episodes}, Récompense: {episode_reward:.2f}")
    ...
    def _calculate_sharpe_ratio(self, portfolio_values_list):
        ...
        for portfolio_values in portfolio_values_list:
            ...
            if len(returns) > 1 and np.std(returns) > 0:
                sharpe = np.mean(returns) / np.std(returns) * np.sqrt(252)  # Annualiser (252 jours de trading)
    ...
    def _generate_plots(self, df, timestamp):
        ...
        plt.axvline(self.best_score, color='r', linestyle='--', label=f'Meilleur score: {self.best_score:.4f}')
        plt.xlabel("Score")
            ...
            try:
                ...
                for param in self.param_grid.keys():
                    if param in df.columns and len(df[param].unique()) > 1:
                        ...
                        plt.bar(param_scores[param].astype(str), param_scores["score"], alpha=0.7)
                        plt.xlabel(param)
                        plt.ylabel("Score moyen")
                        plt.title(f"Impact de {param} sur le score")
                        plt.xticks(rotation=45)
                        plt.tight_layout()
                        plt.savefig(os.path.join(self.save_dir, f"param_impact_{param}_{timestamp}.png"))
            except Exception as e:
                logger.warning(f"Erreur lors de la génération des graphiques d'importance des paramètres: {e}")
