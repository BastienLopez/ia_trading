...
class PerformanceMetrics:
    ...
    def calculate_sharpe_ratio(returns, risk_free_rate=0.0):
        ...
        # Annualiser le ratio de Sharpe (multiplier par sqrt(252) pour les données quotidiennes)
    ...
    def calculate_expectancy(trades):
        ...
        avg_win = avg_win / sum(1 for trade in trades if trade['profit'] > 0) if sum(1 for trade in trades if trade['profit'] > 0) > 0 else 0
        
        avg_loss = sum(trade['profit'] for trade in trades if trade['profit'] < 0)
        avg_loss = avg_loss / sum(1 for trade in trades if trade['profit'] < 0) if sum(1 for trade in trades if trade['profit'] < 0) > 0 else 0
        
        return (win_rate * avg_win) + ((1 - win_rate) * avg_loss)
    
    @staticmethod
    def calculate_all_metrics(portfolio_values, benchmark_values=None, risk_free_rate=0.0):
        """
        ...
        # Drawdown maximum
        max_drawdown, _, _, _ = PerformanceMetrics.calculate_max_drawdown(portfolio_values)
        ...
        if len(returns_below_threshold) == 0 or np.sum(threshold - returns_below_threshold) == 0:
            omega_ratio = np.inf if len(returns_above_threshold) > 0 else 0
        else:
            omega_ratio = np.sum(returns_above_threshold - threshold) / np.sum(threshold - returns_below_threshold)
        ...
        if benchmark_values is not None and len(benchmark_values) == len(portfolio_values):
            ...
            benchmark_annualized_return = (1 + benchmark_total_return) ** (252 / n_periods) - 1
...
class PerformanceVisualizer:
    ...
    def plot_portfolio_performance(self, portfolio_values, benchmark_values=None, dates=None, title="Performance du portefeuille"):
        """
        ...
        if benchmark_values is not None:
            plt.plot(x, benchmark_values, label='Référence', color='red', linestyle='--', linewidth=1.5)
    ...
    def plot_returns_distribution(self, returns, title="Distribution des rendements"):
        ...
        plt.axvline(x=np.mean(returns), color='green', linestyle='-', linewidth=1.5, label=f'Moyenne: {np.mean(returns):.4f}')
    ...
    def plot_drawdown(self, portfolio_values, dates=None, title="Drawdown"):
        ...
        # Trouver le drawdown maximum
        max_drawdown, start_idx, end_idx, recovery_idx = PerformanceMetrics.calculate_max_drawdown(portfolio_values)
        ...
        if max_drawdown < 0:
            plt.fill_between(x, 0, drawdowns, where=(drawdowns < 0), color='red', alpha=0.3)
            ...
            plt.scatter(x[end_idx], max_drawdown, color='red', s=100, marker='v', label='Creux')
            
            if recovery_idx is not None:
                plt.scatter(x[recovery_idx], 0, color='blue', s=100, marker='o', label='Récupération')
    ...
    def plot_trade_analysis(self, trades, title="Analyse des transactions"):
        ...
        ax1 = axes[0, 0]
        ax1.bar(range(len(profits)), profits, color=['green' if p > 0 else 'red' for p in profits])
        ...
        ax2.axvline(x=np.mean(profits), color='green', linestyle='-', linewidth=1.5, label=f'Moyenne: {np.mean(profits):.2f}')
        ...
        ax3.plot(range(len(cumulative_profits)), cumulative_profits, color='blue', linewidth=2)
    ...
    def plot_performance_metrics(self, portfolio_values, benchmark_values=None, risk_free_rate=0.0, title="Métriques de performance"):
        """
        Trace un tableau des métriques de performance.
        ...
        metrics = PerformanceMetrics.calculate_all_metrics(portfolio_values, benchmark_values=benchmark_values, risk_free_rate=risk_free_rate)
        ...
        rows.append('Rendement total')
        values.append(f"{metrics['total_return']:.2%}")
        
        rows.append('Rendement annualisé')
        values.append(f"{metrics['annualized_return']:.2%}")
        
        rows.append('Volatilité annualisée')
        values.append(f"{metrics['volatility']:.2%}")
        
        # Métriques de risque
        rows.append('Ratio de Sharpe')
        values.append(f"{metrics['sharpe_ratio']:.4f}")
        
        rows.append('Ratio de Sortino')
        values.append(f"{metrics['sortino_ratio']:.4f}")
        
        rows.append('Drawdown maximum')
        values.append(f"{metrics['max_drawdown']:.2%}")
        
        rows.append('Ratio de Calmar')
        values.append(f"{metrics['calmar_ratio']:.4f}")
        
        rows.append('Ratio Omega')
        values.append(f"{metrics['omega_ratio']:.4f}")
        
        # Métriques relatives au benchmark
        if 'alpha' in metrics:
            rows.append('Alpha')
            values.append(f"{metrics['alpha']:.4f}")
            
            rows.append('Bêta')
            values.append(f"{metrics['beta']:.4f}")
            
            rows.append('Ratio d\'information')
            values.append(f"{metrics['information_ratio']:.4f}")
            
            rows.append('Erreur de suivi')
            values.append(f"{metrics['tracking_error']:.2%}")
    ...
    def plot_actions_distribution(self, actions, action_labels=None, title="Distribution des actions"):
        """
    ...
    def create_performance_dashboard(self, results, benchmark_results=None, dates=None, trades=None, actions=None, risk_free_rate=0.0):
        """
        ...
        benchmark_values = benchmark_results['portfolio_history'] if benchmark_results else None
        ...
        self.plot_performance_metrics(portfolio_values, benchmark_values, risk_free_rate)
...
def evaluate_agent(agent, env, num_episodes=1, render=False):
    ...
    results = {
        ...
        'portfolio_history': all_portfolio_values[0] if num_episodes == 1 else all_portfolio_values,
    ...
    logger.info(f"Évaluation terminée. Rendement moyen: {avg_returns*100:.2f}%, Ratio de Sharpe: {sharpe_ratio:.4f}")
