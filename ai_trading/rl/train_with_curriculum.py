...
def main():
    ...
    model_path = (
        save_dir / f"sac_agent_curriculum_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    )
    ...
    plot_path = (
        vis_dir / f"curriculum_training_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
    )
