...
class RLTradingSystem:
    ...
    def evaluate(self, agent=None, env=None, num_episodes=1, test_data=None, visualize=False, save_dir=None):
        """
    ...
    def test_random_strategy(self, num_episodes=5):
        ...
        if not hasattr(self, 'env'):
            raise ValueError("L'environnement n'a pas été créé. Appelez create_environment d'abord.")
        ...
        for episode in range(num_episodes):
            ...
            logger.info(f"Stratégie aléatoire - Épisode {episode+1}/{num_episodes}: "
                       f"Valeur finale: ${portfolio_value:.2f}, Rendement: {returns:.2f}%")
        
        return results
    
    def integrate_data(self, market_data=None, news_data=None, social_data=None, 
                      sentiment_data=None, split_ratio=0.8, **kwargs):
        """
        ...
        logger.info(f"Données intégrées: {len(train_data)} points d'entraînement, {len(test_data)} points de test")
