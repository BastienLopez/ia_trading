...
class FrameCompressor:
    ...
    def decompress_frame(
        self, compressed_data: bytes, shape: Tuple[int, ...], dtype: np.dtype
    ) -> np.ndarray:
        """
    ...
    def stack_frames(self, frames: List[np.ndarray]) -> np.ndarray:
        ...
        while len(frames) < self.frame_stack_size:
            ...
            frames.insert(
                0, frames[0].copy() if len(frames) > 0 else np.zeros_like(frames[0])
            )
    ...
    def process_state(
        self, state: np.ndarray, update_stack: bool = True
...
class FrameStackWrapper:
    ...
    def __init__(
        ...
        if self.compress:
            self.compressor = FrameCompressor(
                compression_level=compression_level, frame_stack_size=n_frames
            )
    ...
    def step(self, action):
        ...
        if self.env is None:
            raise ValueError(
                "Cette méthode ne peut être utilisée que si env est fourni"
            )
