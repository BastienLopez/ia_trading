...
class PolicyLag:
    ...
    def _copy_model_attributes(self, src_model, dst_model):
        ...
        for attr_name in dir(src_model):
            ...
            if attr_name in ['training', 'modules', 'parameters', 'buffers', 'named_parameters',
                           'named_buffers', 'named_modules', 'state_dict', 'load_state_dict']:
                continue
    ...
    def _update_collect_model(self):
        ...
        with torch.no_grad():
            if self.copy_fn is not None:
                ...
                if hasattr(self.collect_model, 'load_state_dict') and hasattr(self.train_model, 'state_dict'):
        ...
        if n == 0:
            ...
            self.metrics["avg_update_time"] = (self.metrics["avg_update_time"] * n + update_time) / (n + 1)
    ...
    def _update_target_model(self):
        ...
        with torch.no_grad():
            if self.copy_fn is not None:
                ...
                # Copie standard par état du dictionnaire
                if hasattr(self.target_model, 'load_state_dict') and hasattr(self.train_model, 'state_dict'):
    ...
    def force_update(self, model_type: str = "collect"):
        ...
        if model_type == "collect":
            ...
            raise ValueError(f"Type de modèle inconnu: {model_type}. Utiliser 'collect' ou 'target'.")
...
class DecoupledPolicyTrainer:
        ...
        while self.is_training:
            try:
                ...
                if len(self.metrics["batch_process_time"]) > max_history:
                    self.metrics["batch_process_time"] = self.metrics["batch_process_time"][-max_history:]
                if len(self.metrics["queue_wait_time"]) > max_history:
                    self.metrics["queue_wait_time"] = self.metrics["queue_wait_time"][-max_history:]
                if len(self.metrics["train_loss"]) > max_history:
                    self.metrics["train_loss"] = self.metrics["train_loss"][-max_history:]
    ...
    def train_sync(self, batch, loss_fn: Callable) -> float:
        ...
        loss = loss_fn(train_model, target_model, states, actions, rewards, next_states, dones)
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        train_loss_avg = sum(self.metrics["train_loss"]) / max(1, len(self.metrics["train_loss"]))
        batch_time_avg = sum(self.metrics["batch_process_time"]) / max(1, len(self.metrics["batch_process_time"]))
        queue_wait_avg = sum(self.metrics["queue_wait_time"]) / max(1, len(self.metrics["queue_wait_time"]))
