...
class HogwildWorker:
    ...
    def __init__(
        ...
        if device is None:
            ...
            if self.device.type != "cpu":
                logging.warning(f"HOGWILD! ne fonctionne qu'avec CPU. Utilisation de CPU pour le travailleur {worker_id}")
                self.device = torch.device("cpu")
    ...
    def update_shared_model(self):
        ...
        for shared_param, local_param in zip(self.shared_model.parameters(), self.local_model.parameters()):
    ...
    def train_batch(self) -> float:
        ...
        if isinstance(batch, tuple) and len(batch) == 5:
            ...
            loss = self.loss_fn(self.local_model, states, actions, rewards, next_states, dones)
        else:
    ...
    def train(self, num_updates: int = 1000):
        ...
        for update in range(num_updates):
            ...
            if (update + 1) % 100 == 0:
                ...
                self.logger.info(f"Travailleur {self.worker_id}: {update+1}/{num_updates} mises à jour, "
        ...
        self.logger.info(f"Entraînement terminé. Travailleur {self.worker_id}: {self.updates_done} mises à jour, "
    ...
    def get_metrics(self) -> Dict:
        ...
        if self.start_time:
            ...
            self.metrics.update({
                ...
                "runtime": runtime,
                "updates_per_sec": updates_per_sec,
                "mini_batches_processed": self.updates_done * self.mini_batches_per_update
            })
...
class HogwildTrainer:
    ...
    def __init__(
        ...
        self.optimizer = self.optimizer_factory(self.model.parameters(), lr=self.learning_rate)
    ...
    def start(self, num_updates_per_worker: int = 1000):
        ...
        """
        self.logger.info(f"Démarrage de l'entraînement HOGWILD! avec {self.num_workers} travailleurs")
        self.start_time = time.time()
        self.is_running = True
    ...
    def wait_until_done(self, timeout: Optional[float] = None):
        ...
        while not all_done:
            ...
            if len(active_processes) > 0:
                self.logger.info(f"{len(active_processes)}/{len(self.processes)} travailleurs encore actifs")
