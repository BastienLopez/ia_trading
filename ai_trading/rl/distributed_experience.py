...
class ExperienceWorker:
    ...
    def _setup_connection(self):
        ...
        if not self.local_mode:
            ...
            self.logger.info(
                f"Connecté au maître: {self.master_address}:{self.master_port}"
            )
    ...
    def _send_experiences(self):
        ...
        if self.local_mode:
            ...
            try:
                ...
                self.logger.info(
                    f"Envoyé {len(self.local_buffer)} expériences au maître"
                )
    ...
    def collect_experience(self, steps: int):
        ...
        self.logger.info(
            f"Collecte terminée: {steps} étapes, {len(self.episode_rewards)} épisodes"
        )
    ...
    def run_in_process(self, steps: int):
        ...
        """
        self.worker_process = mp.Process(target=self.collect_experience, args=(steps,))
        self.worker_process.start()
        self.logger.info(
            f"Travailleur démarré dans le processus {self.worker_process.pid}"
        )
            ...
            metrics.update(
                {
                    ...
                    "latest_reward": (
                        float(self.episode_rewards[-1]) if self.episode_rewards else 0
                    ),
                }
            )
...
class ExperienceMaster:
        ...
        while self.running:
            try:
                ...
                if self.running:  # Ignorer les erreurs pendant l'arrêt
                    self.logger.error(
                        f"Erreur lors de la réception des expériences: {e}"
                    )
    ...
    def _process_experiences(self, experiences, worker_id):
        ...
        self.logger.info(
            f"Reçu {exp_count} expériences de {worker_id}. Total: {self.total_experiences}"
        )
    ...
    def _update_worker_policies(self):
        ...
        try:
            ...
            self.logger.info(
                f"Politiques mises à jour pour {len(self.local_workers)} travailleurs locaux"
            )
        except Exception as e:
...
class DistributedExperienceManager:
    ...
    def initialize(self):
        ...
        self.logger.info(
            f"Système initialisé avec {self.n_local_workers} travailleurs locaux"
        )
        return self
    ...
    def start_collection(self, steps_per_worker: int = 1000):
        ...
        """
        self.logger.info(
            f"Démarrage de la collecte: {steps_per_worker} étapes par travailleur"
        )
    ...
    def wait_until_done(self, timeout: Optional[float] = None):
        ...
        self.logger.info(
            f"Collection terminée. {len(self.local_workers) - active_count}/{len(self.local_workers)} travailleurs terminés"
        )
        return self
    ...
    def get_metrics(self) -> Dict:
        ...
        return {
            ...
            "buffer_size": (
                ...
                if hasattr(self.replay_buffer, "__len__")
                else "unknown"
            ),
