...
def test_random_actions():
    ...
    prices = (
        np.cumsum(np.random.normal(loc=0, scale=1, size=100)) + 100
    )  # Marche aléatoire
    ...
    while not done:
        ...
        print(
            f"Action: {action}, Reward: {reward:.4f}, Portfolio: ${info['portfolio_value']:.2f}"
        )

    print(f"Episode terminé. Récompense totale: {total_reward:.4f}")
    print(f"Valeur finale du portefeuille: ${info['portfolio_value']:.2f}")
    print(
        f"Rendement: {((info['portfolio_value'] / env.initial_balance) - 1) * 100:.2f}%"
    )
...
def test_sophisticated_rewards():
    ...
    for i in range(20):
        ...
        print(
            f"Step {i+1}: Action: {action}, Reward: {reward:.4f}, Portfolio: ${info['portfolio_value']:.2f}"
        )
    ...
    print(
        f"Récompense totale du scénario 1 (transactions fréquentes): {sum(rewards):.4f}"
    )
    print(
        f"Récompense totale du scénario 2 (stratégie cohérente): {sum(rewards_scenario2):.4f}"
    )
    ...
    print(
        f"Graphique sauvegardé dans '{os.path.join(VISUALIZATION_DIR, 'reward_comparison.png')}'"
    )
