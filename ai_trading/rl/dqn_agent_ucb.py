...
from ai_trading.rl.ucb_exploration import UCBExploration, NoveltyExploration, HybridExploration
...
class DQNAgentAdvancedExploration:
    ...
    def __init__(self, state_size, action_size, 
        ...
        if self.exploration_strategy == 'ucb':
            ...
            self.explorer = NoveltyExploration(state_size, action_size, 
                                              novelty_scale=novelty_scale,
                                              decay_rate=decay_rate,
                                              hash_bins=hash_bins,
                                              max_count=max_count)
        elif self.exploration_strategy == 'hybrid':
            self.explorer = HybridExploration(state_size, action_size, 
                                             ucb_c=ucb_c,
                                             novelty_scale=novelty_scale,
                                             decay_rate=decay_rate,
                                             hash_bins=hash_bins,
            ...
            raise ValueError(f"Stratégie d'exploration '{exploration_strategy}' non reconnue. "
        ...
        logger.info(f"Agent DQN initialisé avec la stratégie d'exploration {exploration_strategy}")
        ...
        for i, (state, action, reward, next_state, done) in enumerate(minibatch):
            ...
            if done:
                ...
                next_q_values = self.target_model.predict(
                    next_state.reshape(1, self.state_size), verbose=0)[0]
    ...
    def end_episode(self, episode_reward):
        ...
        if len(self.rewards_history) % 10 == 0 and self.verbose:
            logger.info(f"Épisode {len(self.rewards_history)}, "
