...
class SumTree:
    ...
    def __init__(self, capacity):
        ...
        self.data = np.zeros(capacity, dtype=object)  # Buffer pour stocker les expériences
...
class PrioritizedReplayMemory:
    ...
    def __init__(self, capacity, alpha=0.6, beta=0.4, beta_increment=0.001, epsilon=0.01):
        ...
        logger.info(f"Mémoire de replay priorisée initialisée avec capacité={capacity}, alpha={alpha}, beta={beta}")
    ...
    def sample(self, batch_size):
        ...
        min_priority = np.min(self.sum_tree.tree[-self.sum_tree.capacity:]) / self.sum_tree.total_priority()
        if min_priority == 0:
