...
class TradingEnvironment(gym.Env):
    ...
    def __init__(self, df, initial_balance=10000, transaction_fee=0.001, window_size=5):
        """
        ...
        logger.info(f"Environnement de trading initialisé avec {len(df)} points de données")
    ...
    def step(self, action):
        ...
        if action == 1:  # Acheter
            ...
            max_crypto_to_buy = self.balance / (current_price * (1 + self.transaction_fee))
            ...
            logger.debug(f"Achat: {max_crypto_to_buy:.6f} unités à ${current_price:.2f}")
            
        elif action == 2:  # Vendre
            if self.crypto_held > 0:
                # Vendre toute la crypto détenue
                self.balance += self.crypto_held * current_price * (1 - self.transaction_fee)
                
                logger.debug(f"Vente: {self.crypto_held:.6f} unités à ${current_price:.2f}")
        ...
        if done:
            logger.info(f"Épisode terminé. Valeur finale du portefeuille: ${portfolio_value:.2f}, "
    ...
    def _get_observation(self):
        ...
        price_history = self.df.iloc[self.current_step - self.window_size:self.current_step + 1]['close'].values
    ...
    def render(self, mode='human'):
        ...
        if mode == 'human':
            ...
            print(f"Profit/Loss: {((portfolio_value / self.initial_balance) - 1) * 100:.2f}%")
            print("-" * 50)
