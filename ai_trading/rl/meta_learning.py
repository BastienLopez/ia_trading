...
class MAMLNetwork(nn.Module):
    ...
    def __init__(
        ...
        for i in range(len(dims) - 1):
            layers.extend(
                [nn.Linear(dims[i], dims[i + 1]), activation, nn.LayerNorm(dims[i + 1])]
            )
...
class MAML:
    ...
    def meta_train_step(
        ...
        return {
            ...
            "meta_accuracy": (
                meta_accuracy.item()
                if isinstance(meta_accuracy, torch.Tensor)
                else meta_accuracy
            ),
        }
    ...
    def meta_evaluate(
        ...
        return {
            ...
            "eval_accuracy": (
                total_accuracy / num_tasks if len(query_y.shape) == 1 else 0
            ),
        }
...
class MarketMAML(MAML):
    ...
    def generate_market_task(
        self, env: TradingEnvironment, support_size: int = 100, query_size: int = 50
    ) -> Tuple[Tuple[torch.Tensor, torch.Tensor], Tuple[torch.Tensor, torch.Tensor]]:
        """
    ...
    def adapt_to_market(self, env: TradingEnvironment, num_episodes: int = 10) -> None:
        """
