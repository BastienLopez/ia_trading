...
class BayesianOptimizer(HyperparameterOptimizer):
    ...
    def __init__(
        ...
        logger.info(
            f"BayesianOptimizer initialisé avec {len(param_space)} paramètres à optimiser"
        )
        logger.info(
            f"Stratégie: {n_initial_points} points initiaux + {n_iterations} itérations"
        )
    ...
    def _find_next_point(self):
        ...
        for x_seed in x_seeds:
            ...
            result = minimize(
                self._negative_ei, x_seed, bounds=bounds, method="L-BFGS-B"
            )
    ...
    def bayesian_optimization(self):
        ...
        """
        logger.info(
            f"Démarrage de l'optimisation bayésienne avec {self.n_initial_points} points initiaux"
        )
        ...
        for i in range(self.n_initial_points):
            ...
            logger.info(
                f"Évaluation du point initial {i+1}/{self.n_initial_points}: {params}"
            )

            score, _, metrics = self._evaluate_params(
                params, i, self.n_initial_points + self.n_iterations
            )
        ...
        for i in range(self.n_iterations):
            ...
            logger.info(
                f"Itération {i+1}/{self.n_iterations}: Évaluation de {next_params}"
            )
            ...
            logger.info(
                f"Meilleur score jusqu'à présent: {self.best_score} avec {self.best_params}"
            )
        ...
        logger.info(
            f"Optimisation bayésienne terminée. Meilleur score: {self.best_score}"
        )
    ...
    def _save_results(self):
        ...
        results_data = []
        for i, (x_vec, score, metrics) in enumerate(
            zip(self.X_samples, self.y_samples, self.metrics_history)
        ):
            params = self._vector_to_param_dict(x_vec[0])
            entry = {"iteration": i + 1, "score": score, **params, **metrics}
        ...
        csv_path = os.path.join(
            self.save_dir, f"bayesian_optimization_history_{timestamp}.csv"
        )
