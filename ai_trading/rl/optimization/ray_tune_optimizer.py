...
try:
    ...
    HAVE_RAY = False
    logging.warning("Ray n'est pas installé. Utilisez 'pip install ray[tune]' pour l'installer.")
...
class RayTuneOptimizer:
    ...
    def __init__(
        ...
        """
        if not HAVE_RAY:
            raise ImportError("Ray n'est pas installé. Utilisez 'pip install ray[tune]' pour l'installer.")
        ...
        self.checkpoint_dir = checkpoint_dir or os.path.join(os.getcwd(), "ray_tune_results")
        self.log_dir = log_dir or os.path.join(os.getcwd(), "ray_tune_logs")
        self.resume = resume
        ...
        if scheduler.lower() == "asha":
            ...
            logger.warning(f"Scheduler {scheduler} non reconnu, utilisation de ASHAScheduler par défaut")
            self.scheduler = ASHAScheduler(
                max_t=max_iterations,
        ...
        if search_alg.lower() == "random":
            ...
            logger.warning(f"Algorithme de recherche {search_alg} non reconnu, utilisation de la recherche aléatoire par défaut")
            self.search_alg = BasicVariantGenerator()
    ...
    def _evaluate_agent(self, agent: Any, env: gym.Env, num_episodes: int = 5) -> Dict[str, float]:
        """
    ...
    def optimize(self) -> Tuple[Dict[str, Any], Dict[str, Any]]:
        ...
        with open(result_path, "w") as f:
            json.dump({
                ...
                "best_metrics": {k: float(v) if isinstance(v, (float, int)) else v 
...
def optimize_rl_agent(
    ...
    """
    Optimise les hyperparamètres d'un agent RL.
    
    Args:
        env_creator: Fonction qui crée l'environnement
        agent_type: Type d'agent ('ppo', 'sac', 'dqn')
        search_space: Espace de recherche personnalisé (utilise valeurs par défaut si None)
        num_samples: Nombre d'échantillons à tester
        max_iterations: Nombre maximal d'itérations par essai
        search_alg: Algorithme de recherche ('random', 'optuna', 'hyperopt', 'bayesopt')
        metric: Métrique à optimiser
        mode: 'max' ou 'min' selon qu'on veut maximiser ou minimiser la métrique
        checkpoint_dir: Répertoire pour les résultats
    ...
    # Définir l'espace de recherche par défaut selon le type d'agent
    if search_space is None:
        if agent_type.lower() == "ppo":
            search_space = {
                ...
                "train_batch_size": tune.choice([2000, 4000, 8000, 16000]),
                "lambda": tune.uniform(0.9, 1.0),
                "clip_param": tune.uniform(0.1, 0.3),
                "entropy_coeff": tune.loguniform(1e-5, 1e-1),
            ...
            search_space = {
                ...
                "q_model_config": {
                    "hidden_sizes": tune.choice([[256, 256], [512, 512], [256, 256, 256]])
                },
                "policy_model_config": {
                    "hidden_sizes": tune.choice([[256, 256], [512, 512], [256, 256, 256]])
                },
                "initial_alpha": tune.loguniform(0.001, 1.0)
            }
            ...
            search_space = {
                ...
                "target_network_update_freq": tune.choice([100, 500, 1000, 5000]),
                "learning_starts": tune.choice([1000, 5000, 10000]),
                "exploration_config": {
                    "type": "EpsilonGreedy",
                    "initial_epsilon": tune.uniform(0.5, 1.0),
                    "final_epsilon": tune.loguniform(0.01, 0.1),
                    "epsilon_timesteps": tune.randint(10000, 100000)
                },
                "dueling": tune.choice([True, False]),
                "double_q": tune.choice([True, False]),
                "hiddens": tune.choice([[256, 256], [512, 512], [256, 256, 256]])
            }
        else:
