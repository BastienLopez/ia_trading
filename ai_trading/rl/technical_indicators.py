...
class TechnicalIndicators:
    ...
    def calculate_macd(
        self, fast_period=12, slow_period=26, signal_period=9, column="close"
    ):
        """
    ...
    def calculate_adx(self, period=14):
        ...
        if not all(col in self.df.columns for col in ["high", "low", "close"]):
            logger.error(
                "Les colonnes 'high', 'low', 'close' sont requises pour calculer l'ADX."
            )
            return None, None, None
    ...
    def calculate_atr(self, period=14):
        ...
        if not all(col in self.df.columns for col in ["high", "low", "close"]):
            logger.error(
                "Les colonnes 'high', 'low', 'close' sont requises pour calculer l'ATR."
            )
            return None
    ...
    def calculate_volume_average(self, period=20):
        ...
        if "volume" not in self.df.columns:
            logger.error(
                "La colonne 'volume' est requise pour calculer la moyenne mobile du volume."
            )
            return None
    ...
    def calculate_mfi(self, period=14):
        ...
        if not all(
            col in self.df.columns for col in ["high", "low", "close", "volume"]
        ):
            logger.error(
                "Les colonnes 'high', 'low', 'close', 'volume' sont requises pour calculer le MFI."
            )
            return None
    ...
    def calculate_cci(self, period=20):
        ...
        if not all(col in self.df.columns for col in ["high", "low", "close"]):
            logger.error(
                "Les colonnes 'high', 'low', 'close' sont requises pour calculer le CCI."
            )
            return None

        typical_price = (self.df["high"] + self.df["low"] + self.df["close"]) / 3
        mean_deviation = (
            abs(typical_price - typical_price.rolling(window=period).mean())
            .rolling(window=period)
            .mean()
        )

        cci = (typical_price - typical_price.rolling(window=period).mean()) / (
    ...
    def calculate_fibonacci_levels(self, start_idx=None, end_idx=None, is_uptrend=True):
        ...
        levels = {
            ...
            "0.236": (
                low_price + 0.236 * diff if is_uptrend else high_price - 0.236 * diff
            ),
            "0.382": (
                low_price + 0.382 * diff if is_uptrend else high_price - 0.382 * diff
            ),
            "0.5": low_price + 0.5 * diff if is_uptrend else high_price - 0.5 * diff,
            "0.618": (
                low_price + 0.618 * diff if is_uptrend else high_price - 0.618 * diff
            ),
            "0.786": (
                low_price + 0.786 * diff if is_uptrend else high_price - 0.786 * diff
            ),
    ...
    def calculate_pivots(self, pivot_type="daily"):
        ...
        if not all(col in self.df.columns for col in ["high", "low", "close"]):
            logger.error(
                "Les colonnes 'high', 'low', 'close' sont requises pour calculer les pivots."
            )
            return None
    ...
    def calculate_volume_profile(self, n_bins=10, lookback=20):
        ...
        """
        logger.info(
            f"Calcul du Volume Profile avec {n_bins} bins et lookback {lookback}"
        )
        ...
        for i in range(lookback, len(self.df)):
            ...
            poc_price = (
                price_bins[poc_bin]
                + (price_bins[poc_bin + 1] - price_bins[poc_bin]) / 2
            )
    ...
    def calculate_ichimoku_cloud(
        self, tenkan_period=9, kijun_period=26, senkou_span_b_period=52, displacement=26
    ):
        """
        ...
        """
        logger.info(
            f"Calcul de l'Ichimoku Cloud avec périodes {tenkan_period}/{kijun_period}/{senkou_span_b_period}"
        )

        # Fonction pour calculer la ligne Donchian (moyenne du plus haut et du plus bas)
        def donchian(high_prices, low_prices, period):
            return (
                high_prices.rolling(window=period).max()
                + low_prices.rolling(window=period).min()
            ) / 2
        ...
        senkou_span_b = donchian(
            self.df["high"], self.df["low"], senkou_span_b_period
        ).shift(displacement)
