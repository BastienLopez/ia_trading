...
class VariableBatchSampler:
    ...
    def sample(self, prioritized: bool = False, beta: float = None) -> Tuple[Any, Optional[Dict[str, Any]]]:
        """
        ...
        if hasattr(self.buffer, 'sample_batch'):
            ...
            if prioritized and beta is not None:
                ...
                batch, batch_info = self.buffer.sample_batch(batch_size=self.current_batch_size)
        elif hasattr(self.buffer, 'sample'):
            # Interface standard
            if prioritized and beta is not None and 'beta' in self.buffer.sample.__code__.co_varnames:
            ...
            raise ValueError("Buffer incompatible: doit avoir une méthode 'sample' ou 'sample_batch'")
        ...
        info = {
            ...
            "avg_sample_time": sum(self.sample_times[-10:]) / max(1, len(self.sample_times[-10:])),
            "resources": self.get_resource_utilization(),
    ...
    def set_strategy(self, strategy: str):
        ...
        if strategy not in valid_strategies:
            raise ValueError(f"Stratégie invalide. Doit être l'une de {valid_strategies}")
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        if self.batch_sizes_history:
            metrics["avg_batch_size"] = sum(self.batch_sizes_history) / len(self.batch_sizes_history)
        
        # Ajouter l'utilisation des ressources
        if self.resource_history:
            last_resources = self.resource_history[-1]
            metrics.update({f"last_{k}": v for k, v in last_resources.items() if v is not None})
...
class BatchOptimizer:
        ...
        for _ in range(self.warmup_iters):
            ...
            with torch.no_grad():
                if isinstance(batch, tuple) and len(batch) == 5:
                    ...
                    self.loss_fn(self.model, states, actions, rewards, next_states, dones)
                elif hasattr(self.loss_fn, '__code__') and 'batch' in self.loss_fn.__code__.co_varnames:
    ...
    def plot_results(self, save_path: Optional[str] = None):
        ...
        if not self.results:
            print("Aucun résultat à afficher. Exécutez find_optimal_batch_size() d'abord.")
            return
        ...
        plt.plot(sizes[best_idx], throughputs[best_idx], 'ro', markersize=10, 
