...
class RiskManager:
    ...
    def __init__(self, config=None):
        ...
        self.max_position_size = self.config.get('max_position_size', 0.2)  # 20% du capital max
        self.max_risk_per_trade = self.config.get('max_risk_per_trade', 0.02)  # 2% du capital max
        ...
        self.trailing_stop_activation = self.config.get('trailing_stop_activation', 0.02)  # 2% de profit
        self.trailing_stop_distance = self.config.get('trailing_stop_distance', 0.01)  # 1% du prix
        self.volatility_lookback = self.config.get('volatility_lookback', 14)  # Période pour calculer la volatilité
        self.volatility_threshold = self.config.get('volatility_threshold', 0.15)  # Seuil de volatilité (15%)
        self.position_adjustment_factor = self.config.get('position_adjustment_factor', 0.5)  # Réduction de position en cas de haute volatilité
    ...
    def calculate_position_size(self, capital, entry_price, stop_loss_price):
        ...
        if atr is not None and len(atr) > 0:
            risk_adjustment = 1 - (atr.iloc[-1] / entry_price)  # Plus l'ATR est élevé, plus on réduit la position
            position_size *= max(risk_adjustment, 0.2)  # Ne pas descendre en dessous de 20%
        ...
        if volatility > self.volatility_threshold:
            adjustment = 1 - (volatility - self.volatility_threshold) * self.position_adjustment_factor
            position_size *= max(adjustment, 0.2)  # Ne pas descendre en dessous de 20%
            logger.info(f"Ajustement de position basé sur la volatilité ({volatility:.2%} > {self.volatility_threshold:.2%})")
        
        logger.info(f"Taille de position calculée: {position_size} unités")
        return position_size
    
    def calculate_atr_stop_loss(self, data, period=14, direction='long', current_price=None, position_id=None):
        """
        ...
        if data is None or len(data) < period:
            logger.warning(f"Données insuffisantes pour calculer l'ATR (besoin de {period} points)")
            return None
        ...
        if position_id is not None:
            if position_id not in self.position_stops:
                self.position_stops[position_id] = {
                    ...
                    'take_profit': None,
                    'trailing_stop': None,
                    'entry_price': current_price if current_price is not None else data['close'].iloc[-1]
                }
            else:
                self.position_stops[position_id]['stop_loss'] = stop_loss
                self.position_stops[position_id]['entry_price'] = current_price if current_price is not None else data['close'].iloc[-1]
        
        logger.info(f"Stop-loss ATR calculé: {stop_loss} pour position {direction}")
        return stop_loss
    
    def calculate_atr_take_profit(self, data, period=14, direction='long', current_price=None, position_id=None):
        """
        ...
        if position_id is not None:
            if position_id not in self.position_stops:
                self.position_stops[position_id] = {'stop_loss': None, 'take_profit': take_profit, 'trailing_stop': None}
            else:
    ...
    def update_trailing_stop(self, position_id, current_price, entry_price, direction='long'):
        """
        ...
        """
        if position_id not in self.position_stops:
            logger.warning(f"Position {position_id} non trouvée dans le gestionnaire de risques")
            return None
        ...
        if profit_pct >= self.trailing_stop_activation:
            if direction == 'long':
                ...
                if (self.position_stops[position_id]['trailing_stop'] is None or 
                    new_stop > self.position_stops[position_id]['trailing_stop']):
                    self.position_stops[position_id]['trailing_stop'] = new_stop
                    logger.info(f"Trailing stop mis à jour: {new_stop} pour position {position_id}")
            else:  # short
                new_stop = current_price * (1 + self.trailing_stop_distance)
        ...
        if stops['stop_loss'] is not None:
            if (direction == 'long' and current_price <= stops['stop_loss']) or \
               (direction == 'short' and current_price >= stops['stop_loss']):
                result = {'stop_triggered': True, 'stop_type': 'stop_loss', 'stop_price': stops['stop_loss']}
                logger.info(f"Stop-loss déclenché pour position {position_id} à {stops['stop_loss']}")
                return result
        ...
        if stops['take_profit'] is not None:
            if (direction == 'long' and current_price >= stops['take_profit']) or \
               (direction == 'short' and current_price <= stops['take_profit']):
                result = {'stop_triggered': True, 'stop_type': 'take_profit', 'stop_price': stops['take_profit']}
                logger.info(f"Take-profit déclenché pour position {position_id} à {stops['take_profit']}")
                return result
        ...
        if stops['trailing_stop'] is not None:
            if (direction == 'long' and current_price <= stops['trailing_stop']) or \
               (direction == 'short' and current_price >= stops['trailing_stop']):
                result = {'stop_triggered': True, 'stop_type': 'trailing_stop', 'stop_price': stops['trailing_stop']}
                logger.info(f"Trailing stop déclenché pour position {position_id} à {stops['trailing_stop']}")
                return result
    ...
    def update_atr_trailing_stop(self, data, period=14, position_id=None, current_price=None, direction='long'):
        """
        ...
        price_change = (current_price - self.position_stops[position_id]['entry_price']) / self.position_stops[position_id]['entry_price'] * 100
        ...
        if price_change > breakout_threshold:
            ...
            logger.info(f"Breakout détecté ({price_change:.2f}%), ajustement du trailing stop")
    ...
    def update_ma_trailing_stop(self, data, ma_period=20, position_id=None, current_price=None, direction='long'):
        """
    ...
    def calculate_historical_volatility(self, lookback=14):
        ...
        returns = np.log(self.indicators.df['close'] / self.indicators.df['close'].shift(1))
    ...
    def update_volatility_adjusted_stops(self, data, position_id):
        ...
        current_volatility = self.calculate_historical_volatility(self.volatility_lookback)
        original_stop = self.position_stops[position_id]['stop_loss']
        
        # Ajuster le stop-loss si la volatilité augmente
        if current_volatility > self.volatility_threshold:
            adjustment = 1 + (current_volatility - self.volatility_threshold) * 0.5  + 0.5
            new_stop = original_stop * adjustment
            self.position_stops[position_id]['stop_loss'] = new_stop
            logger.info(f"Stop-loss ajusté à {new_stop:.2f} (volatilité {current_volatility:.2%})") 
    ...
    def adjust_action(self, action, portfolio_value, current_crypto, current_price):
        ...
        if self.should_limit_position([portfolio_value], current_crypto):
            ...
            adjustment_factor = max_allowed / position_value if position_value > 0 else 0
            
            # Appliquer l'ajustement progressif
            adjusted_action = action * adjustment_factor * self.config.get('risk_adjustment_factor', 0.7)
