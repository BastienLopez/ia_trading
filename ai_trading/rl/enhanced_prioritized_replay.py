...
class EnhancedPrioritizedReplay:
    ...
    def __init__(
        ...
        logger.info(
            f"Tampon de replay prioritaire amélioré initialisé avec capacité={capacity}"
        )
        logger.info(
            f"Paramètres d'optimisation: redundancy={redundancy_threshold}, "
    ...
    def add(self, state, action, reward, next_state, done, info=None, td_error=None):
        """
        ...
        # Traitement n-step
        processed, state, action, reward, next_state, done, info = (
            self._n_step_processing(state, action, reward, next_state, done, info)
        )
    ...
    def sample(self, batch_size, beta=None):
        ...
        min_priority = (
            np.min(
                self.sum_tree.tree[-self.sum_tree.capacity :][
    ...
    def optimize_buffer(self):
        ...
        self.metrics["buffer_reduction"] = len(redundant_indices) / max(
            1, self.sum_tree.size
        )

        logger.info(
            f"Buffer optimisé: {len(important_indices)} expériences importantes, "
    ...
    def get_metrics(self):
        ...
        if hasattr(self, "sample_count") and self.sample_count > 0:
            self.metrics["sample_efficiency"] = self.metrics[
                "important_experiences"
            ] / max(1, self.sample_count)
...
if __name__ == "__main__":
    ...
    buffer = EnhancedPrioritizedReplay(
        capacity=10000, alpha=0.6, beta=0.4, n_step=3, redundancy_threshold=0.95
    )
