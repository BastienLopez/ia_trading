...
def generate_test_data(periods=100, trend="bullish", volatility=0.02, seed=42):
    ...
    high_prices = close_prices + np.abs(
        np.random.normal(0, daily_volatility * 100, periods)
    )
    low_prices = close_prices - np.abs(
        np.random.normal(0, daily_volatility * 100, periods)
    )
    open_prices = (high_prices + low_prices) / 2 + np.random.normal(
        0, daily_volatility * 50, periods
    )
...
def test_stop_loss_take_profit():
    ...
    risk_config = {"stop_loss_atr_factor": 2.0, "take_profit_atr_factor": 4.0}
    ...
    print(
        f"Stop-loss: {stop_loss:.2f} (distance: {(entry_price - stop_loss) / entry_price * 100:.2f}%)"
    )
    print(
        f"Take-profit: {take_profit:.2f} (distance: {(take_profit - entry_price) / entry_price * 100:.2f}%)"
    )
    ...
    for i, price in enumerate(test_prices):
        result = risk_manager.check_stop_conditions(
            position_id, price, direction="long"
        )
        if result["stop_triggered"]:
            print(
                f"Étape {i+1}: Prix = {price:.2f} => {result['stop_type']} déclenché à {result['stop_price']:.2f}"
            )
        else:
...
def test_trailing_stop():
    ...
    for i, price in enumerate(prices):
        ...
        trailing_stops.append(
            trailing_stop if trailing_stop is not None else initial_stop
        )
        ...
        print(
            f"Jour {i+1}: Prix = {price:.2f}, Trailing Stop = {trailing_stops[-1]:.2f} => {status}"
        )
    ...
    plt.plot(
        range(len(prices[: len(trailing_stops)])),
        prices[: len(trailing_stops)],
        label="Prix",
        color="blue",
    )
    plt.plot(
        range(len(trailing_stops)),
        trailing_stops,
        label="Trailing Stop",
        color="red",
        linestyle="--",
    ...
    plt.axhline(
        y=initial_stop, color="orange", linestyle=":", label="Stop-loss initial"
    )
    ...
    print(
        f"Graphique sauvegardé dans '{os.path.join(VISUALIZATION_DIR, 'trailing_stop_evolution.png')}'"
    )
...
def test_position_size_calculation():
    ...
    for scenario in scenarios:
        ...
        print(f"\n{scenario['name']}:")
        print(
            f"  Stop-loss à {scenario['stop_loss_price']} ({(entry_price - scenario['stop_loss_price']) / entry_price * 100:.1f}% de distance)"
        )
        print(
            f"  Taille de position: {position_size:.2f} unités (valeur: ${position_size * entry_price:.2f})"
        )
...
def test_risk_manager_integration():
    ...
    for i in range(start_idx, len(df) - 1):
            ...
            if (
                ...
                if sma_20.iloc[-1] > sma_50.iloc[-1]:
                    ...
                    if stop_loss is not None and stop_loss < current_price:
                        ...
                        print(
                            f"Jour {i}: Ouverture position LONG à {entry_price:.2f}, "
        ...
        # Si en position, vérifier les stops et mettre à jour le trailing stop
            ...
            if (current_price - entry_price) / entry_price > risk_config[
            ...
            result = risk_manager.check_stop_conditions(
                position_id, current_price, direction=position
            )
            ...
            if result["stop_triggered"]:
                ...
                print(
                    f"Jour {i}: Fermeture position LONG à {exit_price:.2f}, "
    ...
    if trade_results:
        ...
        avg_win = (
            sum(t["profit_loss"] for t in trade_results if t["profit_loss"] > 0)
            / winning_trades
            if winning_trades > 0
            else 0
        )
        ...
        plt.plot(
            ...
            [10000 + p for p in cumulative_pnl],
            label="Capital",
        )
        ...
        print(
            f"Graphique de l'évolution du capital sauvegardé dans '{os.path.join(VISUALIZATION_DIR, 'risk_management_capital.png')}'"
        )
