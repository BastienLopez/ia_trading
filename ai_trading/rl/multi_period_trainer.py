...
class MultiEnvTrading(TradingEnvironment):
    def __init__(self, market_data=None, sentiment_data=None, initial_balance=10000, transaction_fee=0.001, window_size=20, is_training=True):
        pass
...
class MultiPeriodTrainer:
    ...
    def __init__(
        ...
        self.sentiment_collector = EnhancedSentimentCollector() if include_sentiment else None
    ...
    def train(self):
        ...
        market_train, sentiment_train, market_val, sentiment_val = self.prepare_datasets(
            market_data, sentiment_data, self.validation_ratio
        )
        ...
        for epoch in range(self.epochs_per_period):
            ...
            logger.info(f"Train: Reward={metrics['train_rewards'][-1]:.4f}, "
                       f"Return={metrics['train_returns'][-1]:.4f}, "
                       f"Sharpe={metrics['train_sharpe'][-1]:.4f}")
            logger.info(f"Val: Reward={metrics['val_rewards'][-1]:.4f}, "
    ...
    def _train_epoch(self, agent, env):
        ...
        for episode in range(self.episodes_per_epoch):
            ...
            logger.info(f"Episode {episode+1}/{self.episodes_per_epoch}: "
    ...
    def _evaluate(self, agent, env):
        ...
        for episode in range(3):  # Évaluer sur 3 épisodes
            ...
            logger.info(f"Validation Episode {episode+1}/3: "
                       f"Reward={total_reward:.4f}, "
                       f"Portfolio={info['portfolio_value']:.2f}")
    ...
    def save_current_agent(self, custom_name=None):
        ...
        # Créer un nom de fichier unique
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        model_name = custom_name if custom_name else f"{self.agent_type}_{self.symbol}_{'gru_' if self.use_gru else ''}{timestamp}"
    ...
    def train_period(self, period):
        ...
        for epoch in range(self.epochs_per_period):
            ...
            for episode in range(self.episodes_per_epoch):
                ...
                logger.info(f"Épisode {episode+1}/{self.episodes_per_epoch}, Reward: {episode_reward:.2f}")
            
            train_rewards.append(np.mean(epoch_rewards))
            train_returns.append(train_env.portfolio_return if hasattr(train_env, 'portfolio_return') else 0.0)
            
            # Validation
            val_epoch_rewards = []
            for episode in range(max(1, self.episodes_per_epoch // 5)):  # Moins d'épisodes pour la validation
            ...
            val_returns.append(val_env.portfolio_return if hasattr(val_env, 'portfolio_return') else 0.0)
            
            logger.info(f"Époque {epoch+1} - Train Reward: {train_rewards[-1]:.2f}, Val Reward: {val_rewards[-1]:.2f}")
            logger.info(f"Époque {epoch+1} - Train Return: {train_returns[-1]:.2f}%, Val Return: {val_returns[-1]:.2f}%")
    ...
    def create_environment(self, data, is_training=True):
        ...
        return self.mock_env_instance if hasattr(self, 'mock_env_instance') else MagicMock()
    ...
    def run_curriculum(self):
        ...
        for i, period in enumerate(self.periods):
            ...
            (train_market, train_sentiment), (val_market, val_sentiment) = self._prepare_data_for_period(
                market_data, sentiment_data, period
            )
            ...
            for epoch in range(self.epochs_per_period):
                ...
                with self.summary_writer.as_default():
                    tf.summary.scalar(f"{period}min/train_reward", train_metrics[0], step=epoch)
                    tf.summary.scalar(f"{period}min/train_portfolio", train_metrics[1], step=epoch)
                    tf.summary.scalar(f"{period}min/val_reward", val_metrics[0], step=epoch)
                    tf.summary.scalar(f"{period}min/val_portfolio", val_metrics[1], step=epoch)
                
                # Log des métriques
                logger.info(f"Train - Reward: {train_metrics[0]:.4f}, Portfolio: ${train_metrics[1]:.2f}")
                logger.info(f"Val - Reward: {val_metrics[0]:.4f}, Portfolio: ${val_metrics[1]:.2f}")
    ...
    def _resample_data(self, data: pd.DataFrame, rule: str) -> pd.DataFrame:
        ...
        if not isinstance(data.index, pd.DatetimeIndex):
            if 'date' in data.columns:
                ...
                raise ValueError("Les données doivent avoir une colonne 'date' ou un index DatetimeIndex")
        ...
        for col in numeric_cols:
            if col in ['open', 'close', 'high', 'low', 'sentiment_score', 'sentiment_magnitude']:
                agg_dict[col] = 'mean'
            elif col == 'volume':
                agg_dict[col] = 'sum'
