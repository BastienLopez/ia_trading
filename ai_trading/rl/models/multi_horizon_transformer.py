...
class LongRangeTemporalAttention(nn.Module):
    ...
    def forward(
        ...
        k = (
            self.k_proj(key)
            .view(batch_size, seq_len_k, self.n_heads, self.head_dim)
            .transpose(1, 2)
        )
        v = (
            self.v_proj(value)
        ...
        if self.use_relative_positions:
            ...
            rel_pos_encodings = self.rel_pos_encoding[
                rel_pos_indices
            ...
            q_reshaped = q.permute(2, 0, 1, 3).reshape(
                seq_len_q, batch_size * self.n_heads, self.head_dim
            )

            # Adapter les encodages relatifs pour le produit matriciel
            rel_pos_encodings_expanded = rel_pos_encodings.permute(
                0, 2, 1
            ...
            rel_scores = rel_scores.reshape(
                seq_len_q, batch_size, self.n_heads, seq_len_k
            )
            rel_scores = rel_scores.permute(
                1, 2, 0, 3
...
class MultiHorizonTemporalTransformer(nn.Module):
    ...
    def forward(
        ...
        Dict[int, torch.Tensor], Tuple[Dict[int, torch.Tensor], List[torch.Tensor]]
    ]:
        """
        ...
        x_permuted = x.permute(
            1, 0, 2
        )  # [seq_len, batch_size, d_model] pour MultiheadAttention
        query_permuted = query.permute(1, 0, 2)  # [1, batch_size, d_model]

        global_context, _ = self.global_attention(
            query=query_permuted, key=x_permuted, value=x_permuted
        )
    ...
    def predict(
        self, x: torch.Tensor, horizon_idx: Optional[int] = None
    ) -> torch.Tensor:
        """
        ...
        with torch.no_grad():
            ...
            if horizon_idx is not None:
                ...
                result = torch.cat(
                    [predictions[h] for h in self.forecast_horizons], dim=1
                )
                return result
