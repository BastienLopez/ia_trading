...
class PricePredictionHead(nn.Module):
    ...
    def __init__(
        ...
        self.price_heads = nn.ModuleDict({
            f"h{h}": nn.Linear(hidden_dim, price_targets) 
            for h in horizons
        })
        
        self.volume_heads = nn.ModuleDict({
            f"h{h}": nn.Linear(hidden_dim, volume_targets)
            for h in horizons
        })
...
class TrendClassificationHead(nn.Module):
    ...
    def __init__(
        ...
        self.classification_heads = nn.ModuleDict({
            f"h{h}": nn.Linear(hidden_dim, num_classes)
            for h in horizons
        })
...
class RiskManagementHead(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        stop_loss = torch.sigmoid(self.stop_loss_head(risk_features)) * 0.2  # 0-20% de stop loss
        take_profit = torch.sigmoid(self.take_profit_head(risk_features)) * 0.5  # 0-50% de take profit
        position_size = torch.sigmoid(self.position_size_head(risk_features))  # 0-100% de la taille de position
        risk_score = torch.sigmoid(self.risk_score_head(risk_features))  # 0-1 score de risque
...
class MultitaskLearningModel(nn.Module):
    ...
    def __init__(
        ...
        self.task_weights = task_weights if task_weights is not None else default_weights
    ...
    def compute_price_volume_loss(
        ...
        for horizon_key, horizon_preds in predictions.items():
            ...
            price_loss = F.mse_loss(
                horizon_preds['price'], 
                targets[horizon_key]['price']
            )
            
            volume_loss = F.mse_loss(
                horizon_preds['volume'], 
                targets[horizon_key]['volume']
            )
    ...
    def compute_portfolio_optimization_loss(
        ...
        if predictions.size(1) != targets.size(1):
            logger.warning(f"Dimensions différentes: predictions {predictions.shape}, targets {targets.shape}")
    ...
    def compute_combined_loss(
        ...
        if 'trend_classification' in targets:
            trend_loss = self.compute_trend_classification_loss(
                outputs['trend_classification'], 
                targets['trend_classification']
            )
