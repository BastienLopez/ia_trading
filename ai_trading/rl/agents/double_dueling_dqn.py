...
class DoubleDQNAgent:
    ...
    def _build_model(self):
        ...
        if self.use_dueling:
            ...
            action_advantages_mean = Lambda(lambda x: tf.reduce_mean(x, axis=1, keepdims=True))(action_advantages)
            action_advantages = Lambda(lambda x: x[0] - x[1])([action_advantages, action_advantages_mean])
    ...
    def train(self):
        ...
        targets = rewards + (1 - dones) * self.discount_factor * target_next_q_values[np.arange(self.batch_size), next_actions]
        ...
        if self.use_prioritized_replay and indices is not None:
            self.memory.update_priorities(indices, td_errors + 1e-6)  # Ajouter un petit epsilon pour éviter les priorités nulles
