...
class TransformerPPOActor(nn.Module):
    ...
    def get_action_and_log_prob(
        self, states_seq: torch.Tensor, deterministic: bool = False
...
class TransformerPPOAgent:
    ...
    def prepare_state_sequences(
        self, states: torch.Tensor, sequence_length: int
    ) -> torch.Tensor:
        """
    ...
    def update(
        ...
        with torch.no_grad():
            ...
            next_state_sequences = self.prepare_state_sequences(
                torch.cat([states[1:], next_states[-1:]], dim=0), self.sequence_length
            )
        ...
        with torch.no_grad():
            old_actions, old_log_probs = self.actor.get_action_and_log_prob(
                state_sequences
            )
        ...
        for _ in range(self.update_epochs):
            ...
            for start_idx in range(0, batch_size, self.mini_batch_size):
                ...
                new_actions, new_log_probs = self.actor.get_action_and_log_prob(
                    mb_states
                )
