...
class NoisyDense(Layer):
    ...
    def __init__(self, units, activation=None, sigma_init=0.5, bias_regularizer=None, **kwargs):
        """
    ...
    def build(self, input_shape):
        ...
        self.weight_sigma = self.add_weight(
            shape=(self.input_dim, self.units),
            initializer=tf.keras.initializers.Constant(self.sigma_init / tf.math.sqrt(tf.cast(self.input_dim, tf.float32))),
            name='weight_sigma',
            trainable=True
        )
    ...
    def call(self, inputs, training=None):
        ...
        def noisy_call():
            ...
            epsilon_w = tf.tensordot(
                self.epsilon_in, self.epsilon_out, axes=0
            )
        ...
        weights, bias = tf.cond(
            tf.constant(training, dtype=tf.bool) if training is not None else tf.constant(True, dtype=tf.bool),
            noisy_call,
            non_noisy_call
        )
