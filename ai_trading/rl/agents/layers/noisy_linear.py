...
class NoisyLinear(nn.Module):
    ...
    def __init__(
        ...
        if factorised_noise:
            ...
            self.register_buffer(
                "epsilon_out", torch.empty(out_features, device=device)
            )
        else:
            self.register_buffer(
            ...
            self.register_buffer(
                "bias_epsilon",
                torch.empty(out_features, device=device) if bias else None,
            )
    ...
    def sample_noise(
        ...
        """
        if self.factorised_noise:
            epsilon_in_samples = torch.stack(
                [self._scale_noise(self.in_features) for _ in range(batch_size)]
            )
            epsilon_out_samples = torch.stack(
                [self._scale_noise(self.out_features) for _ in range(batch_size)]
            )

            # Reshape pour broadcast
            # [batch_size, out_features, 1] * [batch_size, 1, in_features]
            weight_epsilons = torch.bmm(
                epsilon_out_samples.unsqueeze(2), epsilon_in_samples.unsqueeze(1)
            )
            ...
            bias_epsilons = (
                torch.randn(batch_size, self.out_features, device=self.weight_mu.device)
                if self.use_bias
                else None
            )
