...
class MultitaskTradingAgent:
            ...
            for state in states:
                if isinstance(state, np.ndarray):
                    if len(state) != self.state_dim:
                        ...
                        uniform_state[: min(len(state), self.state_dim)] = state[
                            : min(len(state), self.state_dim)
                        ]
                    else:
                        uniform_state = state
                else:
    ...
    def process_multitask_outputs(self, outputs):
        ...
        try:
            ...
            encoded_features = torch.zeros(
                ...
                device=outputs["portfolio_optimization"].device,
            )
            ...
            logger.error(
                f"Dimensions: trend_probs: {trend_probs.shape}, portfolio: {portfolio_alloc.shape}, risk: {risk_params.shape}"
            )
            # Retourner un tenseur de dimension compatible en cas d'erreur
            return torch.zeros(
                (batch_size, self.policy_network[0].in_features),
                device=outputs["portfolio_optimization"].device,
            )
    ...
    def _create_simulated_targets(self, states, next_states, rewards):
        ...
        portfolio_targets = (
            torch.ones(batch_size, self.num_assets, device=self.device)
            / self.num_assets
        )
