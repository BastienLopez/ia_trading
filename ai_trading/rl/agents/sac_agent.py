...
class SequenceReplayBuffer:
    ...
    def _correct_sequence_shape(self, sequence):
        ...
        logging.warning(
            f"Impossible de corriger la forme de la séquence {sequence.shape}. Utilisation de zéros."
        )
        return np.zeros(
            (
                self.sequence_length,
                sequence.shape[-1] if len(sequence.shape) > 1 else sequence.shape[0],
            )
        )
    ...
    def sample(self, batch_size):
        ...
        if len(states_seqs.shape) > 3:
            logging.warning(
                f"Forme incorrecte détectée dans states_seqs échantillonnés: {states_seqs.shape}"
            )
        ...
        if len(next_states_seqs.shape) > 3:
            logging.warning(
                f"Forme incorrecte détectée dans next_states_seqs échantillonnés: {next_states_seqs.shape}"
            )
            # Si la forme est (batch_size, 1, sequence_length, state_size)
            if next_states_seqs.shape[1] == 1:
                next_states_seqs = next_states_seqs.reshape(
        ...
        if len(next_states_seqs.shape) == 2:
            logging.warning(
                f"next_states_seqs n'est pas une séquence: {next_states_seqs.shape}. Création de séquences."
            )
            # Créer une séquence pour chaque état
            repeated_next_states = []
            for next_state in next_states_seqs:
                repeated_next_states.append(
                    np.array([next_state] * self.sequence_length)
                )
        ...
        if np.any(np.isnan(states_seqs)):
            logging.warning(
                "NaN détectés dans les états échantillonnés. Remplacés par 0."
            )
            states_seqs = np.nan_to_num(states_seqs, nan=0.0)

        if np.any(np.isnan(next_states_seqs)):
            logging.warning(
                "NaN détectés dans les états suivants échantillonnés. Remplacés par 0."
            )
...
class SACAgent:
    ...
    def __init__(
        ...
        if self.use_gru:
            self.memory = SequenceReplayBuffer(
                buffer_size=buffer_size, sequence_length=sequence_length
            )
        else:
        ...
        logging.info(
            f"SACAgent initialisé avec state_size={state_size}, action_size={action_size}, "
    ...
    def _build_gru_critic_network(self):
        ...
        state_inputs = tf.keras.layers.Input(
        ...
        state_features = tf.keras.layers.GRU(
            units=self.gru_units, return_sequences=False
        )(state_inputs)
    ...
    def _normalize_sequence_states(self, state_sequence):
        ...
        if tf.reduce_any(tf.math.is_nan(state_sequence)):
            logging.warning(
                "NaNs détectés dans la séquence d'états pendant la normalisation"
            )
        ...
        batch_size, seq_length, state_size = (
            tf.shape(state_sequence)[0],
            tf.shape(state_sequence)[1],
            tf.shape(state_sequence)[2],
        )
        ...
        normalized_sequence = tf.reshape(
            norm_flat_states, [batch_size, seq_length, state_size]
        )
    ...
    def _get_action_and_log_prob(self, state, is_evaluation=False):
        ...
        if tf.reduce_any(tf.math.is_nan(state_tensor)):
            ...
            state_tensor = tf.where(
                tf.math.is_nan(state_tensor), tf.zeros_like(state_tensor), state_tensor
            )
        ...
        if tf.reduce_any(tf.math.is_nan(log_std)):
            ...
            log_std = tf.where(
                tf.math.is_nan(log_std), -20.0 * tf.ones_like(log_std), log_std
            )
        ...
        if tf.reduce_any(tf.math.is_nan(squashed_action)):
            ...
            squashed_action = tf.where(
                tf.math.is_nan(squashed_action),
                tf.zeros_like(squashed_action),
                squashed_action,
            )
        ...
        if tf.reduce_any(tf.math.is_nan(log_prob)):
            ...
            log_prob = tf.where(
                tf.math.is_nan(log_prob), -1000.0 * tf.ones_like(log_prob), log_prob
            )
    ...
    def act(self, state, evaluate=False):
        ...
        try:
            ...
            if isinstance(state, np.ndarray):
                try:
                    if np.any(np.isnan(state)):
                        logging.warning(
                            "NaN détectés dans l'état. Remplacés par zéros."
                        )
                        state = np.nan_to_num(state, nan=0.0)
                except TypeError:
                    # Si le type n'est pas compatible avec isnan, convertir en float
                    logging.warning(
                        "Type de données non compatible avec isnan. Conversion en float32."
                    )
            ...
            action, _ = self._get_action_and_log_prob(
                state_input, is_evaluation=evaluate
            )
    ...
    def _scale_action(self, action):
        ...
        if np.any(np.isnan(action)):
            print(
                "[WARNING] NaN détecté dans l'action avant mise à l'échelle. Remplacement par zéro."
            )
        ...
        if np.any(np.isnan(scaled)):
            print(
                "[WARNING] NaN détecté après mise à l'échelle. Remplacement par la limite inférieure."
            )
    ...
    def _unscale_action(self, scaled_action):
        ...
        if np.any(np.isnan(scaled_action)):
            print(
                "[WARNING] NaN détecté dans l'action mise à l'échelle. Remplacement par la limite inférieure."
            )
        ...
        if np.any(np.isnan(unscaled)):
            print(
                "[WARNING] NaN détecté après inversion de mise à l'échelle. Remplacement par zéro."
            )
    ...
    def _train_step(self, states, actions, rewards, next_states, dones):
        ...
        if self.grad_clip_value is not None:
            critic1_gradients = [
                tf.clip_by_norm(g, self.grad_clip_value) if g is not None else g
    ...
    def train(self):
        ...
        if self.use_gru:
            ...
            states_seqs, actions, rewards, next_states_seqs, dones = self.memory.sample(
                self.batch_size
            )

            # Vérifier les formes
            logging.debug(
                f"Formes échantillonnées - states_seqs: {states_seqs.shape}, next_states_seqs: {next_states_seqs.shape}"
            )
