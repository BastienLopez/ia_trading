...
class NStepSACAgent(SACAgent):
    ...
    def _train_step(self, states, actions, rewards, next_states, dones):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            target_q = rewards + (1 - dones) * self.n_step_discount_factor * next_q_value
    ...
    def save(self, filepath):
        ...
        logger.info(f"Informations sur les retours multi-étapes sauvegardées dans {filepath}")
    ...
    def load(self, filepath):
        ...
        if os.path.exists(n_step_info_path):
            ...
            self.n_step_discount_factor = float(n_step_info["n_step_discount_factor"][0])
            ...
            logger.info(f"Informations sur les retours multi-étapes chargées depuis {filepath}")
        else:
            logger.warning(f"Aucune information sur les retours multi-étapes trouvée dans {filepath}")
