    ...
    for episode in range(n_episodes):
        ...
        for step in range(max_steps):
            try:
                ...
                logger.error(
                    f"Erreur pendant l'entraînement (épisode {episode+1}, étape {step+1}): {str(e)}"
                )
        ...
        stats["actor_losses"].append(
            np.mean(episode_actor_losses) if episode_actor_losses else 0.0
        )
        stats["critic_losses"].append(
        ...
        if (episode + 1) % 10 == 0:
            ...
            logger.info(
                f"Valeur finale du portefeuille: {portfolio_values[-1] if portfolio_values else 0:.2f}"
            )
            logger.info(f"Ratio de Sharpe: {sharpe_ratio:.2f}")
            logger.info(f"Max Drawdown: {max_drawdown:.2%}")
            logger.info(
                f"Perte moyenne acteur: {np.mean(episode_actor_losses) if episode_actor_losses else 0:.4f}"
            )
            logger.info(
                f"Perte moyenne critique: {np.mean(episode_critic_losses) if episode_critic_losses else 0:.4f}"
            )
...
def main():
    ...
    data_path = os.path.join(
        os.path.dirname(__file__), "../data/processed/synthetic_mixed_1h.csv"
    )
    ...
    logger.info(
        f"Rendement moyen sur l'entraînement: {np.mean(train_stats['episode_returns']):.2f}"
    )
    logger.info(
        f"Ratio de Sharpe moyen sur l'entraînement: {np.mean(train_stats['sharpe_ratios']):.2f}"
    )
    logger.info(
        f"Rendement moyen sur l'évaluation: {np.mean(eval_stats['returns']):.2f}"
    )
    logger.info(
        f"Ratio de Sharpe moyen sur l'évaluation: {np.mean(eval_stats['sharpe_ratios']):.2f}"
    )
    logger.info(
        f"Ratio de Sortino moyen sur l'évaluation: {np.mean(eval_stats['sortino_ratios']):.2f}"
    )
