...
class StateCache:
    ...
    def _save_to_disk(self, state_hash: str, state, value, metadata: Dict):
        ...
        try:
            data = {"value": value, "metadata": metadata, "timestamp": time.time()}
        ...
        with self.lock:
                ...
                for h, entry in list(self.cache.items()):
                    ...
                    if self._states_similar(state, cached_state):
                        ...
                        if h in self.state_metrics:
                            ...
                            self.state_metrics[h]["similar_hits"] = (
                                self.state_metrics[h].get("similar_hits", 0) + 1
                            )
            ...
            if disk_entry:
                ...
                if self._states_similar(state, disk_state):
                    ...
                    if state_hash in self.state_metrics:
                        self.state_metrics[state_hash]["hits"] += 1
                        self.state_metrics[state_hash]["disk_hits"] = (
                            self.state_metrics[state_hash].get("disk_hits", 0) + 1
                        )
                        self.state_metrics[state_hash]["last_access"] = time.time()
                    else:
    ...
    def put(self, state, value, metadata: Optional[Dict] = None) -> str:
        ...
        with self.lock:
            ...
            if state_hash in self.state_metrics:
                self.state_metrics[state_hash]["updates"] = (
                    self.state_metrics[state_hash].get("updates", 0) + 1
                )
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            if self.state_metrics:
                ...
                metrics["top_states"] = [
                    {"hash": h, "hits": hits} for h, hits in top_hits
                ]
    ...
    def get_state_info(self, state_or_hash) -> Optional[Dict]:
        ...
        with self.lock:
            ...
            if metrics or cache_entry:
                info = {"hash": state_hash, "in_memory_cache": cache_entry is not None}
