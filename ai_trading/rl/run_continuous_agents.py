...
def evaluate_agent(agent, env: TradingEnvironment, n_episodes: int = 10) -> Dict[str, List[float]]:
    returns = []
    sharpe_ratios = []
    max_drawdowns = []
    ...
    for episode in range(n_episodes):
        ...
        while not (done or truncated):
            ...
            if isinstance(agent, PPOAgent):
                action, _ = agent.get_action(state, deterministic=True)  # Ignorer le log_prob
            else:
                action = agent.get_action(state, deterministic=True)
            
            next_state, reward, done, truncated, _ = env.step(action)  # Ignorer les infos supplémentaires
...
def train_agent(agent, env: TradingEnvironment, n_episodes: int = 1000) -> Dict[str, List[float]]:
    returns = []
    actor_losses = []
    critic_losses = []
...
def plot_results(ppo_results: Dict[str, List[float]], sac_results: Dict[str, List[float]]):
...
def main():
    ...
    data_path = os.path.join(os.path.dirname(__file__), '../data/processed/btc_usd_1h.csv')
