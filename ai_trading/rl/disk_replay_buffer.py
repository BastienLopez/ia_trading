...
class DiskReplayBuffer:
    ...
    def _flush_cache(self, force: bool = False):
        ...
        """
        if not self.cache["states"] or (len(self.cache["states"]) < self.cache_size and not force):
            return
        ...
        with h5py.File(self.h5_path, "r+") as f:
            ...
            if pos + n_items > self.buffer_size:
                ...
                f["rewards"][pos:] = np.array(self.cache["rewards"][:first_part]).reshape(-1, 1)
                f["next_states"][pos:] = np.array(self.cache["next_states"][:first_part])
                f["dones"][pos:] = np.array(self.cache["dones"][:first_part]).reshape(-1, 1)
                
                # Écrire la seconde partie (retour au début du buffer)
                remaining = n_items - first_part
                if remaining > 0:
                    f["states"][:remaining] = np.array(self.cache["states"][first_part:])
                    f["actions"][:remaining] = np.array(self.cache["actions"][first_part:])
                    f["rewards"][:remaining] = np.array(self.cache["rewards"][first_part:]).reshape(-1, 1)
                    f["next_states"][:remaining] = np.array(self.cache["next_states"][first_part:])
                    f["dones"][:remaining] = np.array(self.cache["dones"][first_part:]).reshape(-1, 1)
                ...
                f["rewards"][pos:pos+n_items] = np.array(self.cache["rewards"]).reshape(-1, 1)
                f["next_states"][pos:pos+n_items] = np.array(self.cache["next_states"])
                f["dones"][pos:pos+n_items] = np.array(self.cache["dones"]).reshape(-1, 1)
    ...
    def sample(
        ...
        if self.metrics["total_reads"] % 100 == 0:
            logger.debug(f"Batch lu depuis le disque en {time.time() - start_time:.4f}s")
    ...
    def load(cls, storage_path: str, metadata_path: Optional[str] = None):
        ...
        if not os.path.exists(metadata_path):
            raise FileNotFoundError(f"Fichier de métadonnées non trouvé: {metadata_path}")
