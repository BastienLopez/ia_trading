...
def test_multitask_model():
    ...
    assert (
        "trend_classification" in outputs
    ), "La sortie trend_classification est manquante"
    assert (
        "portfolio_optimization" in outputs
    ), "La sortie portfolio_optimization est manquante"
    ...
    for h in prediction_horizons:
        h_key = f"h{h}"
        assert (
            h_key in outputs["price_prediction"]
        ), f"Horizon {h_key} manquant dans price_prediction"
        assert (
            "price" in outputs["price_prediction"][h_key]
        ), f"price manquant dans {h_key}"
        assert (
            "volume" in outputs["price_prediction"][h_key]
        ), f"volume manquant dans {h_key}"

        assert outputs["price_prediction"][h_key]["price"].shape == (
            batch_size,
            4,
        ), f"Forme incorrecte pour {h_key} price: {outputs['price_prediction'][h_key]['price'].shape}"
        assert outputs["price_prediction"][h_key]["volume"].shape == (
            batch_size,
            1,
    ...
    for h in prediction_horizons:
        h_key = f"h{h}"
        assert (
            h_key in outputs["trend_classification"]
        ), f"Horizon {h_key} manquant dans trend_classification"
        assert outputs["trend_classification"][h_key].shape == (
            batch_size,
            num_trend_classes,
    ...
    assert outputs["portfolio_optimization"].shape == (
        batch_size,
        num_assets,
    ...
    for param in risk_params:
        assert (
            param in outputs["risk_management"]
        ), f"Paramètre {param} manquant dans risk_management"
        assert outputs["risk_management"][param].shape == (
            batch_size,
            1,
...
def test_multitask_agent():
    ...
    assert action.shape == (
        action_dim,
    ), f"Forme incorrecte pour action: {action.shape}"
