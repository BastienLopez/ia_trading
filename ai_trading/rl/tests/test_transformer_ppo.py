...
def setup_environment(df=None, use_synthetic_data=True):
    ...
    if df is None and use_synthetic_data:
        ...
        if os.path.exists(data_path):
            ...
            logger.warning(
                f"Données réelles non trouvées à {data_path}. Utilisation de données synthétiques."
            )
            df = generate_synthetic_data()
...
def train_transformer_ppo(env, n_episodes=100, sequence_length=50):
    ...
    for episode in range(n_episodes):
        ...
        if len(states) > 0:
            ...
            sharpe_ratio = (
                calculate_sharpe_ratio(episode_reward_list)
                if episode_reward_list
                else 0
            )
            max_drawdown = (
                calculate_max_drawdown(portfolio_values) if portfolio_values else 0
            )
    ...
    if episode_portfolio_values:
        all_portfolio_values = [
            item for sublist in episode_portfolio_values for item in sublist
        ]
    ...
    visualize_results(
        ...
        [
            ...
            for values in episode_portfolio_values
            if values
        ],
    )
...
def evaluate_agent(agent, env, n_episodes=10):
    ...
    for episode in range(n_episodes):
        ...
        max_drawdowns.append(
            calculate_max_drawdown(portfolio_values) if portfolio_values else 0.0
        )
...
def main():
    ...
    agent, env, train_metrics = train_transformer_ppo(
        env, n_episodes=100, sequence_length=50
    )
