...
RESULTS_DIR = BASE_DIR / "ai_trading" / "info_retour" / "visualisations" / "multitask_agent"
...
class SimpleTradingEnvironment:
    ...
    def render(self, title="Performance de l'agent multi-tâches"):
        ...
        benchmark_values = self.close_prices[self.window_size:self.window_size + len(portfolio_values)]
...
def train_multitask_agent(env, agent, n_episodes=100, save_path=None):
    ...
    for episode in range(n_episodes):
        ...
        pbar = tqdm(desc=f"Épisode {episode+1}/{n_episodes}", total=len(env.close_prices) - env.window_size)
        ...
        logger.info(f"Épisode {episode+1}: Rendement total = {metrics['total_return']:.2%}, Sharpe = {metrics['sharpe_ratio']:.2f}")
...
def evaluate_agent(env, agent, visualize=True):
    ...
    logger.info(f"Évaluation: Rendement total = {metrics['total_return']:.2%}, Sharpe = {metrics['sharpe_ratio']:.2f}")
