...
from ai_trading.rl.trainer.multitask_trainer import (
    FinancialMultitaskDataset,
    MultitaskTrainer,
)
        ...
        for inputs, targets in test_loader:
            ...
            for h_key, h_preds in outputs["price_prediction"].items():
                if h_key not in all_preds["price_prediction"]:
                    ...
                    metrics["price_prediction_mse"][h_key] = {
                        "price": 0.0,
                        "volume": 0.0,
                    }
                ...
                if h_key in targets["price_prediction"]:
                    ...
                    all_targets["price_prediction"][h_key]["volume"].append(
                        target_volume
                    )
            ...
            for h_key, h_preds in outputs["trend_classification"].items():
                ...
                if h_key in targets["trend_classification"]:
                    ...
                    target_class = (
                        targets["trend_classification"][h_key].squeeze().numpy()
                    )
...
def visualize_predictions(results, title="Prédictions multi-tâches"):
    ...
    if horizon_key in results["predictions"]["price_prediction"]:
        ...
        pred_prices = np.concatenate(
            results["predictions"]["price_prediction"][horizon_key]["price"]
        )
        target_prices = np.concatenate(
            results["targets"]["price_prediction"][horizon_key]["price"]
        )

        # Tracer les prix pour quelques exemples
        n_examples = min(100, len(pred_prices))
        plt.plot(
            pred_prices[:n_examples, 3], label="Prédiction (close)"
        )  # Indice 3 pour le prix de clôture
    ...
    if horizon_key in results["predictions"]["trend_classification"]:
        # Concaténer toutes les prédictions
        pred_trends = np.concatenate(
            results["predictions"]["trend_classification"][horizon_key]
        )
        target_trends = np.concatenate(
            results["targets"]["trend_classification"][horizon_key]
        )
        ...
        for i in range(3):
            plt.bar(
                ...
                confusion_norm[:, i],
                width,
                label=f"Prédit {trend_labels[i]}",
            )

        plt.xlabel("Tendance réelle")
        plt.ylabel("Proportion")
        plt.title(
            f"Matrice de confusion pour la classification de tendances (horizon {horizon_key})"
        )
    ...
    if results["predictions"]["portfolio_optimization"]:
        # Concaténer toutes les prédictions
        pred_allocations = np.concatenate(
            results["predictions"]["portfolio_optimization"]
        )
        target_allocations = np.concatenate(
            results["targets"]["portfolio_optimization"]
        )
    ...
    if all(param in results["predictions"]["risk_management"] for param in risk_params):
        ...
        for param in risk_params:
            pred_values = np.concatenate(
                results["predictions"]["risk_management"][param]
            )
        ...
        plt.bar(
            x - width / 2,
            [param_values[p]["target"] for p in risk_params],
            width,
            label="Cible",
        )
        plt.bar(
            x + width / 2,
            [param_values[p]["pred"] for p in risk_params],
            width,
            label="Prédiction",
        )
...
def main():
    ...
    logger.info(
        f"DataLoaders préparés: {len(train_loader)} batches d'entraînement, {len(val_loader)} batches de validation"
    )
    ...
    logger.info(
        f"Modèle créé avec {sum(p.numel() for p in model.parameters())} paramètres"
    )
    ...
    logger.info(
        f"MSE Prix (h1): {test_results['metrics']['price_prediction_mse'].get('h1', {}).get('price', 'N/A')}"
    )
    logger.info(
        f"Précision Classification (h1): {test_results['metrics']['trend_classification_accuracy'].get('h1', 'N/A')}"
    )
    logger.info(
        f"MSE Portefeuille: {test_results['metrics']['portfolio_optimization_mse']}"
    )
