...
class FinancialMultitaskDataset(Dataset):
    ...
    def __init__(
        ...
        if asset_columns is None:
            ...
            self.asset_columns = data.select_dtypes(
                include=[np.number]
            ).columns.tolist()
        else:
    ...
    def _calculate_trends(self):
        ...
        for horizon in self.prediction_horizons:
            ...
            for i in range(len(trends)):
                pct_change = (
                    close_prices[i + horizon] - close_prices[i]
                ) / close_prices[i]
    ...
    def _prepare_risk_parameters(self):
        ...
        if "close" in self.data.columns:
            ...
            for i in range(1, len(self.data)):
                ...
                self.risk_parameters["stop_loss"][i] = min(
                    0.2, volatility * 2
                )  # 2x volatilité, max 20%
                self.risk_parameters["take_profit"][i] = min(
                    0.5, volatility * 4
                )  # 4x volatilité, max 50%
                self.risk_parameters["position_size"][i] = max(
                    0.1, 1.0 - volatility * 5
                )  # Inversement proportionnel
                self.risk_parameters["risk_score"][i] = min(
                    0.9, volatility * 10
                )  # Proportionnel, max 90%
        else:
            # Valeurs par défaut si pas de prix de clôture
            self.risk_parameters["stop_loss"] = np.random.uniform(
                0.02, 0.1, len(self.data)
            )
            self.risk_parameters["take_profit"] = np.random.uniform(
                0.1, 0.3, len(self.data)
            )
            self.risk_parameters["position_size"] = np.random.uniform(
                0.3, 0.8, len(self.data)
            )
            self.risk_parameters["risk_score"] = np.random.uniform(
                0.1, 0.6, len(self.data)
            )
    ...
    def __getitem__(self, idx):
        ...
        window_data = (
            self.data.iloc[real_idx - self.window_size : real_idx]
        ...
        for h in self.prediction_horizons:
            ...
            if future_idx < len(self.data):
                ...
                # Volume
                volume_target = (
                    future_data["volume"] if "volume" in future_data else 0.0
                )
                volume_target = (
                    (volume_target - window_mean[0, 4]) / window_std[0, 4]
        ...
        tensor_targets["portfolio_optimization"] = torch.FloatTensor(
...
class MultitaskTrainer:
    ...
    def train_epoch(self) -> Dict[str, float]:
        ...
        for inputs, targets in self.train_loader:
            ...
            # Optimisation de portefeuille
            tensor_targets["portfolio_optimization"] = targets[
                "portfolio_optimization"
            ].to(self.device)
    ...
    def train(
        ...
        for epoch in range(num_epochs):
            ...
            if val_losses and val_losses["total"] < best_val_loss:
                ...
                logger.info(
                    f"Nouveau meilleur modèle sauvegardé avec perte de validation {best_val_loss:.4f}"
                )
            elif val_losses:
                no_improve_count += 1
