...
class PredictionCache:
    ...
    def _cleanup(self) -> None:
        ...
        with self.lock:
            ...
            if expired_keys:
                logger.debug(
                    f"Nettoyage du cache: {len(expired_keys)} entrées supprimées"
                )
...
class ParallelOperations:
    ...
    def parallel_model_inference(
        model: nn.Module, inputs: List[torch.Tensor], batch_size: Optional[int] = None
    ) -> List[torch.Tensor]:
        """
        ...
        if torch.cuda.device_count() > 1:
            ...
            # Créer des batchs
            batches = [
                torch.stack(inputs[i : i + batch_size])
