...
class CrossPlatformCompiler:
    ...
    def compile(
        ...
        if not CrossPlatformCompiler.is_available():
            if IS_WINDOWS:
                logger.warning(
                    "torch.compile n'est pas supporté sur Windows. Le modèle original sera utilisé."
                )
            else:
                logger.warning(
                    "torch.compile n'est pas disponible. Le modèle original sera utilisé."
                )
            return model
        ...
        try:
            ...
            compiled_model = torch.compile(
                model, mode=mode, fullgraph=fullgraph, dynamic=dynamic, **options
            )
            logger.info(f"Modèle {type(model).__name__} compilé avec torch.compile()")
            return compiled_model
        except Exception as e:
            logger.warning(
                f"Erreur lors de la compilation avec torch.compile: {str(e)}"
            )
...
def create_jit_function_if_compile_not_available(func):
    ...
    if is_compile_available():
        ...
        try:
            logger.info(
                f"torch.compile n'est pas disponible, utilisation de torch.jit.script pour {func.__name__}"
            )
...
def create_optimized_model(model_class: Type[nn.Module], *args, **kwargs) -> nn.Module:
    ...
    if is_compile_available():
        ...
        logger.warning(
            f"torch.compile n'est pas supporté sur Windows. {model_class.__name__} non optimisé."
        )
    else:
        logger.warning(
            f"torch.compile n'est pas disponible. {model_class.__name__} non optimisé."
        )
...
def optimize_model_for_inference(model: nn.Module) -> nn.Module:
    ...
    try:
        ...
        example_forward_input = (
            next(model.parameters()).new_zeros(1, *model.input_size)
