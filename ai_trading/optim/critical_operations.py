...
def use_jit_script(func):
    ...
    try:
        ...
        logger.warning(
            f"Impossible d'appliquer torch.jit.script à {func.__name__} : {str(e)}"
        )
...
def optimize_model_with_compile(
    model: nn.Module, options: Optional[Dict[str, Any]] = None
) -> nn.Module:
    """
    ...
    if platform.system() == "Windows":
        logger.warning(
            "torch.compile() n'est pas encore pris en charge sur Windows. Le modèle original sera utilisé."
        )
        return model
    ...
    try:
        ...
        logger.warning(
            f"Impossible d'optimiser le modèle avec torch.compile() : {str(e)}"
        )
        return model
...
class VectorizedOperations:
    ...
    def batch_matrix_vector_product(
        ...
        try:
            ...
            return torch.stack(
                [torch.mv(matrices[i], vectors[i]) for i in range(matrices.shape[0])]
            )
