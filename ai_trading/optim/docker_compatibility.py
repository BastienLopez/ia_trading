...
def detect_docker() -> bool:
    ...
    try:
        with open('/etc/hostname', 'r') as f:
            if len(f.read().strip()) == 12:  # Les hostname Docker font souvent 12 caractères
                try:
                    with open('/proc/self/cgroup', 'r') as cgroup:
                        if 'docker' in cgroup.read():
                            IS_DOCKER = True
                            logger.info("Environnement Docker détecté (hostname + cgroup)")
                            return True
                except (FileNotFoundError, PermissionError):
                    pass
    except (FileNotFoundError, PermissionError):
...
def detect_environment() -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                with open('/sys/fs/cgroup/memory/memory.limit_in_bytes', 'r') as f:
                    memory_limit = int(f.read().strip()) / (1024 * 1024)  # Convertir en MB
            except (FileNotFoundError, PermissionError):
                pass
            ...
            logger.warning(f"Erreur lors de la récupération des limites Docker: {str(e)}")
    ...
    try:
        ...
        if env_info["cuda_available"]:
            ...
            env_info["cuda_device_names"] = [torch.cuda.get_device_name(i) for i in range(torch.cuda.device_count())]
    except ImportError:
...
def apply_docker_optimizations() -> Dict[str, bool]:
    ...
    if not optimizations["environment_is_docker"]:
        logger.info("L'environnement n'est pas Docker, pas d'optimisations spécifiques appliquées")
        ...
        if hasattr(torch, "compile"):
            ...
            try:
                ...
                if is_compile_available():
                    ...
                    logger.info("torch.compile est disponible et utilisable dans Docker")
            except ImportError:
    ...
    try:
        ...
        if torch.cuda.is_available() and torch.cuda.device_count() > 1:
            try:
                ...
                logger.info(f"Training distribué activé pour {torch.cuda.device_count()} GPUs")
            except ImportError:
...
def setup_for_docker():
    ...
    """
    if not (IS_DOCKER or detect_docker()):
        logger.info("Pas de configuration Docker - l'application ne s'exécute pas dans Docker")
        return False
...
if __name__ == "__main__":
    ...
    print(f"Résultat: {'Configuration appliquée' if result else 'Pas de configuration nécessaire'}") 