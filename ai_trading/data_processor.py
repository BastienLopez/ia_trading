...
class DataProcessor:
    ...
    def download_historical_data(self, exchange_id='binance', symbol='BTC/USDT', timeframe='1h', 
                               start_date=None, end_date=None, save=True):
        """
        ...
        all_candles = []
        while since < until:
            print(f"Téléchargement des données depuis {datetime.fromtimestamp(since/1000)}")
    ...
    def preprocess_for_training(self, df, train_ratio=0.8):
        ...
        print(f"Données divisées: {len(train_data)} pour l'entraînement, {len(test_data)} pour le test")
