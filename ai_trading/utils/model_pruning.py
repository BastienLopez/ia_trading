...
def apply_global_unstructured_pruning(
    ...
    if PRUNE_AVAILABLE:
        try:
            ...
            logger.info(
                f"Élagage global appliqué à {len(parameters_to_prune)} paramètres avec {amount*100:.1f}% de sparsité"
            )
        ...
        if isinstance(module, (nn.Conv1d, nn.Conv2d, nn.Conv3d, nn.Linear)):
            if PRUNE_AVAILABLE:
                try:
                    ...
                    logger.error(
                        f"Erreur lors de l'élagage de la couche {name}: {str(e)}"
                    )
            else:
                return (
                    model  # Si prune n'est pas disponible, retourner le modèle intact
                )

    logger.info(
        f"Élagage par couche appliqué à {pruned_layers} couches avec {amount*100:.1f}% de sparsité"
    )
...
def iterative_pruning(
    ...
    for iteration in range(max_iterations):
            ...
            try:
                ...
                if method == "l1":
                    prune.l1_unstructured(
                        module, name=param_name, amount=effective_prune_rate
                    )
                else:
        ...
        logger.info(
            f"Itération {iteration+1}: {pruned_layers} couches élaguées, sparsité théorique: {global_sparsity:.4f}"
        )
        ...
        logger.info(
            f"Performance après élagage et réentraînement: {new_performance:.4f}"
        )
        ...
        if performance_drop > threshold_accuracy:
            logger.warning(
                f"Arrêt précoce: perte de performance {performance_drop:.4f} > seuil {threshold_accuracy:.4f}"
            )
            break
    ...
    logger.info(
        f"Élagage itératif terminé. Sparsité finale: {sparsity_stats['global_sparsity']:.4f} (théorique: {global_sparsity:.4f})"
    )
    ...
    for layer_name, layer in pruneable_layers:
        ...
        for amount in prune_amounts:
            if not PRUNE_AVAILABLE:
                ...
                layer_results[amount] = baseline_performance * (
                    1.0 - amount / 2.0
                )  # Simulation fictive
                continue
            ...
            logger.info(
                f"  Taux: {amount:.2f}, Performance: {performance:.4f}, Dégradation: {performance_delta:.4f}"
            )
...
class PruningScheduler:
    ...
    def __init__(
        ...
        self.model = model
        self.pruning_method = (
            pruning_method if pruning_method is not None else prune.L1Unstructured
        )
    ...
    def step(self, epoch: int) -> None:
        ...
        if self.end_epoch == self.start_epoch:
            ...
            normalized_epoch = (epoch - self.start_epoch) / (
                self.end_epoch - self.start_epoch
            )
            target_sparsity = self.initial_sparsity + normalized_epoch * (
        ...
        if self.current_sparsity >= 1.0:
            ...
            relative_amount = (target_sparsity - self.current_sparsity) / (
                1.0 - self.current_sparsity
            )
            relative_amount = max(
                0.0, min(relative_amount, 1.0)
            )  # Limiter entre 0 et 1

        # Appliquer l'élagage si nécessaire
        if relative_amount > 0.0:
            logger.info(
                f"Élagage époque {epoch}: {self.current_sparsity:.4f} -> {target_sparsity:.4f} (relative: {relative_amount:.4f})"
            )
