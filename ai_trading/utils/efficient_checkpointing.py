...
def save_model_weights_only(
    model: torch.nn.Module, save_path: Union[str, Path], create_dir: bool = True
) -> Path:
    """
    ...
    logger.info(
        f"Modèle sauvegardé (poids uniquement) à {save_path} ({size_mb:.2f} MB)"
    )
...
class AsyncCheckpointSaver:
    ...
    def save_checkpoint(
        ...
        try:
            ...
            self.save_queue.put(
                (save_fn, save_args, save_kwargs, callback, save_path), block=False
            )
            self.pending_tasks += 1
            logger.info(
                f"Sauvegarde asynchrone enfilée: {save_path} (poids uniquement: {weights_only})"
            )
            return True
        except queue.Full:
            logger.warning(
                "File d'attente de sauvegarde pleine, ignoration de la demande"
            )
            return False
...
class CheckpointManager:
    ...
    def _enforce_max_checkpoints(self):
        ...
        while len(self.checkpoints) > self.max_checkpoints:
            ...
            try:
                ...
                logger.error(
                    f"Erreur lors de la suppression du checkpoint {to_remove}: {e}"
                )
    ...
    def save(
        ...
        if self.async_save and self.async_saver:
            ...
            if self.weights_only:
                ...
                size_mb = os.path.getsize(save_path) / (1024 * 1024)
                logger.info(
                    f"Checkpoint complet sauvegardé à {save_path} ({size_mb:.2f} MB)"
                )
    ...
    def load(
        ...
        if isinstance(checkpoint, dict) and "model_state_dict" in checkpoint:
            ...
            logger.info(
                f"Checkpoint chargé: {checkpoint_path.name}, époque: {metadata.get('epoch', 'inconnu')}"
            )
            return metadata
        else:
...
if __name__ == "__main__":
    ...
    checkpoint_mgr = CheckpointManager(
        base_dir="checkpoints/example", max_checkpoints=3, async_save=True
    )
