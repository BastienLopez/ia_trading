...
def setup_environment():
    ...
    if torch.cuda.is_available():
        ...
        logger.error(
            "Impossible de configurer l'environnement CUDA: CUDA non disponible"
        )
        return False
...
def patch_tests():
    ...
    try:
        ...
        if "force_cuda" in is_cuda_quantization_supported.__code__.co_varnames:
            ...
            logger.warning(
                "La fonction is_cuda_quantization_supported n'a pas le paramètre force_cuda"
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Configuration et test de la quantification CUDA"
    )
    parser.add_argument(
        "--check-only",
        action="store_true",
        help="Vérifier uniquement la disponibilité CUDA sans exécuter les tests",
    )
    parser.add_argument(
        "--run-tests", action="store_true", help="Exécuter les tests de quantification"
    )
    parser.add_argument(
        "--test-module",
        type=str,
        nargs="+",
        help="Modules de test spécifiques à exécuter",
    )
    ...
    if not cuda_available:
        logger.warning(
            "CUDA n'est pas disponible, les tests seront exécutés en mode CPU"
        )
