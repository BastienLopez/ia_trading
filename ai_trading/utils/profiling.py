...
class ProfilingManager:
    ...
    def __init__(self, output_dir: Optional[str] = None):
        ...
        self.output_dir = (
            Path(output_dir)
            if output_dir
            else Path(tempfile.gettempdir()) / "ai_trading_profiles"
        )
    ...
    def _check_nsight_available(self) -> bool:
        ...
        try:
            result = subprocess.run(
                ["nsys", "--version"], stdout=subprocess.PIPE, stderr=subprocess.PIPE
            )
    ...
    def _analyze_torch_profile(self, prof) -> List[Dict[str, Any]]:
        ...
        for item in prof.key_averages():
            if item.cpu_time_total > 0.01 or (
                hasattr(item, "cuda_time_total") and item.cuda_time_total > 0.01
            ):
                result.append(
                    {
                        "name": item.key,
        ...
        return sorted(
            result, key=lambda x: x["cpu_time"] + x["cuda_time"], reverse=True
        )
    ...
    def profile_tensorflow(
        ...
        return {
            ...
            "success": True,
            "note": "Pour visualiser les résultats, exécutez: tensorboard --logdir="
            + str(log_dir),
        }
