...
class DaskParallelProcessor:
    ...
    def __init__(
        ...
        if use_existing_cluster and existing_scheduler_address:
            logger.info(
                f"Connexion à un cluster Dask existant à {existing_scheduler_address}"
            )
    ...
    def _setup_local_cluster(self):
        ...
        try:
            ...
            logger.info(
                f"Cluster Dask démarré avec {len(self.cluster.workers)} workers"
            )
    ...
    def parallelize_dataframe(
        self, df: pd.DataFrame, partition_size: int = None
    ) -> dd.DataFrame:
        """
    ...
    def apply_parallel(
        self, df: pd.DataFrame, func: Callable, column_subset: List[str] = None
    ) -> pd.DataFrame:
        """
    ...
    def process_chunks(
        ...
        logger.info(
            f"Traitement parallèle de {len(chunks)} morceaux de taille {chunk_size}"
        )
