...
def train_ddp_model(
    ...
    if world_size <= 1:
        logger.warning("Un seul GPU détecté ou spécifié. L'entraînement distribué n'est pas nécessaire.")
    ...
    def train_worker(rank, world_size, results_queue=None):
        ...
        # Créer l'optimiseur
        optimizer = optimizer_fn(ddp_model.parameters()) if optimizer_fn else torch.optim.Adam(ddp_model.parameters())
        ...
        for epoch in range(epochs):
            ...
            for batch_idx, batch in enumerate(train_dataloader):
                ...
                if rank == 0 and (batch_idx + 1) % 10 == 0:
                    logger.info(f"Époque {epoch+1}/{epochs} - Batch {batch_idx+1}/{len(train_dataloader)} - Loss: {batch_loss:.4f}")
            ...
            if val_dataloader:
                ...
                with torch.no_grad():
                    for batch in val_dataloader:
                        # Utiliser une fonction d'étape personnalisée si fournie
                        if val_step_fn:
                            batch_loss = val_step_fn(ddp_model, batch, criterion, device)
                        else:
                ...
                # Logs pour le processus principal uniquement
                if rank == 0:
                    logger.info(f"Époque {epoch+1}/{epochs} - Train Loss: {train_loss:.4f}, Val Loss: {val_loss:.4f}")
                ...
                if rank == 0 and checkpoint_dir:
                    ...
                    if not save_best_only or val_loss < best_val_loss:
                        ...
                        checkpoint_file = os.path.join(checkpoint_dir, f"model_epoch_{epoch+1}_valloss_{val_loss:.4f}.pt")
...
class DDPModelWrapper:
    ...
    def __init__(
        ...
        if self.world_size <= 1:
            logger.warning("Un seul GPU détecté ou spécifié. L'entraînement distribué n'est pas utilisé.")
            self.use_ddp = False
        else:
            self.use_ddp = True
            logger.info(f"Configuration pour l'entraînement distribué sur {self.world_size} GPUs")
    ...
    def train(
        ...
        if not self.use_ddp:
            ...
            return self._train_single_gpu(
                train_dataset, val_dataset, batch_size, epochs, 
                num_workers, train_step_fn, val_step_fn, save_best_only
            )
    ...
    def _train_single_gpu(
        ...
        # Créer l'optimiseur
        optimizer = self.optimizer_fn(model.parameters()) if self.optimizer_fn else torch.optim.Adam(model.parameters())
        ...
        # Créer un GradScaler pour la précision mixte si nécessaire
        scaler = torch.cuda.amp.GradScaler() if self.mixed_precision and torch.cuda.is_available() else None
        ...
        for epoch in range(epochs):
            ...
            if val_dataloader:
                ...
                if self.checkpoint_dir:
                    ...
                    if not save_best_only or val_loss < best_val_loss:
                        best_val_loss = val_loss
                        checkpoint_file = os.path.join(self.checkpoint_dir, f"model_epoch_{epoch+1}_valloss_{val_loss:.4f}.pt")
