...
class BlockchainDataCollector:
    ...
    def _make_request(
        ...
        if cache_key in self._cache and time.time() < self._cache_expiry.get(
            cache_key, 0
        ):
    ...
    def get_eth_transactions(
        ...
        try:
                ...
                if "value" in df.columns:
                    if df["value"].dtype == object:
                        df["value"] = df["value"].apply(
                            lambda x: int(x, 16) if x and str(x).startswith("0x") else x
                        )
            ...
            self.logger.error(
                f"Erreur lors de la récupération des transactions Ethereum: {e}"
            )
            return pd.DataFrame()
    ...
    def get_defillama_tvl(self, protocol: str = None) -> pd.DataFrame:
        ...
        try:
            ...
            self.logger.error(
                f"Erreur lors de la récupération des données TVL DeFi: {e}"
            )
            return pd.DataFrame()
    ...
    def get_blockchair_stats(self, blockchain: str = "bitcoin") -> Dict:
        ...
        try:
            ...
            self.logger.error(
                f"Erreur lors de la récupération des statistiques {blockchain}: {e}"
            )
            return {}
    ...
    def get_staking_data(self, blockchain: str = "ethereum") -> pd.DataFrame:
        ...
        try:
            ...
            if blockchain.lower() == "ethereum":
                ...
                eth_data = next(
                    (
                        item
                        for item in data
                        if item.get("name", "").lower() == "ethereum"
                    ),
                    None,
                )
            ...
            self.logger.error(
                f"Erreur lors de la récupération des données de staking: {e}"
            )
            return pd.DataFrame()
    ...
    def get_governance_data(self, protocol: str) -> pd.DataFrame:
        ...
        try:
            ...
            self.logger.info(
                f"Récupération de {len(df)} propositions de gouvernance pour {protocol}"
            )
            return df

        except Exception as e:
            self.logger.error(
                f"Erreur lors de la récupération des données de gouvernance: {e}"
            )
            return pd.DataFrame()

    def get_capital_flows(
        self, n_days: int = 30, blockchain: str = "ethereum"
    ) -> pd.DataFrame:
        """
        ...
        try:
            ...
            blockchain_protocols = protocols_df[
                protocols_df["chains"].apply(
                    lambda x: blockchain.lower() in [c.lower() for c in x]
                )
            ]

            # Calculer les changements de TVL comme proxy pour les flux de capitaux
            blockchain_protocols["tvlPrevDay"] = (
                blockchain_protocols["tvl"] - blockchain_protocols["change_1d"]
            )
            blockchain_protocols["tvlPrevWeek"] = (
                blockchain_protocols["tvl"] - blockchain_protocols["change_7d"]
            )
            ...
            self.logger.info(
                f"Analyse des flux de capitaux pour {len(sorted_flows)} protocoles sur {blockchain}"
            )
            return sorted_flows
    ...
    def get_top_eth_gas_consumers(self) -> pd.DataFrame:
        ...
        try:
            ...
            self.logger.info(
                f"Récupération des données de consommation de gas pour {len(df)} contrats"
            )
            return df

        except Exception as e:
            self.logger.error(
                f"Erreur lors de la récupération des données de consommation de gas: {e}"
            )
            return pd.DataFrame()
        ...
        try:
            ...
            if "timeStamp" in df.columns:
                ...
                if len(transactions_by_date) > 1:
                    metrics["tx_growth_rate"] = (
                        (transactions_by_date.iloc[-1] / transactions_by_date.iloc[0])
                        - 1
                    ) * 100
    ...
    def get_combined_blockchain_data(
        self, blockchain: str = "ethereum", days: int = 30
    ) -> pd.DataFrame:
        """
        ...
        try:
            ...
            combined_data = {
                ...
                "stats": stats,
                "protocols_count": (
                    len(tvl_data) if isinstance(tvl_data, pd.DataFrame) else 0
                ),
                "total_tvl": (
                    tvl_data["tvl"].sum()
                    if isinstance(tvl_data, pd.DataFrame) and "tvl" in tvl_data.columns
                    else 0
                ),
                "staking_ratio": (
                    staking_data["staking_ratio"].iloc[0]
                    if isinstance(staking_data, pd.DataFrame)
                    and "staking_ratio" in staking_data.columns
                    and not staking_data.empty
                    else 0
                ),
                "pools_count": (
                    len(pools_data) if isinstance(pools_data, pd.DataFrame) else 0
                ),
                "capital_inflow_24h": (
                    flows_data["flow_1d"].sum()
                    if isinstance(flows_data, pd.DataFrame)
                    and "flow_1d" in flows_data.columns
                    else 0
                ),
                "capital_inflow_7d": (
                    flows_data["flow_7d"].sum()
                    if isinstance(flows_data, pd.DataFrame)
                    and "flow_7d" in flows_data.columns
                    else 0
                ),
            }
            ...
            self.logger.error(
                f"Erreur lors de la combinaison des données blockchain: {e}"
            )
            return pd.DataFrame()
