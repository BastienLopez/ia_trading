...
class OrderBookCollector:
        ...
        for config in symbols_config:
            ...
            if exchange_id not in self.exchanges:
                try:
                    ...
                    logger.error(
                        f"Erreur lors de l'initialisation de {exchange_id}: {e}"
                    )
    ...
    def fetch_orderbook(self, exchange_id: str, symbol: str, limit: int = 100) -> Dict:
        ...
        try:
            ...
            logger.error(
                f"Erreur lors de la récupération du carnet d'ordres pour {symbol} sur {exchange_id}: {e}"
            )
            return None
    ...
    def collect_orderbook_data(
        ...
        """
        logger.info(
            f"Début de la collecte des données pour {duration_minutes} minutes..."
        )

        end_time = datetime.now() + timedelta(minutes=duration_minutes)
        data_by_symbol = {
            f"{config['exchange']}_{config['symbol']}": []
        ...
        for key, data in data_by_symbol.items():
            if data:
                ...
                logger.info(
                    f"Collecte terminée pour {key}. {len(df)} échantillons collectés."
                )
                result[key] = df
    ...
    def get_vwap_levels(
        self, orderbook: Dict, volume_threshold: float
    ) -> Tuple[float, float]:
        """
    ...
    def estimate_market_impact(
        self, orderbook: Dict, side: str, volume: float
    ) -> Dict[str, float]:
        """
    ...
    def simulate_execution_delay(
        self, volume: float, current_load: float = 0.5
    ) -> float:
        """
        ...
        volume_factor = np.log1p(volume) * 0.2 + (
            volume / 100
        )  # Ajout d'un terme linéaire pour les gros volumes
    ...
    def _calibrate_market_impact(self, data: pd.DataFrame) -> Dict[str, float]:
        ...
        data["real_impact"] = (
            (data["price_after"] - data["price_before"]) / data["price_before"]
        ) * 100
    ...
    def _calibrate_slippage(self, data: pd.DataFrame) -> Dict[str, float]:
        ...
        data["real_slippage"] = (
            abs((data["price_after"] - data["price_before"]) / data["price_before"])
            * 100
        )
