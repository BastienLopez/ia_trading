...
try:
    ...
    HAVE_ONNX = False
    logging.warning("ONNX n'est pas installé. Exécutez 'pip install onnx onnxruntime' pour l'installer.")
...
try:
    ...
    HAVE_TF = False
    logging.warning("TensorFlow n'est pas installé. L'export des modèles TF vers ONNX ne sera pas disponible.")
...
class ONNXExporter:
    ...
    def __init__(
        ...
        """
        if not HAVE_ONNX:
            raise ImportError("ONNX n'est pas installé. Exécutez 'pip install onnx onnxruntime' pour l'installer.")
        ...
        logger.info(f"ONNXExporter initialisé avec le répertoire de sortie: {output_dir}")
        logger.info(f"Utilisation de l'appareil: {self.device}, version d'opset: {opset_version}")
    ...
    def export_pytorch_model(
        ...
        try:
            ...
            model_info = ModelInfo(
                ...
                output_shape=tuple(output_shape) if isinstance(output_shape, torch.Size) else output_shape,
                input_names=input_names,
                output_names=output_names,
                dynamic_axes=dynamic_axes
            )
    ...
    def export_tensorflow_model(
        ...
        """
        if not HAVE_TF:
            raise ImportError("TensorFlow n'est pas installé. L'export des modèles TF vers ONNX n'est pas disponible.")
        
        try:
            import tf2onnx
        except ImportError:
            raise ImportError("tf2onnx n'est pas installé. Exécutez 'pip install tf2onnx' pour l'installer.")
...
def export_tensorflow_policy_model(
    ...
    if not HAVE_ONNX or not HAVE_TF:
        logger.warning("ONNX ou TensorFlow n'est pas installé. L'export n'est pas disponible.")
        return None
