        ...
        with self._lock:
            ...
            if self.state == CircuitState.OPEN:
                if (
                    ...
                    logger.info(
                        "Circuit passant en état semi-ouvert après le temps de récupération"
                    )
        ...
        with self._lock:
            ...
            if self.state == CircuitState.CLOSED:
                ...
                if self.failure_count >= self.failure_threshold:
                    logger.warning(
                        f"Circuit ouvert après {self.failure_count} échecs consécutifs"
                    )
                    self.state = CircuitState.OPEN
                ...
                metrics.update(
                    {
                        "avg_response_time": round(
                            statistics.mean(self.response_times), 2
                        ),
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                if attempt == self.max_retries:
                    ...
                    if self.fallback_function:
                        logger.warning(
                            f"Toutes les tentatives ont échoué pour {func.__name__}, utilisation du fallback"
                        )
                        return self.fallback_function(*args, **kwargs)

                    logger.error(
                        f"Échec définitif après {attempt + 1} tentatives pour {func.__name__}: {e}"
                    )
                    raise
                ...
                logger.warning(
                    f"Tentative {attempt + 1}/{self.max_retries + 1} échouée pour {func.__name__}: {e}. Retry dans {delay:.2f}s"
                )
                time.sleep(delay)
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            return requester.request(
                func, *args, retry_on_exceptions=retry_on_exceptions, **kwargs
            )
