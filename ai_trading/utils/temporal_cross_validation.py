...
class TemporalCrossValidator:
    ...
    def evaluate(
        ...
        for i, (train_data, val_data) in enumerate(self.split()):
            logger.info(
                f"Division {i+1}/{self.n_splits} - Entraînement: {len(train_data)} exemples, "
...
class WalkForwardOptimizer:
    ...
    def _recursive_grid_search(
        ...
        for value in param_values[idx]:
            ...
            self._recursive_grid_search(
                param_keys, param_values, idx + 1, current_combo, result
            )
    ...
    def optimize(
        ...
        best_params = None
        best_metric = (
            float("-inf") if self.evaluation_metric != "error" else float("inf")
        )
        all_results = []
