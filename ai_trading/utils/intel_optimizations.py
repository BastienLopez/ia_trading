...
def configure_torch(config: Dict[str, Any] = None) -> bool:
    ...
    try:
        ...
        if config["use_mkl"] and hasattr(torch, "backends") and hasattr(torch.backends, "mkl"):
        ...
        if config["use_mkldnn"] and hasattr(torch, "backends") and hasattr(torch.backends, "mkldnn"):
...
def configure_numpy(config: Dict[str, Any] = None) -> bool:
    ...
    try:
        ...
        using_mkl = "mkl" in np.__config__.get_info("blas_mkl_info").get("libraries", [])
...
def configure_tensorflow(config: Dict[str, Any] = None) -> bool:
    ...
    try:
        ...
        if config["intra_op_threads"] is not None:
            tf.config.threading.set_intra_op_parallelism_threads(config["intra_op_threads"])
            logger.info(f"TensorFlow intra_op_threads = {config['intra_op_threads']}")
        
        # Configurer le parallélisme inter-op
        if config["inter_op_threads"] is not None:
            tf.config.threading.set_inter_op_parallelism_threads(config["inter_op_threads"])
...
def optimize_for_intel(config: Dict[str, Any] = None, 
                       optimize_torch: bool = True,
                       optimize_numpy: bool = True,
                       optimize_tensorflow: bool = True) -> Dict[str, bool]:
    """
...
def get_optimization_info() -> Dict[str, Any]:
    ...
    info = {
        ...
        "environment": {
            ...
            "TF_ENABLE_ONEDNN_OPTS": os.environ.get("TF_ENABLE_ONEDNN_OPTS", "non défini"),
        }
    }
    ...
    if HAS_TF:
        ...
        try:
            info["tensorflow"]["intra_op_parallelism"] = tf.config.threading.get_intra_op_parallelism_threads()
            info["tensorflow"]["inter_op_parallelism"] = tf.config.threading.get_inter_op_parallelism_threads()
        except Exception:
            pass
...
def print_optimization_info() -> None:
    ...
    if "torch" in info:
        ...
        if "mkl_available" in info["torch"]:
            print(f"  MKL disponible: {'Oui' if info['torch']['mkl_available'] else 'Non'}")
        if "mkl_enabled" in info["torch"]:
            print(f"  MKL activé: {'Oui' if info['torch']['mkl_enabled'] else 'Non'}")
        if "mkldnn_available" in info["torch"]:
            print(f"  MKL-DNN disponible: {'Oui' if info['torch']['mkldnn_available'] else 'Non'}")
        if "mkldnn_enabled" in info["torch"]:
            print(f"  MKL-DNN activé: {'Oui' if info['torch']['mkldnn_enabled'] else 'Non'}")
    ...
    if "tensorflow" in info:
        ...
        if "intra_op_parallelism" in info["tensorflow"]:
            print(f"  Parallélisme intra-op: {info['tensorflow']['intra_op_parallelism']}")
        if "inter_op_parallelism" in info["tensorflow"]:
            print(f"  Parallélisme inter-op: {info['tensorflow']['inter_op_parallelism']}")
