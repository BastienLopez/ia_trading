...
class ModelDistillation:
    ...
    def train(
        ...
        logger.info(f"Taille de l'enseignant: {self._count_parameters(self.teacher_model):,} paramètres")
        logger.info(f"Taille de l'élève: {self._count_parameters(self.student_model):,} paramètres")
        logger.info(f"Ratio de compression: {self._count_parameters(self.teacher_model) / self._count_parameters(self.student_model):.2f}x")
        ...
        for epoch in range(epochs):
            ...
            for batch_idx, (inputs, targets) in enumerate(train_loader):
                ...
                if batch_idx % log_interval == 0:
                    logger.info(f"Époque {epoch+1}/{epochs} [{batch_idx}/{len(train_loader)}] "
            ...
            if val_loader is not None:
                ...
                if "student_accuracy" in val_metrics:
                    ...
                    logger.info(f"Époque {epoch+1} - Train Loss: {avg_train_loss:.4f}, Train Acc: {avg_train_accuracy:.4f}, "
                                f"Val Loss: {val_metrics['val_loss']:.4f}, Val Acc: {val_metrics['student_accuracy']:.4f}, "
                                f"Teacher Acc: {val_metrics['teacher_accuracy']:.4f}")
                else:
                    history["val_accuracy"].append(-val_metrics["student_mse"])  # Négatif car nous voulons maximiser
                    logger.info(f"Époque {epoch+1} - Train Loss: {avg_train_loss:.4f}, Train MSE: {avg_train_accuracy:.4f}, "
                ...
                if early_stopping_patience is not None:
                    if val_metrics["val_loss"] < best_val_loss:
                        ...
                        patience_counter += 1
                        logger.info(f"EarlyStopping: {patience_counter}/{early_stopping_patience}")
                        
                        if patience_counter >= early_stopping_patience:
                            logger.info(f"Early stopping déclenché à l'époque {epoch+1}")
                            break
            else:
                logger.info(f"Époque {epoch+1} - Train Loss: {avg_train_loss:.4f}, Train Acc: {avg_train_accuracy:.4f}")
