...
class TestEnhancedDistributedCache:
    ...
    def test_get_set(self, enhanced_cache, mock_redis):
        ...
        key = "test_key"
        value = {"key": "value"}
        encoded_value = json.dumps(value).encode() if isinstance(json.dumps(value), str) else json.dumps(value)
    ...
    def test_mget_mset(self, enhanced_cache, mock_redis):
        ...
        def mock_mget(keys_list):
            return [json.dumps(values.get(k, None)).encode() if k in values else None for k in keys_list]
