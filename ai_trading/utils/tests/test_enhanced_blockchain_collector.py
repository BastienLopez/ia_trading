...
from ai_trading.utils.blockchain_data_collector import (
    DEFILLAMA_BASE_URL,
    ETHERSCAN_BASE_URL,
)
...
def mock_enhanced_cache():
    ...
    with patch(
        "ai_trading.utils.enhanced_blockchain_collector.EnhancedDistributedCache"
    ) as mock:
        ...
        mock_instance.set.side_effect = lambda key, value, ttl=None: mock_data.update(
            {key: value}
        )
...
def mock_blockchain_collector():
    ...
    collector.get_defillama_pools.return_value = pd.DataFrame(
        {"pool": ["pool1", "pool2"]}
    )
    collector.get_staking_data.return_value = pd.DataFrame(
        {"validator": ["val1", "val2"]}
    )

    # Pour test_process_eth_transactions
    collector._process_eth_transactions.return_value = pd.DataFrame(
        {"tx": ["tx1", "tx2"]}
    )
...
class TestEnhancedBlockchainCollector:
    ...
    async def test_make_request_cache_hit(self, enhanced_collector):
        ...
        with patch.object(
            enhanced_collector, "_make_request", new=AsyncMock()
    ...
    @pytest.mark.asyncio
    async def test_make_request_cache_miss(
        self, enhanced_collector, mock_async_session
    ):
        ...
        try:
            # Appeler la méthode
            result = await enhanced_collector._make_request(
                mock_async_session, url, params
            )
    ...
    async def test_get_ttl_for_source(self, enhanced_collector):
        ...
        assert (
            enhanced_collector._get_ttl_for_source("unknown") == 3600
        )  # Valeur par défaut
    ...
    async def test_identify_data_type(self, enhanced_collector):
        ...
        assert (
            enhanced_collector._identify_data_type("etherscan", {"action": "txlist"})
            == "address"
        )

        assert (
            enhanced_collector._identify_data_type(
                "etherscan", {"action": "eth_getBlockByNumber"}
            )
            == "block"
        )

        assert (
            enhanced_collector._identify_data_type("defillama", {"protocol": "uniswap"})
            == "protocol"
        )

        assert (
            enhanced_collector._identify_data_type("unknown", {"unknown": "params"})
            is None
        )
    ...
    async def test_get_url_for_type(self, enhanced_collector):
        ...
        # Test pour différents types de données
        assert (
            enhanced_collector._get_url_for_type("address", "etherscan")
            == expected_etherscan_url
        )
        assert (
            enhanced_collector._get_url_for_type("transactions", "etherscan")
            == expected_etherscan_url
        )
        assert (
            enhanced_collector._get_url_for_type("protocol", "defillama")
            == expected_defillama_url
        )
    ...
    async def test_prefetch_related_data(self, enhanced_collector):
        ...
        enhanced_collector._get_url_for_type = MagicMock(
            return_value=ETHERSCAN_BASE_URL
        )

        try:
            # Appeler la méthode
            await enhanced_collector._prefetch_related_data(
                session, source, data, params
            )
