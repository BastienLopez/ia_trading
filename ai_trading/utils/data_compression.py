...
class ZstandardParquetCompressor:
    ...
    def __init__(
        ...
        logger.info(f"Initialisation du compresseur Parquet+Zstd (niveau={compression_level})")
    ...
    def _check_dependencies(self):
        ...
        try:
            ...
            logger.error("Pour installer les dépendances: pip install pyarrow zstandard")
    ...
    def train_zstd_dict(self, sample_data: List[pd.DataFrame]) -> bytes:
        ...
        logger.info(f"Entraînement du dictionnaire Zstd avec {len(sample_data)} échantillons")
        ...
        try:
            self.zstd_dict = zstd.train_dictionary(self.zstd_dict_size, samples_bytes).as_bytes()
            logger.info(f"Dictionnaire Zstd entraîné (taille: {len(self.zstd_dict)/1024:.2f} KB)")
            return self.zstd_dict
        except zstd.ZstdError:
            logger.warning("Impossible d'entraîner le dictionnaire Zstd, utilisation de la compression standard")
    ...
    def save_to_parquet(
        ...
        file_size_mb = output_path.stat().st_size / (1024 * 1024) if not partition_cols else sum(
            f.stat().st_size for f in output_path.glob("**/*.parquet")
        ) / (1024 * 1024)
        
        logger.info(f"DataFrame sauvegardé à {output_path}")
        logger.info(f"Taille du fichier: {file_size_mb:.2f} MB (ratio: {df_size_mb/file_size_mb:.2f}x)")
    ...
    def load_from_parquet(
        ...
        file_size_mb = file_path.stat().st_size / (1024 * 1024) if file_path.is_file() else sum(
            f.stat().st_size for f in file_path.glob("**/*.parquet")
        ) / (1024 * 1024)
        df_size_mb = df.memory_usage(deep=True).sum() / (1024 * 1024)
        
        logger.info(f"Chargé {file_path}")
        logger.info(f"Taille du fichier: {file_size_mb:.2f} MB, Taille en mémoire: {df_size_mb:.2f} MB")
...
class DataStreamProcessor:
    ...
    def __init__(
        ...
        logger.info(f"Initialisation du processeur de flux (chunk_size={chunk_size}, use_dask={use_dask})")
        ...
        **kwargs
        ...
        if self.use_dask:
            ...
            try:
                ...
                logger.warning(f"Erreur avec Dask: {e}. Utilisation du traitement séquentiel.")
        ...
        if not self.use_dask:
            ...
            if combine_results and results:
                try:
                    ...
                    logger.warning(f"Erreur lors de la concaténation: {e}. Retour de la liste de résultats.")
                    result = results
            else:
                result = results
