...
def check_python_version():
    ...
    if python_version.major < 3 or (python_version.major == 3 and python_version.minor < 8):
        ...
        logger.info(f"Python {python_version.major}.{python_version.minor} détecté - OK")
        return True
...
def check_system_optimization():
    ...
    if system == "Windows":
        try:
            ...
            if priority > 0:  # Plus bas = priorité plus élevée sur Windows
                optimizations.append(f"Priorité de processus actuelle: {priority} (Normal)")
            else:
                optimizations.append(f"Priorité de processus actuelle: {priority} (Élevée)")
            ...
            if logical_cores and physical_cores and logical_cores > physical_cores:
                ...
                optimizations.append(f"Hyperthreading détecté: {physical_cores} cœurs physiques, {logical_cores} cœurs logiques (ratio {ratio:.1f})")
            else:
                warnings.append("Hyperthreading non détecté ou non disponible. Performances potentiellement réduites pour le multi-threading.")
                
        except ImportError:
            warnings.append("psutil non installé, impossible de vérifier les paramètres système avancés.")
        ...
        try:
            ...
            if soft < 4096:
                warnings.append(f"Limite de fichiers ouverts trop basse: {soft}. Recommandé: 4096+")
            else:
            ...
            try:
                ...
                if soft == 0:
                    warnings.append("Verrouillage mémoire désactivé. Performances potentiellement réduites pour les opérations intensives.")
                else:
                    optimizations.append(f"Verrouillage mémoire activé: {soft} - OK")
            except Exception:
                warnings.append("Impossible de vérifier les paramètres de verrouillage mémoire.")
                
        except ImportError:
            warnings.append("Module resource non disponible, impossible de vérifier les paramètres système avancés.")
    ...
    try:
        ...
        try:
            ...
            if hasattr(np_config, "show") and callable(np_config.show):
                ...
                if "mkl" in str(config_info).lower() or "blas" in str(config_info).lower():
                    optimizations.append("NumPy utilise MKL ou BLAS - OK")
                else:
                    warnings.append("NumPy n'utilise pas MKL/BLAS - performances potentiellement réduites pour les calculs matriciels.")
        except Exception:
            pass
            
    except ImportError:
        warnings.append("NumPy non installé, impossible de vérifier les optimisations NumPy.")
    ...
    try:
        ...
        if pools:
            for pool in pools:
                optimizations.append(f"Pool de threads détecté: {pool['user_api']} avec {pool.get('num_threads', 'inconnu')} threads")
        else:
            warnings.append("Aucun pool de threads OpenMP/MKL/BLAS détecté.")
            
    except ImportError:
        warnings.append("threadpoolctl non installé, impossible de vérifier les configurations de threads.")
...
def main(install_missing=True, check_system=True):
    ...
    logger.info("Configuration des variables d'environnement pour l'optimisation CPU...")
    ...
    if check_system and not warnings:
        ...
        logger.warning(f"⚠ {len(warnings)} avertissements sur la configuration système.")
        
    logger.info("\nPour optimiser davantage les performances CPU:")
    logger.info("1. Utilisez 'get_financial_dataloader()' avec num_workers=-1 pour auto-détection")
    logger.info("2. Activez async_prefetch=True dans FinancialDataset pour le chargement asynchrone")
    logger.info("3. Mesurez les performances avec PerformanceTracker pour identifier les goulots d'étranglement")
    logger.info("4. Utilisez le décorateur @track_time pour profiler les fonctions critiques")
...
if __name__ == "__main__":
    ...
    main(
        ...
        check_system=not args.no_check
    ) 