...
class SystemOptimizer:
    ...
    def _load_config(self, config_file: Optional[str]) -> Dict[str, Any]:
        ...
        default_config = {
            "env_vars": {
                ...
                "OMP_NUM_THREADS": str(max(1, psutil.cpu_count(logical=False) // 2)),  # Threads OpenMP optimisés
                "MKL_NUM_THREADS": str(max(1, psutil.cpu_count(logical=False) // 2)),  # Threads MKL optimisés
                "NUMEXPR_NUM_THREADS": str(max(1, psutil.cpu_count(logical=False) // 2))  # Threads NumExpr optimisés
            },
            "system_limits": {
    ...
    def optimize_environment_variables(self) -> Dict[str, str]:
        ...
        if self.system_info["os"] == "Linux":
            ...
            os.environ["TF_FORCE_GPU_ALLOW_GROWTH"] = "true"  # Allocation dynamique de la mémoire GPU pour TensorFlow
            os.environ["TF_XLA_FLAGS"] = "--tf_xla_cpu_global_jit"  # Optimisation XLA pour TensorFlow
        
        elif self.system_info["os"] == "Windows":
            # Optimisations spécifiques Windows
            os.environ["TF_CPP_MIN_LOG_LEVEL"] = "2"  # Réduire les logs TensorFlow sur Windows
    ...
    def configure_system_limits(self) -> Dict[str, Any]:
        ...
        if self.system_info["os"] != "Windows":
            try:
                ...
                # Limites actuelles
                current_limits["file_limit_soft"] = resource.getrlimit(resource.RLIMIT_NOFILE)[0]
                current_limits["file_limit_hard"] = resource.getrlimit(resource.RLIMIT_NOFILE)[1]
                ...
                if self.system_info["is_admin"]:
                    ...
                    try:
                        ...
                        logger.info(f"Limite de fichiers ouverts configurée à {file_limit}")
                    except (ValueError, resource.error) as e:
                        logger.warning(f"Impossible de définir la limite de fichiers: {e}")
                    ...
                    try:
                        ...
                        logger.warning(f"Impossible de définir la limite de processus: {e}")
                    ...
                    if memory_limit_mb > 0:
                        ...
                        try:
                            ...
                            logger.info(f"Limite de mémoire configurée à {memory_limit_mb} MB")
                        except (ValueError, resource.error) as e:
                            logger.warning(f"Impossible de définir la limite de mémoire: {e}")
                
                # Limites après configuration
                current_limits["file_limit_soft_new"] = resource.getrlimit(resource.RLIMIT_NOFILE)[0]
                current_limits["file_limit_hard_new"] = resource.getrlimit(resource.RLIMIT_NOFILE)[1]
    ...
    def optimize_disk_io(self) -> Dict[str, Any]:
        ...
        if self.config["disk_optimization"]["tmp_in_ram"] and self.system_info["os"] != "Windows":
            temp_dir = None
    ...
    def configure_memory_params(self) -> Dict[str, Any]:
        ...
        if self.system_info["os"] == "Linux" and self.system_info["is_admin"]:
            ...
            logger.info("Configuration des paramètres mémoire non disponible sur ce système")
    ...
    def setup_logging(self) -> logging.Logger:
        ...
        try:
            ...
            logger.info(f"Système de logging configuré avec rotation, niveau: {log_config['level']}")
        except Exception as e:
            logger.warning(f"Impossible de configurer le logger avec rotation de fichiers: {e}")
