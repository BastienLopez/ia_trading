                ...
                if limit_percent is None:
                    ...
                    tf.config.experimental.set_virtual_device_configuration(
                        ...
                        [tf.config.experimental.VirtualDeviceConfiguration(
                            memory_limit=limit * tf.config.experimental.get_device_details(gpu)['memory_limit']
                        )]
                    )
            logger.info(f"Configuration mémoire GPU appliquée: {'adaptative' if limit_percent is None else f'{limit_percent}%'}")
        except Exception as e:
...
def create_tf_gpu_compatible_model(model_fn, *args, **kwargs):
    ...
    logger.info(f"Modèle TensorFlow créé sur {device_type} avec stratégie {type(strategy).__name__}")
...
def is_tf_gpu_available():
    ...
    if TF_GPU_AVAILABLE:
        ...
        return True, f"TensorFlow GPU disponible: {len(GPU_DEVICES)} GPU(s) détecté(s): {', '.join(gpu_names)}"
    else:
...
if __name__ == "__main__":
    ...
    if TF_AVAILABLE:
        ...
        try:
            ...
            print(f"Modèle créé et testé avec succès sur {TFDeviceSelector.get_recommended_device()}")
