...
class PerformanceTracker:
    ...
    def log_summary(self, level: int = logging.INFO):
        ...
        logger.log(
            level,
            f"CPU moyen: {summary['cpu']['avg_percent']:.1f}%, max: {summary['cpu']['max_percent']:.1f}%",
        )
        logger.log(
            level, f"Mémoire max: {max_rss_mb:.1f} MB, delta: {delta_rss_mb:.1f} MB"
        )
        logger.log(
            level,
            f"Temps CPU: utilisateur={summary['cpu']['user_time_total']:.2f}s, système={summary['cpu']['system_time_total']:.2f}s",
        )
...
def log_memory_usage(label=""):
    ...
    logger.info(
        f"{prefix}Utilisation mémoire - RSS: {rss_mb:.1f} MB, VMS: {vms_mb:.1f} MB, {memory_percent:.1f}%"
    )
...
def profile_memory_periodic(interval=5.0, label=None, stop_event=None):
    ...
    def _profile_thread():
        ...
        if measurements:
            ...
            logger.info(
                f"{prefix}Résumé du profilage mémoire - "
...
class MemoryTracker:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        logger.info(
            f"Mémoire pour {self.name}: finale={end_rss_mb:.1f}MB, delta={rss_delta_mb:+.1f}MB"
        )
