...
class AccessPattern:
    ...
    def __init__(self, max_history: int = 1000, time_window: int = 3600):
        ...
        self.sequence_patterns = defaultdict(Counter)  # {clé_précédente: {clé_suivante: count}}
    ...
    def get_frequent_patterns(self, min_count: int = 2) -> Dict[str, List[Tuple[str, int]]]:
        """
        ...
        for prev_key, counters in self.sequence_patterns.items():
            patterns = [(next_key, count) for next_key, count in counters.items() 
                       if count >= min_count]
            if patterns:
    ...
    def get_hot_keys(self, limit: int = 10) -> List[Tuple[str, int]]:
        ...
        recent_accesses = [key for ts, key in self.access_history 
...
class EnhancedDistributedCache:
    ...
    def _prefetch(self, current_key: str) -> None:
        ...
        next_keys = self.access_pattern.predict_next_accesses(current_key, self.max_prefetch)
        ...
        logger.debug(f"Préchargement de {len(keys_to_prefetch)} clés: {keys_to_prefetch}")
...
if __name__ == "__main__":
    ...
    for i in range(50):
        ...
        value = cache.get(key)
        if value is None:
            cache.set(key, {"title": f"Article {key.split(':')[1]}", "content": f"Contenu {i}"})
