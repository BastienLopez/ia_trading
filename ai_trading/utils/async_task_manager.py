...
DEFAULT_BACKEND_URL = os.environ.get("CELERY_RESULT_BACKEND", "redis://localhost:6379/0")
...
class AsyncTaskManager:
    ...
    def __init__(self, 
        ...
        logger.info(f"Gestionnaire de tâches asynchrones initialisé avec broker {broker_url}")
    ...
    def create_base_task(self, name: str = None, bind: bool = True, **options) -> Callable:
        """
    ...
    def execute_task(self, 
                    ...
                    retry: bool = True,
                    retry_policy: Dict = None) -> AsyncResult:
        """
        ...
        args = args or ()
        kwargs = kwargs or {}
        retry_policy = retry_policy or {"max_retries": 3, "interval_start": 0, "interval_step": 0.2}
    ...
    def execute_parallel_tasks(self, 
                              ...
                              queue: str = "default",
                              collect_results: bool = True) -> Union[AsyncResult, List[AsyncResult]]:
        """
        ...
        task_group = group(
            task.s(data, **common_kwargs) for data in data_list
        )
        ...
        if collect_results:
            ...
            header = task_group
            callback = self.app.tasks.get(f"{self.app_name}.tasks.utils.collect_results")
    ...
    def execute_task_chain(self, 
        ...
        for i, task_info in enumerate(tasks):
            ...
            if i == 0:
                task_chain = task.s(*initial_args, *task_args, **initial_kwargs, **task_kwargs)
            else:
                # Tâches suivantes
                if task_args or task_kwargs:
                    ...
                    def adapt_args(previous_result, task_args=task_args, task_kwargs=task_kwargs):
    ...
    def schedule_task(self, 
        ...
        if not crontab_schedule and not periodic_seconds:
            raise ValueError("Vous devez spécifier soit crontab_schedule soit periodic_seconds")
        ...
        if crontab_schedule:
            minute, hour, day_of_month, month_of_year, day_of_week = crontab_schedule.split()
            schedule = crontab(
                minute=minute,
                hour=hour,
