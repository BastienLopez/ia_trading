...
def check_vram_requirements(model: torch.nn.Module, input_size: Tuple[int, ...], 
    ...
    if not torch.cuda.is_available():
        logger.warning("CUDA n'est pas disponible, impossible d'estimer les besoins en VRAM")
        return {"model_size_mb": 0, "forward_pass_mb": 0, "total_required_mb": 0, 
                "available_mb": 0, "is_sufficient": False}
...
def setup_model_offloading(model: torch.nn.Module) -> torch.nn.Module:
    ...
    for name, module in model.named_children():
        ...
        is_critical = any(critical_name in name.lower() for critical_name in critical_layers)
...
def setup_accelerate_offloading(model_class, checkpoint_path: Optional[str] = None, 
                              max_memory: Optional[Dict[str, Union[int, str]]] = None) -> torch.nn.Module:
    """
    ...
    if not is_accelerate_available():
        logger.error("Accelerate n'est pas disponible. Veuillez l'installer: pip install accelerate")
    ...
    if max_memory is None:
        ...
        if torch.cuda.is_available():
            ...
            free_memory = torch.cuda.get_device_properties(0).total_memory - 2 * (1024 ** 3)
            cuda_memory = f"{free_memory // (1024 ** 2)}MB"
        
        max_memory = {
            "cpu": "16GB",
            "cuda": cuda_memory if cuda_memory else "0MB"
        }
    ...
    if checkpoint_path:
        ...
        for module_name, device in device_map.items():
            try:
                ...
                logger.warning(f"Erreur lors du déplacement du module '{module_name}': {e}")
    ...
    def _configure_offloading(self):
        ...
        if self.strategy == "none":
            ...
            if not self.accelerate_available:
                logger.error("Stratégie 'accelerate' sélectionnée mais Accelerate n'est pas installé")
            ...
            temp_path = Path("./temp_weights.pt")
    ...
    def to_eval_mode(self):
        ...
        if self.strategy == "standard":
            for name, module in self.model.named_modules():
                ...
                is_output = any(x in name.lower() for x in ['output', 'head', 'decoder', 'classifier'])
    ...
    def to_train_mode(self):
        ...
        if self.strategy == "standard" and self.cuda_available:
            ...
            for name, module in self.model.named_modules():
                ...
                is_critical = (
                    any(x in name.lower() for x in ['input', 'embed', 'output', 'decoder']) or
                    len(name.split('.')) <= 1  # modules de premier niveau
                )
