...
def is_mixed_precision_supported() -> bool:
    ...
    return any(gpu_type in device_name for gpu_type in [
        "RTX", "V100", "A100", "A10", "A6000", "A40", "A16", "A2", "A30", 
        "T4", "Quadro", "Tesla", "GTX 16", "RTX 30", "RTX 40"
    ])
...
class MixedPrecisionWrapper:
    ...
    def __init__(
        ...
        if self.enabled and not is_mixed_precision_supported():
            logger.warning("La précision mixte est activée mais peut ne pas être optimale sur ce matériel")
        ...
        if self.enabled:
            ...
            logger.info("MixedPrecisionWrapper initialisé avec précision mixte désactivée")
