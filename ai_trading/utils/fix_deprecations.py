...
def fix_distutils_version(file_path):
    ...
    content = re.sub(
        r"from\s+distutils\s+import\s+version", "from packaging import version", content
    )
    content = re.sub(
        r"import\s+distutils\.version", "from packaging import version", content
    )

    # Remplacer les utilisations
    content = re.sub(
        r"distutils\.version\.LooseVersion\((.*?)\)", r"version.parse(\1)", content
    )
...
def fix_jax_make_jaxpr(file_path):
    """
    ...
    content = re.sub(r"xla_computation\((.*?)\)", r"make_jaxpr(jit(\1))", content)
...
def fix_all_files(directory="."):
    ...
    for file_path in python_files:
        ...
        fixed_jax = fix_jax_make_jaxpr(file_path_str)
