...
import logging
from typing import List, Tuple, Dict, Any, Callable, Union, Optional, Type, Sequence
import functools
import warnings
    ...
    for name, module in list(model.named_children()):
        ...
        if isinstance(module, nn.Sequential) and nn.Sequential in module_types:
            ...
            def create_checkpointed_sequential(orig_module, num_segments):
                ...
                orig_module.forward = functools.partial(checkpointed_forward, orig_module)
                return orig_module
            
            # Appliquer le checkpointing
            setattr(model, name, create_checkpointed_sequential(module, num_segments))
            logger.info(f"Appliquer activation checkpointing à {name} avec {num_segments} segments")
        ...
        # Pour les autres types de modules spécifiés
            ...
            logger.info(f"Appliquer activation checkpointing à {name} (module individuel)")
...
class ActivationCheckpointingModifier:
    ...
    def _save_original_forwards(self, model, prefix=''):
        ...
        for name, module in model.named_children():
            ...
            if (isinstance(module, nn.Sequential) and nn.Sequential in self.module_types) or \
               any(isinstance(module, mt) for mt in self.module_types if mt != nn.Sequential):
...
def analyze_memory_usage(model, input_size, batch_size=1, detailed=False):
    ...
    torch.cuda.synchronize()
    mem_after_forward = torch.cuda.memory_allocated() if torch.cuda.is_available() else 0
    ...
    torch.cuda.synchronize()
    mem_after_backward = torch.cuda.memory_allocated() if torch.cuda.is_available() else 0
    ...
    if detailed:
        ...
        def hook_fn(name):
            def hook(module, input, output):
                input_size = sum(i.nelement() * i.element_size() for i in input if isinstance(i, torch.Tensor))
                output_size = output.nelement() * output.element_size() if isinstance(output, torch.Tensor) else \
                              sum(o.nelement() * o.element_size() for o in output if isinstance(o, torch.Tensor))
...
def analyze_checkpointing_savings(model, input_size, batch_size=1, layer_types=None, chunk_size=3):
    """
    ...
    checkpointed_memory = analyze_memory_usage(model_checkpointed, input_size, batch_size)
    
    # Calculer les économies
    memory_savings = orig_memory["total_memory_mb"] - checkpointed_memory["total_memory_mb"]
    savings_percent = (memory_savings / orig_memory["total_memory_mb"]) * 100 if orig_memory["total_memory_mb"] > 0 else 0
...
if __name__ == "__main__":
    ...
    if torch.cuda.is_available():
        ...
        logger.info(f"Mémoire avec checkpointing : {results['checkpointed_memory_mb']:.2f} MB")
        logger.info(f"Économie mémoire : {results['memory_savings_mb']:.2f} MB ({results['savings_percent']:.1f}%)") 