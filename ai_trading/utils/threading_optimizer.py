...
class ThreadingOptimizer:
    ...
    def calculate_optimal_workers(self, force_recalculate: bool = False) -> Dict[str, int]:
        """
        ...
        if self.has_hyperthreading:
            ...
            cpu_based_workers = min(usable_cores, self.physical_cores + self.physical_cores // 2)
        else:
            cpu_based_workers = usable_cores
            
        # Calculer le nombre de workers basé sur la mémoire
        memory_based_workers = max(1, int(self.available_memory_gb / self.memory_per_worker))
        ...
        self._calculated_optimal_workers = {
            ...
            'training': max(1, min(optimal_workers, self.physical_cores)),  # Pour l'entraînement, limiter aux cœurs physiques
            'preprocessing': max(1, optimal_workers - 1),  # Garder un cœur libre pour la boucle principale
            'inference': max(1, min(optimal_workers, self.physical_cores * 2))  # Pour l'inférence, utiliser plus de cœurs
        }
        
        logger.info(f"Nombre optimal de workers calculé: {self._calculated_optimal_workers}")
        logger.info(f"Basé sur {self.cpu_count} cœurs logiques, {self.physical_cores} cœurs physiques, "
    ...
    def configure_thread_limits(self, 
                               ...
                               omp_threads: Optional[int] = None) -> Dict[str, int]:
        """
        ...
        try:
            ...
            if hasattr(np, 'set_num_threads'):
                ...
                logger.info(f"NumPy configuré via variable d'environnement NUMPY_NUM_THREADS={numpy_threads}")
        except ImportError:
        ...
        try:
            ...
            if torch.cuda.is_available():
                logger.info(f"PyTorch configuré pour utiliser {torch_threads} threads CPU + GPU")
            else:
                logger.info(f"PyTorch configuré pour utiliser {torch_threads} threads CPU")
        except ImportError:
        ...
        logger.info(f"Variables d'environnement pour threads configurées: OMP={omp_threads}, MKL/BLAS={numpy_threads}")
    ...
    def get_dataloader_config(self, 
                             ...
                             batch_size: int = 32,
                             persistent_workers: bool = True) -> Dict[str, Union[int, bool]]:
        """
    ...
    def get_multiprocessing_config(self, task_type: str = 'preprocessing') -> Dict[str, Union[str, int]]:
        """
        ...
        logger.info(f"Configuration multiprocessing optimale pour {task_type}: {config}")
        return config
    ...
    def set_process_priority(self, higher_priority: bool = False) -> bool:
        ...
        try:
            ...
            logger.warning("Impossible de modifier la priorité du processus (permission refusée)")
            return False
        except Exception as e:
            logger.error(f"Erreur lors de la modification de la priorité du processus: {e}")
            return False

def optimize_torch_dataloader(dataset_size: int, batch_size: int) -> Dict[str, Union[int, bool]]:
    """
    ...
    return optimizer.get_dataloader_config(data_size=dataset_size, batch_size=batch_size)
...
def optimize_system_for_training():
    ...
    if optimizer.system == 'Windows':
        try:
            ...
            logger.warning("Module win32api non disponible, impossible de modifier la classe de scheduling")
    else:
        try:
...
if __name__ == "__main__":
    ...
    print(f"CPU: {optimizer.cpu_count} cœurs logiques, {optimizer.physical_cores} cœurs physiques")
    print(f"Hyperthreading: {'Détecté' if optimizer.has_hyperthreading else 'Non détecté'}")
    print(f"Mémoire: {optimizer.total_memory_gb:.1f} GB total, {optimizer.available_memory_gb:.1f} GB disponible")
