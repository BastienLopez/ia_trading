...
class EnhancedDataCollector:
    ...
    def get_crypto_prices_coingecko(self, coin_id: str = 'bitcoin', vs_currency: str = 'usd', days: int = 30) -> pd.DataFrame:
        """
        ...
        """
        try:
            logger.info(f"Récupération des prix pour {coin_id} sur {days} jours via CoinGecko")
            ...
            # Fusion des données
            df = prices_df.merge(volumes_df, on='timestamp').merge(market_caps_df, on='timestamp')
    ...
    def get_crypto_prices_coincap(self, coin_id: str = 'bitcoin', interval: str = 'd1', days: int = 30) -> pd.DataFrame:
        """
        ...
        """
        try:
            logger.info(f"Récupération des prix pour {coin_id} sur {days} jours via CoinCap")
    ...
    def get_crypto_prices_cryptocompare(self, coin_symbol: str = 'BTC', vs_currency: str = 'USD', days: int = 30) -> pd.DataFrame:
        """
        ...
        """
        try:
            logger.info(f"Récupération des prix pour {coin_symbol} sur {days} jours via CryptoCompare")
            ...
            logger.error(f"Erreur lors de la récupération des prix via CryptoCompare: {e}")
            return pd.DataFrame()
    ...
    def get_global_crypto_data(self) -> Dict:
        ...
        """
        try:
            logger.info("Récupération des données globales du marché crypto via CoinGecko")
    ...
    def get_merged_price_data(self, coin_id: str, days: int = 30, vs_currency: str = 'usd', include_fear_greed: bool = True, mock_data: bool = False) -> pd.DataFrame:
        """
        ...
        try:
            df_cryptocompare = self.get_crypto_prices_cryptocompare(coin_symbol, days, vs_currency)
        except Exception as e:
            logger.warning(f"Erreur lors de la récupération des données CryptoCompare: {e}")
        ...
        if df_coingecko.empty and df_coincap.empty and df_cryptocompare.empty:
            ...
            if not mock_data:
                ...
                return self.get_merged_price_data(coin_id, days, vs_currency, include_fear_greed, mock_data=True)
            else:
        ...
        logger.info(f"Source primaire: {primary_source} avec {sources[primary_source]} entrées")
            ...
            try:
                ...
                if not df_fear_greed.empty:
                    ...
                    if 'value_classification' in df_fear_greed.columns:
                        df_fear_greed = df_fear_greed.drop(columns=['value_classification'])
