...
try:
    ...
    logger.warning(
        "Ray n'est pas installé. Utilisez 'pip install ray[rllib]' pour l'installer."
    )
...
class RayRLlibOptimizer:
    ...
    def _create_base_config(
        ...
        if framework == "torch":
            config.update(
                {
                    "use_pytorch": True,
                    "torch_optimizer_options": {"weight_decay": 1e-5},
    ...
    def _create_algorithm(self) -> Algorithm:
        ...
        if self.algorithm_name.upper() == "PPO":
            ...
            return ray.rllib.algorithms.algorithm.Algorithm.from_preset(
            ...
            for i in range(num_iterations):
                ...
                if (i + 1) % self.checkpoint_freq == 0:
                    checkpoint = self.agent.save(
                        os.path.join(self.checkpoint_dir, f"checkpoint_{i+1}")
                    )
                    logger.info(f"Checkpoint sauvegardé: {checkpoint}")

            # Sauvegarder le checkpoint final
            final_checkpoint = self.agent.save(
                os.path.join(self.checkpoint_dir, "checkpoint_final")
            )
    ...
    def evaluate(self, num_episodes: int = 10, render: bool = False) -> Dict[str, Any]:
        ...
        for i in range(num_episodes):
            ...
            logger.info(
                f"Épisode {i+1}: Récompense={episode_reward:.2f}, Longueur={episode_steps}"
            )
        ...
        logger.info(
            f"Évaluation terminée. Récompense moyenne: {mean_reward:.2f} ± {std_reward:.2f}"
        )
    ...
    def export_model(
        self, export_dir: str, policy_id: str = "default_policy"
    ) -> Dict[str, Any]:
        """
        ...
        if self.framework == "torch":
            ...
            meta_info = {
                ...
                "input_shape": (
                    model.obs_space.shape if hasattr(model, "obs_space") else None
                ),
                "output_shape": (
                    model.action_space.shape if hasattr(model, "action_space") else None
                ),
                "num_outputs": (
                    model.num_outputs if hasattr(model, "num_outputs") else None
                ),
                "policy_id": policy_id,
            }
