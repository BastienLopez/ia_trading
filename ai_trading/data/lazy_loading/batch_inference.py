...
class BatchInferenceOptimizer:
    ...
    def _prepare_model(
        self, model: nn.Module, optimize_for_inference: bool
    ) -> nn.Module:
        """
        ...
        if optimize_for_inference:
            ...
            try:
                ...
                if sample_shape is not None:
                    ...
                    logger.warning(
                        "Impossible de déterminer la forme d'entrée du modèle pour l'optimisation"
                    )
            except Exception as e:
                logger.warning(
                    f"Erreur lors de l'optimisation du modèle avec TorchScript: {e}"
                )
        ...
        for module in model.modules():
            if isinstance(module, nn.Conv1d):
                return (
                    ...
                    module.in_channels,
                    100,
                )  # Estimation pour séquences temporelles
    ...
    def predict(
        ...
        logger.info(
            f"Inférence complétée: {len(dataset)} échantillons en {elapsed_time:.2f}s ({samples_per_second:.2f} échantillons/s)"
        )
    ...
    def predict_proba(
        ...
        raw_predictions = self.predict(
            data, return_numpy=False, progress_callback=progress_callback
        )
            ...
            for data_item in data_generator:
                ...
                if len(current_batch) >= self.batch_size:
                    ...
                    if batch_transform:
                        ...
                        batch = (
                            torch.stack(current_batch)
                            if isinstance(current_batch[0], torch.Tensor)
                            else torch.tensor(current_batch)
                        )
