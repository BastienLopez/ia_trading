...
class LazyFileReader:
    ...
    def __init__(
        self, file_path: Union[str, Path], chunk_size: int = 10000, cache_size: int = 10
    ):
        """
    ...
    def _detect_file_type(self) -> str:
        ...
        if suffix == ".csv":
            ...
            if not HAVE_PYARROW:
                raise ImportError(
                    "PyArrow est requis pour lire des fichiers Parquet. Installez-le avec: pip install pyarrow"
                )
            return "parquet"
        elif suffix in [".h5", ".hdf5"]:
            if not HAVE_HDF5:
                raise ImportError(
                    "h5py est requis pour lire des fichiers HDF5. Installez-le avec: pip install h5py"
                )
            return "hdf5"
        elif suffix == ".zst":
            if not HAVE_ZSTD:
                raise ImportError(
                    "zstandard est requis pour lire des fichiers compressés zstd. Installez-le avec: pip install zstandard"
                )
            return "zstd"
        else:
    ...
    def _get_file_length(self) -> int:
        ...
        try:
            if self._file_type == "csv":
                ...
                logger.warning(
                    f"Impossible de déterminer la taille pour le type de fichier: {self._file_type}"
                )
                return 1000  # Valeur par défaut
        except Exception as e:
            logger.error(
                f"Erreur lors de la détermination de la taille du fichier: {e}"
            )
            return 1000  # Valeur par défaut
    ...
    def get_chunk(self, chunk_idx: int) -> pd.DataFrame:
        ...
        try:
            if self._file_type == "csv":
                ...
                raise ValueError(
                    f"Type de fichier non pris en charge: {self._file_type}"
                )
    ...
    def get_row(self, row_idx: int) -> pd.Series:
        ...
        if row_idx < 0 or row_idx >= self._file_length:
            raise IndexError(
                f"Index de ligne {row_idx} hors limites [0, {self._file_length})"
            )
        ...
        if local_idx < len(chunk):
            ...
            raise IndexError(
                f"Index local {local_idx} hors limites pour le chunk {chunk_idx}"
            )
    ...
    def get_rows(self, start_idx: int, end_idx: int) -> pd.DataFrame:
        ...
        if start_idx < 0 or start_idx >= self._file_length:
            raise IndexError(
                f"Index de début {start_idx} hors limites [0, {self._file_length})"
            )
        ...
        if self._file_type == "csv":
            ...
            with h5py.File(self.file_path, "r") as f:
                ...
                if isinstance(f[key], h5py.Dataset):
                    ...
                    for sub_key in f[key].keys():
                        if isinstance(f[key][sub_key], h5py.Dataset):
                            if hasattr(f[key][sub_key], "dtype") and hasattr(
                                f[key][sub_key].dtype, "names"
                            ):
                                return list(f[key][sub_key].dtype.names)
                            else:
                                return [
                                    f"col_{i}" for i in range(f[key][sub_key].shape[1])
                                ]
                    return []
        elif self._file_type == "zstd":
            ...
            logger.warning(
                f"Type de fichier non pris en charge pour get_column_names: {self._file_type}"
            )
            return []
...
class LazyDataset(Dataset):
    ...
    def __init__(
        ...
        self.reader = LazyFileReader(
            file_path=file_path, chunk_size=chunk_size, cache_size=cache_size
        )
        ...
        if self.data_length > self.sequence_length + self.predict_n_ahead - 1:
            self.num_examples = (
                self.data_length - self.sequence_length - self.predict_n_ahead + 1
            )
        else:
            self.num_examples = 0
            logger.warning(
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, Optional[torch.Tensor]]:
        ...
        if self.target_idx is not None:
            ...
            if target_idx < self.data_length:
                ...
                target_value = (
                    target_row.iloc[self.target_idx]
                    if hasattr(target_row, "iloc")
                    else target_row[self.target_idx]
                )
