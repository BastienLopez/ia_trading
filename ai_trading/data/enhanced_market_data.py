...
class EnhancedMarketDataFetcher:
    ...
    def __init__(self, use_synthetic: bool = True, api_key: Optional[str] = None):
        ...
        self.order_flow_collector = OrderFlowCollector(use_synthetic=use_synthetic, api_key=api_key)
        self.order_book_collector = OrderBookDepthCollector(use_synthetic=use_synthetic, api_key=api_key)
        self.use_synthetic = use_synthetic
        logger.info(f"EnhancedMarketDataFetcher initialisé (mode {'synthétique' if use_synthetic else 'réel'} pour données alternatives)")
    ...
    def fetch_enhanced_data(self, 
                           ...
                           depth_levels: int = 5) -> pd.DataFrame:
        """
        ...
        if include_order_flow:
            try:
                ...
                order_flow_data = self.order_flow_collector.calculate_flow_metrics(order_flow_data)
                
                logger.info(f"Données de flux d'ordres récupérées: {len(order_flow_data)} points")
            except Exception as e:
                logger.error(f"Erreur lors de la récupération des données de flux d'ordres: {e}")
                order_flow_data = None
        
        # Récupérer les données de profondeur du carnet si demandé
        if include_order_book:
            try:
                logger.info(f"Récupération des données de profondeur du carnet pour {symbol}")
                ...
                # Calculer des métriques supplémentaires
                order_book_data = self.order_book_collector.calculate_depth_metrics(order_book_data)
                
                logger.info(f"Données de profondeur du carnet récupérées: {len(order_book_data)} points")
            except Exception as e:
                logger.error(f"Erreur lors de la récupération des données de profondeur du carnet: {e}")
                order_book_data = None
        ...
        try:
            enhanced_data = integrate_order_flow_and_market_data(
                ...
                order_flow_data=order_flow_data if order_flow_data is not None else pd.DataFrame(index=market_data.index),
                order_book_data=order_book_data
            )
            
            logger.info(f"Données de marché améliorées intégrées avec succès: {enhanced_data.shape}")
    ...
    def get_alternative_data_features(self) -> List[str]:
        ...
        order_flow_features = [
            'buy_volume', 'sell_volume', 'total_volume', 'buy_sell_ratio',
            'order_book_imbalance', 'num_orders', 'num_buy_orders', 'num_sell_orders',
            'avg_order_size', 'cancellation_rate', 'cumulative_buy_pressure',
            'imbalance_ma5', 'imbalance_ma20', 'imbalance_signal', 'buy_sell_momentum',
            'normalized_pressure'
        ]
        
        # Caractéristiques de profondeur du carnet
        order_book_features = [
            'mid_price', 'spread', 'ask_total_volume', 'bid_total_volume',
            'ask_count', 'bid_count', 'book_imbalance', 'bid_ask_ratio',
            'bid_ask_ratio_ma10', 'imbalance_slope', 'liquidity_measure',
            'liquidity_ma20', 'depth_volatility'
        ]
        ...
        for level in range(1, depth_levels + 1):
            order_book_features.extend([
                f'ask_price_{level}', f'bid_price_{level}',
                f'ask_volume_{level}', f'bid_volume_{level}'
            ])
