...
class CompressedStorage:
    ...
    def _get_compressor(self) -> zstd.ZstdCompressor:
        ...
        if self.use_dict and self.compression_dict:
            ...
            return zstd.ZstdCompressor(
                level=self.compression_level, dict_data=dict_data
            )
        else:
    ...
    def compress_file(
        ...
        logger.info(
            f"Original size: {original_size/1e6:.2f} MB, Compressed size: {compressed_size/1e6:.2f} MB"
        )
        logger.info(
            f"Compression ratio: {compression_ratio:.2f}x, Time: {elapsed_time:.2f}s"
        )
    ...
    def decompress_file(
        ...
        if output_path is None:
            if input_path.suffix == self.extension:
                ...
                output_path = input_path.with_suffix(
                    input_path.suffix + ".decompressed"
                )
        else:
            output_path = Path(output_path)
        ...
        logger.info(
            f"Compressed size: {compressed_size/1e6:.2f} MB, Original size: {original_size/1e6:.2f} MB"
        )
        logger.info(
            f"Compression ratio: {compression_ratio:.2f}x, Time: {elapsed_time:.2f}s"
        )
    ...
    def save_dataframe(
        self, df: pd.DataFrame, path: Union[str, Path], format: str = "parquet"
    ) -> Path:
        """
        ...
        logger.info(
            f"Original size: {len(data)/1e6:.2f} MB, Compressed size: {len(compressed_data)/1e6:.2f} MB"
        )
    ...
    def save_json(self, data: Dict[str, Any], path: Union[str, Path]) -> Path:
        ...
        logger.info(
            f"Original size: {len(json_str)/1e6:.2f} MB, Compressed size: {len(compressed_data)/1e6:.2f} MB"
        )
    ...
    def read_compressed_chunks(
        self, path: Union[str, Path], chunk_size: int = 1024 * 1024
    ) -> bytes:
        """
    ...
    def stream_dataframe_chunks(
        self, path: Union[str, Path], format: str = "parquet", chunksize: int = 10000
    ) -> pd.DataFrame:
        """
...
def get_compression_info(file_path: Union[str, Path]) -> Dict[str, Any]:
    ...
    return {
        ...
        "compression_ratio": (
            (decompressed_size / file_size) if decompressed_size else None
        ),
...
def optimize_compression_level(
    data: bytes, test_levels: List[int] = None
    ...
    for level in test_levels:
        ...
        results[level] = {
            ...
            "efficiency": ratio
            / compression_time,  # Ratio de compression par unité de temps
        }
...
def compressed_to_dataframe(
    input_path: Union[str, Path], format: str = "parquet"
) -> pd.DataFrame:
    """
