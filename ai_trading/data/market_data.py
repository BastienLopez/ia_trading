...
class MarketDataFetcher:
    ...
    def __init__(self, api_key: Optional[str] = None, use_cache: bool = True):
        ...
        self.cache_dir = os.path.join(
            os.path.dirname(__file__), "../info_retour/cache/market_data"
        )
        ...
        logger.info(
            f"MarketDataFetcher initialisé (cache {'activé' if use_cache else 'désactivé'})"
        )
    ...
    def fetch_crypto_data(
        ...
        logger.warning(
            f"Source '{source}' non implémentée, utilisation de données synthétiques"
        )
        return self._generate_synthetic_data(symbol, start_date, end_date, interval)
    ...
    def _generate_synthetic_data(
        ...
        base_price = {
            ...
            symbol.upper(), 100.0
        )  # Prix par défaut si symbole non reconnu
        ...
        df["high"] = df["close"] * (
            1 + np.random.uniform(0, daily_volatility, size=len(df))
        )
        df["low"] = df["close"] * (
            1 - np.random.uniform(0, daily_volatility, size=len(df))
        )
        ...
        if self.use_cache:
            cache_file = self._get_cache_filename(
                symbol, start_date, end_date, interval, "synthetic"
            )
            try:
                df.to_csv(cache_file)
                logger.info(
                    f"Données synthétiques enregistrées dans le cache: {cache_file}"
                )
            except Exception as e:
