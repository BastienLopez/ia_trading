...
def generate_synthetic_market_data(
    ...
    # Utilisation de float32 pour éviter les problèmes de précision
    dtype = np.float32
    ...
    # Calcul des prix de clôture avec contrôle des valeurs
    close_prices = np.cumprod(1 + price_changes) * start_price
    ...
    if depreciation_point < n_points:
        ...
        if depreciation_type == "exponential":
            ...
            decay = np.exp(-depreciation_rate * np.arange(n_points - depreciation_point))
            ...
            linear_decay = 1 - (depreciation_rate * np.arange(n_points - depreciation_point))
    ...
    high_prices = close_prices * (1 + np.clip(np.random.uniform(0, volatility, n_points), 0, 0.1))
    low_prices = close_prices * (1 - np.clip(np.random.uniform(0, volatility, n_points), 0, 0.1))
    ...
    if include_volume:
        ...
        if depreciation_point < n_points:
            volume[depreciation_point:] = volume[depreciation_point:] * depreciation_factor[depreciation_point:]
    ...
    df["volume_ratio"] = (df["volume"] / base_volume).astype(dtype) if include_volume else 0
...
def generate_market_with_indicators(n_points=1000, include_technical=True):
    ...
    if include_technical:
        ...
        for window in windows:
            df[f"sma_{window}"] = df["close"].rolling(window=window).mean().astype(dtype)
            df[f"ema_{window}"] = df["close"].ewm(span=window, adjust=False).mean().astype(dtype)
