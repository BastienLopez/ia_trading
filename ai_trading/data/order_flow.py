...
class OrderFlowCollector:
    ...
    def __init__(self, use_synthetic: bool = True, api_key: Optional[str] = None):
        ...
        logger.info(f"OrderFlowCollector initialisé (mode {'synthétique' if use_synthetic else 'réel'})")
    ...
    def collect_order_flow(self, 
        ...
        if self.use_synthetic:
            logger.info(f"Génération de données synthétiques de flux d'ordres pour {symbol}")
            return self._generate_synthetic_order_flow(symbol, start_date, end_date, interval)
        else:
            try:
                logger.info(f"Tentative de collecte de données réelles de flux d'ordres pour {symbol}")
                return self._collect_real_order_flow(symbol, start_date, end_date, interval)
    ...
    def _collect_real_order_flow(self, 
                                symbol: str, 
                                start_date: Union[str, datetime],
                                end_date: Union[str, datetime],
                                interval: str = '1h') -> pd.DataFrame:
    ...
    def _generate_synthetic_order_flow(self, 
                                      symbol: str, 
                                      start_date: Union[str, datetime],
                                      end_date: Union[str, datetime],
                                      interval: str = '1h') -> pd.DataFrame:
        ...
        raw_buy_ratio = 0.5 + 0.15 * np.sin(np.linspace(0, 4*np.pi, n)) + np.random.normal(0, 0.05, n)
        buy_sell_ratio = np.clip(raw_buy_ratio, 0.3, 0.7)
        
        # Volume total des ordres (en unités de la crypto)
        volume_base = liquidity_factor * (1 + 0.3 * np.sin(np.linspace(0, 2*np.pi, n)))
        ...
        cumul_pressure = 50 + np.cumsum(order_book_imbalance - np.mean(order_book_imbalance)) * 5
        ...
        logger.info(f"Données de flux d'ordres synthétiques générées: {len(df)} points pour {symbol}")
        return df
    ...
    def calculate_flow_metrics(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if not all(col in df.columns for col in required_cols):
            logger.warning("Le DataFrame ne contient pas toutes les colonnes requises pour les métriques")
            return df
        ...
        result['buy_sell_momentum'] = df['buy_sell_ratio'].diff(5).rolling(window=10).mean()
        ...
        result['normalized_pressure'] = (buy_volume_ma - sell_volume_ma) / total_volume_ma
...
class OrderBookDepthCollector:
    ...
    def __init__(self, use_synthetic: bool = True, api_key: Optional[str] = None):
        ...
        self.use_synthetic = use_synthetic
        self.api_key = api_key
        logger.info(f"OrderBookDepthCollector initialisé (mode {'synthétique' if use_synthetic else 'réel'})")
    ...
    def collect_order_book_depth(self, 
                                ...
                                end_date: Union[str, datetime],
                                interval: str = '1h',
                                depth_levels: int = 10) -> pd.DataFrame:
        """
        ...
        """
        if self.use_synthetic:
            logger.info(f"Génération de données synthétiques de profondeur du carnet pour {symbol}")
            return self._generate_synthetic_depth(symbol, start_date, end_date, interval, depth_levels)
        else:
            try:
                logger.info(f"Tentative de collecte de données réelles de profondeur pour {symbol}")
                return self._collect_real_depth(symbol, start_date, end_date, interval, depth_levels)
            except Exception as e:
                logger.error(f"Erreur lors de la collecte des données réelles: {e}")
                logger.info("Utilisation de données synthétiques comme fallback")
                return self._generate_synthetic_depth(symbol, start_date, end_date, interval, depth_levels)
    
    def _collect_real_depth(self, 
                           symbol: str, 
                           start_date: Union[str, datetime],
                           end_date: Union[str, datetime],
                           interval: str = '1h',
        ...
        """
        # Placeholder pour l'implémentation d'API réelle
        logger.warning("La collecte de données réelles de profondeur n'est pas encore implémentée")
    ...
    def _generate_synthetic_depth(self, 
        ...
        data['book_imbalance'] = (data['bid_total_volume'] - data['ask_total_volume']) / (data['bid_total_volume'] + data['ask_total_volume'])
        
        # Ajouter des métriques pour chaque niveau de profondeur
        for level in range(1, min(depth_levels + 1, 11)):  # Max 10 niveaux pour simplifier
            ...
            data[f'ask_volume_{level}'] = data['ask_total_volume'] * level_factor * np.random.uniform(0.1, 0.3, n)
            data[f'bid_volume_{level}'] = data['bid_total_volume'] * level_factor * np.random.uniform(0.1, 0.3, n)
        
        # Créer le DataFrame
        df = pd.DataFrame(data, index=date_range)
        
        logger.info(f"Données de profondeur synthétiques générées: {len(df)} points pour {symbol}")
        return df
    ...
    def calculate_depth_metrics(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'book_imbalance' in df.columns:
            result['imbalance_slope'] = df['book_imbalance'].diff().rolling(window=5).mean()
        
        # Calculer l'indicateur de liquidité
        result['liquidity_measure'] = (df['ask_total_volume'] + df['bid_total_volume']) / 2
        ...
        if 'ask_volume_1' in df.columns and 'bid_volume_1' in df.columns:
            ...
            result['depth_volatility'] = top_level_volume.rolling(window=10).std() / top_level_volume.rolling(window=10).mean()
...
def integrate_order_flow_and_market_data(market_data: pd.DataFrame, 
    ...
    if order_book_data is not None and not isinstance(order_book_data.index, pd.DatetimeIndex):
    ...
    if order_book_data is not None:
        ...
        book_columns = [col for col in book_reindexed.columns if col not in result.columns]
