...
class FakeNewsDetector:
    ...
    def analyze_propagation(self, news_id: str, sharing_data: List[Dict]) -> Dict:
        ...
        for share in sharing_data:
            G.add_edge(
                share["from_user"], share["to_user"], timestamp=share["timestamp"]
            )
    ...
    def calculate_credibility_score(
        ...
        bot_ratio = (
            sum(1 for pred in bot_predictions if pred) / len(user_behaviors)
            if user_behaviors
            else 0
        )
        ...
        credibility_score = (
            ...
            + weights["propagation_pattern"]
            * (1 - prop_analysis["suspicious_patterns"])
    ...
    def _contains_obvious_fake_indicators(self, content: str) -> bool:
        ...
        if "URGENT" in content.upper() and (
            "FERMENT" in content.upper() or "DÉFINITIVEMENT" in content.upper()
        ):
            return True
    ...
    def _analyze_citations(self, content: str) -> float:
        ...
        study_patterns = [r"étude", r"rapport", r"analyse", r"recherche"]
