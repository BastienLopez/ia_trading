...
class NewsAnalyzer:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        try:
            ...
            if TRANSFORMERS_AVAILABLE:
                try:
                    ...
                    self.sentiment_pipeline = pipeline(
                        "sentiment-analysis", model=sentiment_model, device=self.device
                    )

                    logger.info(
                        f"Chargement du modèle de reconnaissance d'entités: {entity_model}"
                    )
            ...
            raise RuntimeError(
                "Veuillez installer les dépendances avec 'pip install tf-keras'"
            ) from e
    ...
    def analyze_news_dataframe(
        ...
        if body_col and body_col in df.columns:
            ...
            enriched_df["body_sentiment_label"] = enriched_df["body_sentiment"].apply(
                lambda x: (
                    x["label"] if isinstance(x, dict) and "label" in x else "neutral"
                )
            )
            ...
            enriched_df["global_sentiment_score"] = (
                0.7 * enriched_df["title_sentiment_score"]
                + 0.3 * enriched_df["body_sentiment_score"]
            )
            ...
            enriched_df["global_sentiment_label"] = enriched_df.apply(
                get_global_label, axis=1
            )
        else:
    ...
    def _get_sentiment_distribution(self, df: pd.DataFrame) -> Dict:
        ...
        return (
            df["global_sentiment"].apply(lambda x: x["label"]).value_counts().to_dict()
        )
...
class EnhancedNewsAnalyzer(NewsAnalyzer):
    ...
    def __init__(self, **kwargs):
        ...
        self.cache_dir = kwargs.get("cache_dir", "ai_trading/info_retour/sentiment_cache")
        os.makedirs(self.cache_dir, exist_ok=True)

    def analyze_news_batch(
        self, news_list: List[Dict], batch_size: int = 32
    ) -> pd.DataFrame:
    ...
    def _is_cached(self, news: Dict) -> bool:
        ...
        return os.path.exists(
            os.path.join(self.cache_dir, self._generate_cache_key(news))
        )
