...
class ModelOptimizer:
    ...
    def __init__(
        ...
        if not self.cuda_available and device == "cuda":
            logger.warning(
                "CUDA demandé mais non disponible. Utilisation du CPU à la place."
            )
            self.device = "cpu"
    ...
    def load_model(
        ...
        if quantization != QuantizationType.NONE:
            ...
            try:
                ...
                logger.info(
                    f"Bitsandbytes version {bnb.__version__} trouvé pour la quantification"
                )
            except ImportError:
                logger.warning(
                    "La bibliothèque bitsandbytes n'est pas installée. Désactivation de la quantification."
                )
        ...
        if quantization == QuantizationType.INT8:
            ...
            logger.info(
                "La quantification dynamique INT8 sera appliquée après le chargement"
            )
    ...
    def prune_model(
        ...
        try:
            ...
            logger.error(
                "PyTorch utils.prune non disponible. Impossible d'élaguer le modèle."
            )
            return model
        ...
        if method == "magnitude":
            ...
            for module, param_name in modules_to_prune:
                prune.ln_structured(
                    module, name=param_name, amount=sparsity, n=2, dim=0
                )
        else:
            logger.warning(
                f"Méthode d'élagage inconnue: {method}. Utilisation de 'magnitude'"
            )
            for module, param_name in modules_to_prune:
                prune.l1_unstructured(module, name=param_name, amount=sparsity)
    ...
    def distill_model(
        ...
        """
        logger.info(
            f"Distillation du modèle {teacher_model.config._name_or_path} vers {student_model_name}"
        )
        ...
        try:
            ...
            logger.error(
                "Bibliothèques datasets ou transformers non disponibles. Impossible de distiller le modèle."
            )
        ...
        try:
            ...
            if (
                hasattr(student_config, "hidden_size")
                and student_config.hidden_size > 512
            ):
                reduction_factor = 2
                student_config.hidden_size = (
                    student_config.hidden_size // reduction_factor
                )
                if hasattr(student_config, "intermediate_size"):
                    student_config.intermediate_size = (
                        student_config.intermediate_size // reduction_factor
                    )
                logger.info(
                    f"Taille du modèle élève réduite: {original_hidden_size} → {student_config.hidden_size}"
                )
            ...
            logger.info(
                f"Modèle élève initialisé: {student_model.config.hidden_size} dimensions cachées"
            )
        except Exception as e:
        ...
        data_collator = DataCollatorForLanguageModeling(
            tokenizer=teacher_tokenizer, mlm=False
        )
    ...
    def benchmark_model(
            ...
            for _ in range(num_runs):
                ...
                if batch_size > 1:
                    ...
                    for i in range(0, num_samples, batch_size):
                        ...
                        batch_input = {k: v[i:batch_end] for k, v in all_tokens.items()}
                        model(**batch_input)
                else:
                    # Inférence simple échantillon par échantillon
                    for i in range(len(input_texts)):
                        single_input = {k: v[i : i + 1] for k, v in all_tokens.items()}
                        model(**single_input)
        ...
        memory_used = (
            (memory_after - memory_before) if memory_before is not None else None
        )
        ...
        logger.info(
            f"  Temps moyen par échantillon: {avg_time_per_sample:.4f} secondes"
        )
