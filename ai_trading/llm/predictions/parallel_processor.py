...
class ParallelProcessor:
    ...
    def __init__(self, max_workers: Optional[int] = None, use_processes: bool = True):
        ...
        logger.info(
            f"ParallelProcessor initialisé avec {self.max_workers} workers, mode: {'processus' if use_processes else 'threads'}"
        )
    ...
    def get_task_result(
        self, task_id: int, wait: bool = True, timeout: Optional[float] = None
    ) -> Dict[str, Any]:
        """
                    ...
                    task_data.update(
                        {
                            ...
                            "duration": self.task_results[task_id]["end_time"]
        ...
        with self.task_lock:
            ...
            for task_id, task_info in self.active_tasks.items():
                if task_info["status"] in ["completed", "failed"]:
                    if (
                        max_age is None
                        or (current_time - task_info["start_time"]) > max_age
                    ):
...
class EnsembleParallelProcessor:
    ...
    def __init__(self, max_workers: Optional[int] = None):
        ...
        logger.info(
            f"EnsembleParallelProcessor initialisé avec {self.processor.max_workers} workers"
        )
    ...
    def train_models(
        ...
        **train_params,
        ...
        trained_models = self.processor.map(
            _train_model, models, X=X, y=y, **train_params
        )
    ...
    def predict_parallel(
        self, models: List[Any], data: pd.DataFrame, predict_method: str = "predict"
    ) -> np.ndarray:
        """
        ...
        predictions = self.processor.map(
            _predict_with_model, models, data=data, method=predict_method
        )
        ...
        if len(valid_predictions) != len(models):
            logger.warning(
                f"Certaines prédictions ont échoué: {len(valid_predictions)}/{len(models)} réussies"
            )
    ...
    def evaluate_models(
        ...
        evaluations = self.processor.map(
            _evaluate_model, models, X=X_test, y=y_test, metrics=metrics
        )
    ...
    def ensemble_predict(
        ...
        if weights is None:
            ...
            if len(weights) != len(models):
                raise ValueError(
                    "Le nombre de poids doit correspondre au nombre de modèles"
                )
            weights = np.array(weights) / np.sum(weights)

        # Obtention des prédictions
        predictions = self.predict_parallel(
            models, data, "predict_proba" if aggregation == "mean" else "predict"
        )
    ...
    def batch_process(
        self, func: Callable, data: pd.DataFrame, batch_size: int = 1000, **kwargs
    ) -> List[Any]:
        """
        ...
        logger.info(
            f"Traitement des données en {num_batches} batchs de taille {batch_size}"
        )
        ...
        for i in range(num_batches):
            ...
            if task_result["success"]:
                ...
                logger.error(
                    f"Erreur lors du traitement du batch {i}: {task_result.get('error')}"
                )
                results.append(None)
