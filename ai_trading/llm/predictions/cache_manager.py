...
class CacheManager:
    ...
    def _prefetch_related_keys(self, key: str) -> None:
        ...
        for next_key, count in sorted_transitions[:3]:
            if (
                count >= 2 and next_key not in self.memory_cache
    ...
    def _save_to_disk(self, key: str, value: Any) -> None:
        ...
        try:
            ...
            disk_data = {"value": value, "expiry": self.expiry_times.get(key, 0)}
        ...
        try:
            ...
            for filename in os.listdir(self.persist_path):
                ...
                try:
                    ...
                    logger.error(
                        f"Erreur lors du chargement du fichier cache {filename}: {e}"
                    )

            logger.info(
                f"Cache disque chargé: {loaded_count}/{file_count} entrées chargées en mémoire"
            )
    ...
    def _estimate_memory_usage(self) -> int:
        ...
        try:
            ...
            metadata_size = (
                len(self.access_times) + len(self.expiry_times)
            ) * 16  # ~16 octets par timestamp
            ...
            return (
                len(self.memory_cache) * 1024
            )  # Estimation grossière: ~1KB par entrée
    ...
    def purge_expired(self) -> int:
        ...
        with self.lock:
            expired_keys = [
                key for key in list(self.memory_cache.keys()) if self._is_expired(key)
            ]
