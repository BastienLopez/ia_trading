...
def main():
    ...
    print(f"Intervalle de confiance calculé: [{ci['lower_bound']:.2f}, {ci['upper_bound']:.2f}]")
    
    # Estimer la distribution de probabilité
    dist = calibrator.estimate_probability_distribution(prediction)
    print(f"Distribution estimée avec succès, direction la plus probable: {dist['most_likely_direction']}")
