...
def generate_mock_sentiment_data(days=100):
    ...
    data = {
        ...
        "news_volume": [
            int(100 * (1 + np.random.uniform(-0.5, 0.5))) for _ in range(days)
        ],
        "social_volume": [
            int(500 * (1 + np.random.uniform(-0.5, 0.5))) for _ in range(days)
        ],
    }
...
def add_direction_labels(market_data, window=5):
    ...
    market_data["future_return"] = (
        market_data["close"].shift(-window) / market_data["close"] - 1
    )
...
def test_prediction_model():
    ...
    def mock_preprocess(self, market_data, sentiment_data):
        ...
        if "date" in market_data.columns and "date" in sentiment_data.columns:
            ...
            data = pd.concat(
                [
                    market_data.reset_index(drop=True),
                    sentiment_data.reset_index(drop=True),
                ],
                axis=1,
            )
    ...
    def mock_train(
        ...
        metrics = {"accuracy": (y_pred == y_test).mean(), "test_samples": len(y_test)}
    ...
    def mock_predict(self, asset: str, timeframe: str = "24h", current_data=None):
        """Version simplifiée pour les tests."""
    ...
    try:
        ...
        metrics = model.train(
            market_data, sentiment_data, target_column="direction_code"
        )
        ...
        calibration_result = {
            "best_weights": {"llm_weight": 0.3, "ml_weight": 0.7},
            "best_f1": 0.85,
        }
