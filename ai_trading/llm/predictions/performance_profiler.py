...
class PerformanceProfiler:
    ...
    def __init__(self, output_dir: Optional[str] = None, enable_memory_tracking: bool = True):
        """
        ...
        logger.info(f"PerformanceProfiler initialisé, suivi mémoire: {'activé' if enable_memory_tracking else 'désactivé'}")
    ...
    def profile_function(self, func: Callable, *args, **kwargs) -> Tuple[Any, Dict[str, Any]]:
        ...
        try:
            ...
            data_lines = [line for line in stats_lines if len(line.strip()) > 0 and not line.startswith(('ncalls', 'Ordered by'))]
            ...
            for i, line in enumerate(data_lines[:5]):
                ...
                if len(parts) >= 6:
                    ...
                    top_functions.append({
                        ...
                        "percentage": time_info / execution_time * 100 if execution_time > 0 else 0
                    })
        except Exception as e:
            top_functions = []
        ...
        if self.output_dir:
            ...
            report_path = os.path.join(self.output_dir, f"profile_{func.__name__}_{timestamp}.txt")
            
            with open(report_path, 'w') as f:
                f.write(profile_result)
            
            # Sauvegarde des statistiques en JSON
            stats_path = os.path.join(self.output_dir, f"stats_{func.__name__}_{timestamp}.json")
    ...
    def take_memory_snapshot(self, label: str) -> Dict[str, Any]:
        ...
        if self.output_dir:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            stats_path = os.path.join(self.output_dir, f"memory_{label}_{timestamp}.json")
    ...
    def compare_memory_snapshots(self, label1: str, label2: str) -> Dict[str, Any]:
        ...
        if not all(label in self.memory_snapshots for label in [label1, label2]):
            missing = [label for label in [label1, label2] if label not in self.memory_snapshots]
        ...
        comparison = {
            ...
            "timestamp": datetime.now().isoformat(),
            "memory_diff_bytes": memory_diff,
            "memory_diff_percentage": (memory_diff / snapshot1["current_bytes"]) * 100 if snapshot1["current_bytes"] > 0 else 0,
            "snapshot1": snapshot1,
            "snapshot2": snapshot2
        }
        ...
        if self.output_dir:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            report_path = os.path.join(self.output_dir, f"memory_comparison_{label1}_vs_{label2}_{timestamp}.json")
    ...
    def generate_performance_report(self, function_name: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        if function_name:
            history = [item for item in self.performance_history if item["function_name"] == function_name]
            if not history:
        ...
        for func_name, items in functions.items():
            # Temps d'exécution
            execution_times = [item["execution_time"] for item in items if item["success"]]
            ...
            # Utilisation mémoire
            memory_diffs = [item["memory_diff_bytes"] for item in items if item["success"] and item["memory_diff_bytes"] is not None]
            ...
            if bottlenecks:
                ...
                top_bottlenecks.sort(key=lambda x: x["average_percentage"], reverse=True)
        ...
        if self.output_dir:
            ...
            report_path = os.path.join(self.output_dir, f"performance_report_{fname}_{timestamp}.json")
    ...
    def _generate_performance_charts(self, report: Dict[str, Any], function_name: Optional[str] = None) -> None:
        """
        ...
        try:
            ...
            chart_path = os.path.join(self.output_dir, f"performance_chart_{fname}_{timestamp}.png")
            ...
            for func_name, stats in report["function_details"].items():
                ...
                for bottleneck in bottlenecks:
                    ...
                    if "(" in name:
                        name = name.split("(")[1].split(")")[0]  # Extraction du nom entre parenthèses
                ...
                bars = plt.bar(range(len(bottleneck_names)), bottleneck_percentages, color='salmon')
                plt.xlabel('Fonctions')
                plt.ylabel('Pourcentage du temps d\'exécution (%)')
                plt.title(f'Goulots d\'étranglement pour {func_name}')
                plt.xticks(range(len(bottleneck_names)), bottleneck_names, rotation=45, ha='right')
                ...
                # Sauvegarde du graphique
                bottleneck_chart_path = os.path.join(self.output_dir, f"bottleneck_chart_{func_name}_{timestamp}.png")
                plt.savefig(bottleneck_chart_path)
                plt.close()
                
                logger.info(f"Graphique des goulots d'étranglement pour {func_name} sauvegardé dans {bottleneck_chart_path}")
...
def profile(output_dir: Optional[str] = None, enable_memory_tracking: bool = True):
    ...
    Returns:
        Fonction décorée
    """
    profiler = PerformanceProfiler(output_dir=output_dir, enable_memory_tracking=enable_memory_tracking)
    ...
    def decorator(func):
        ...
        wrapper.get_profile_report = lambda: profiler.generate_performance_report(func.__name__)
