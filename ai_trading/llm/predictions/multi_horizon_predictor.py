...
class MultiHorizonPredictor:
    ...
    def __init__(
        ...
        logger.info(
            f"MultiHorizonPredictor initialisé avec modèle: {llm_model}, mode hybride: {use_hybrid}"
        )
        ...
        for timeframe in timeframes:
            if timeframe not in self.prediction_models:
                model_path = os.path.join(
                    self.model_save_dir, f"prediction_model_{timeframe}.pkl"
                )
        ...
        for timeframe in horizons:
            ...
            if self.use_hybrid and timeframe in self.prediction_models:
                ...
                try:
                    ...
                    logger.error(
                        f"Erreur lors de la prédiction hybride pour {timeframe}: {str(e)}"
                    )
    ...
    def _get_data_for_timeframe(self, asset: str, timeframe: str) -> Dict:
        ...
        from ai_trading.llm.predictions.test_predictions import (
    ...
    def train_models(
        self, asset: str, timeframes: List[str], historical_days: int = 365
    ) -> Dict[str, Dict]:
        """
        ...
        for timeframe in timeframes:
            ...
            try:
                ...
                results[timeframe] = {"metrics": metrics, "model_path": model_path}

                logger.info(
                    f"Modèle pour {timeframe} entraîné avec succès. Metrics: {metrics}"
                )

            except Exception as e:
                logger.error(
                    f"Erreur lors de l'entraînement pour {timeframe}: {str(e)}"
                )
    ...
    def _get_training_data(self, asset: str, timeframe: str, days: int) -> Dict:
        ...
        return {"market_data": market_data, "sentiment_data": sentiment_data}
    ...
    def analyze_consistency(self, predictions: Dict[str, Dict]) -> Dict:
        ...
        results = {
            "short_term": self._analyze_horizon_consistency(
                directions.get("short_term", []), confidences.get("short_term", [])
            ),
            "medium_term": self._analyze_horizon_consistency(
                directions.get("medium_term", []), confidences.get("medium_term", [])
            ),
            "long_term": self._analyze_horizon_consistency(
                directions.get("long_term", []), confidences.get("long_term", [])
            ),
        }
    ...
    def _analyze_horizon_consistency(
        self, directions: List[str], confidences: List[str]
    ) -> Dict:
        """
    ...
    def _generate_trading_signals(self, horizon_analysis: Dict) -> Dict:
        ...
        all_bullish = (
            short_term.get("overall_direction") == "bullish"
            and medium_term.get("overall_direction") == "bullish"
            and long_term.get("overall_direction") == "bullish"
        )

        all_bearish = (
            short_term.get("overall_direction") == "bearish"
            and medium_term.get("overall_direction") == "bearish"
            and long_term.get("overall_direction") == "bearish"
        )

        # Vérifier les divergences
        bullish_divergence = (
            short_term.get("overall_direction") == "bearish"
            and medium_term.get("overall_direction") == "neutral"
            and long_term.get("overall_direction") == "bullish"
        )
