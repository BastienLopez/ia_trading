...
class RTXOptimizer:
    ...
    def __init__(
        ...
        if HAS_CUDA:
            ...
            if device_id >= torch.cuda.device_count():
                logger.warning(
                    f"Device ID {device_id} non valide, utilisation du GPU 0"
                )
                device_id = 0
            ...
            self.has_rtx_gpu = any(
                rtx_model in self.gpu_model for rtx_model in SUPPORTED_GPUS
            )
            ...
            logger.info(
                f"RTXOptimizer initialisé pour {self.gpu_model} (compute capability: {self.compute_capability})"
            )
            if self.has_rtx_gpu:
                logger.info(
                    f"GPU RTX détecté et supporté, optimisations spécifiques activées"
                )
            else:
                logger.info(
                    f"GPU non reconnu comme RTX série 30/40, optimisations génériques appliquées"
                )
        else:
        ...
        if self.enable_tensor_cores:
            if self.compute_capability and float(self.compute_capability) >= 7.0:
                ...
                if HAS_CUDNN:
                    ...
                    logger.info(
                        "cuDNN benchmark activé pour optimiser les performances"
                    )
    ...
    def to_device(self, model: Any) -> Any:
        ...
        try:
            if hasattr(model, "to") and callable(model.to):
                ...
                if self.enable_half_precision and (
                    self.compute_capability and float(self.compute_capability) >= 7.0
                ):
                ...
                if hasattr(torch, "quantization") and hasattr(
                    ...
                    try:
                        ...
                        for name, module in model.named_modules():
                            ...
                            ) and f"{name}.bn" in dict(model.named_modules()):
                                modules_to_fuse.append([name, f"{name}.bn"])

                        if modules_to_fuse:
                            model = torch.quantization.fuse_modules(
                                model, modules_to_fuse
                            )
                    ...
                    try:
                        ...
                        if hasattr(model, "input_shape"):
                            example_input = torch.randn(
                                model.input_shape, device=self.device
                            )
    ...
    def autocast_context(self):
        ...
        if hasattr(torch.cuda, "amp") and hasattr(torch.cuda.amp, "autocast"):
            ...
            if "RTX 40" in self.gpu_model and hasattr(torch, "bfloat16"):
                ...
                logger.info(
                    "Utilisation de FP16/BF16 pour précision mixte sur RTX série 40"
                )
            else:
    ...
    def clear_cache(self) -> None:
        ...
        if self.optimize_memory and self.has_rtx_gpu:
            if not (
                "RTX 3090" in self.gpu_model
                or "RTX 4090" in self.gpu_model
                or "RTX 3080" in self.gpu_model
                or "RTX 4080" in self.gpu_model
            ):
...
def detect_rtx_gpu() -> Optional[Dict[str, Any]]:
    ...
    for i in range(torch.cuda.device_count()):
        ...
        if any(rtx_model in gpu_name for rtx_model in SUPPORTED_GPUS):
            ...
            series = (
                "40 series"
                if any(rtx_model in gpu_name for rtx_model in RTX_40_SERIES)
                else "30 series"
            )
...
def setup_rtx_environment() -> bool:
    ...
    logger.info(
        f"GPU RTX détecté: {rtx_info['name']} (série {rtx_info['series']}, VRAM estimée: {rtx_info['vram_gb']} GB)"
    )
    ...
    try:
        ...
        if HAS_TORCH:
            ...
            if (
                rtx_info["compute_capability"]
                and float(rtx_info["compute_capability"]) >= 8.0
            ):
                if hasattr(torch.backends.cuda, "matmul") and hasattr(
                    torch.backends.cuda.matmul, "allow_tf32"
                ):
                ...
                logger.info(
                    "Optimisations TF32 activées pour GPU RTX Ampere/Ada Lovelace"
                )
