...
class PredictionReporter:
    ...
    def generate_prediction_report(
        ...
        """
        logger.info(
            f"Génération de rapport pour {prediction.get('asset', 'unknown')} au format {format}"
        )
        ...
        for exp_type in explanation_types:
            if exp_type == "shap":
                explanations["shap"] = self.explainer.explain_with_shap(
                    prediction, market_data
                )
            elif exp_type == "lime":
                explanations["lime"] = self.explainer.explain_with_lime(
                    prediction, market_data
                )
            elif exp_type == "llm":
        ...
        if format.lower() == "html":
            report_path = self._generate_html_report(
                prediction, explanations, market_data, output_dir
            )
        ...
        for horizon, prediction in predictions.items():
            ...
            for exp_type in ["shap", "lime", "llm"]:
                if exp_type == "shap":
                    ...
                    explanations["llm"] = self.explainer._explain_llm_prediction(
                        prediction
                    )
    ...
    def _generate_multi_horizon_html_report(
        self,
        predictions: Dict[str, Any],
        ...
        if output_dir:
            ...
            report_dir = os.path.join(
                temp_dir,
                f"multihorizon_report_{asset}_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
            )
        ...
        dashboard_path = self.visualizer.create_prediction_dashboard(
            predictions, market_data, dashboard_path
        )

        consistency_path = os.path.join(report_dir, "consistency_visualization.html")
        consistency_path = self.visualizer.create_consistency_visualization(
        ...
        for horizon, prediction in predictions.items():
            ...
            horizon_report_paths[horizon] = os.path.relpath(
                horizon_report_path, report_dir
            )
    ...
    def _generate_multi_horizon_pdf_report(
        self,
        predictions: Dict[str, Any],
        horizon_explanations: Dict[str, Dict[str, Any]],
        consistency_analysis: Dict[str, Any],
    ...
    def _generate_multi_horizon_json_report(
        self,
        predictions: Dict[str, Any],
        horizon_explanations: Dict[str, Dict[str, Any]],
        consistency_analysis: Dict[str, Any],
