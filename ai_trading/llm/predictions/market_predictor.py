...
if torch.cuda.is_available():
    ...
    if rtx_gpu_info:
        ...
        logger.info(f"Environnement GPU RTX {rtx_gpu_info['series']} configuré pour les prédictions de marché")
    else:
...
class MarketPredictor:
    ...
    def __init__(self, custom_config: Optional[Dict[str, Any]] = None):
        ...
        if use_gpu and torch.cuda.is_available():
            ...
            if rtx_gpu_info:
                ...
                logger.info(f"Accélération GPU RTX activée: {self.rtx_optimizer.get_optimization_info()}")
            else:
    ...
    def _get_cached_social_sentiment(self, text_query: str) -> Dict[str, Any]:
        ...
        self.cache.set(cache_key, result, 1800)  # TTL: 30 minutes (données sociales plus volatiles)
    ...
    def _format_prompt(self, 
        ...
        key_indicators = {}
        indicator_cols = [col for col in data.columns if any(ind in col.lower() for ind in ["rsi", "ma", "ema", "macd", "bb", "volume", "volatility"])]
...
"""
    ...
    def _format_insights_prompt(self, 
                          data: pd.DataFrame, 
                          news_sentiment: Dict[str, Any], 
                          social_sentiment: Dict[str, Any],
                          asset: str) -> str:
        ...
        price_change_24h = (data['close'].iloc[-1] - data['close'].iloc[-24]) / data['close'].iloc[-24] * 100 if len(data) >= 24 else "N/A"
            ...
            if cols:
                for col in cols:
                    if len(data) >= 5:
                        ...
                        change = ((current - prev) / abs(prev)) * 100 if prev != 0 else 0
...
"""
    ...
    def _call_llm_with_retry(self, prompt: str, max_retries: int = 3) -> str:
        ...
        while retries < max_retries:
            try:
                ...
                logger.warning(f"Erreur lors de l'appel au LLM (tentative {retries}/{max_retries}): {e}")
                if retries < max_retries:
                    logger.info(f"Nouvelle tentative dans {wait_time} secondes...")
                    time.sleep(wait_time)
                else:
                    logger.error(f"Échec de l'appel au LLM après {max_retries} tentatives")
                    raise
    ...
    def _call_llm(self, prompt: str) -> str:
        ...
        messages = [
            {"role": "system", "content": "Vous êtes un expert en analyse de marché crypto. Fournissez des prédictions fondées sur les données de marché et de sentiment. Répondez toujours dans le format JSON exact demandé sans explications supplémentaires."},
        ...
        try:
            response = self.client.chat.completions.create(
                ...
                response_format={"type": "json_object"}  # Forcer le format JSON pour faciliter le parsing
            )
            
            # Calcul du temps d'exécution
            execution_time = time.time() - start_time
            logger.info(f"Appel au LLM {self.model_name} terminé en {execution_time:.2f}s")
    ...
    def batch_predict_directions(self, assets: list, timeframe: str = "24h") -> Dict[str, Dict[str, Any]]:
        """
        ...
        """
        logger.info(f"Génération de prédictions par lots pour {len(assets)} actifs sur {timeframe}")
        results = {}
        ...
        for i in range(0, len(assets_to_predict), batch_size):
            ...
            if len(batch) == 1:
                ...
                asset = batch[0]
                results[asset] = self.predict_market_direction(asset, timeframe, batch_data[asset]["data"])
            else:
    ...
    def _batch_call_llm(self, assets: list, batch_data: dict, timeframe: str) -> Dict[str, Dict[str, Any]]:
        """
        ...
        try:
            ...
            for asset in assets:
                if asset in parsed_response:
                    ...
                    prediction = self._parse_prediction(json.dumps(asset_prediction), asset, timeframe)
                    ...
                    logger.warning(f"Pas de prédiction pour {asset} dans la réponse groupée")
            
            return results
            
        except json.JSONDecodeError:
            logger.error("Impossible de parser la réponse JSON du LLM pour la prédiction par lots")
            ...
            for asset in assets:
                try:
                    ...
                    logger.error(f"Erreur lors de la prédiction individuelle pour {asset}: {e}")
            
            return results
    
    def _parse_prediction(self, 
                         response: str, 
                         asset: str, 
