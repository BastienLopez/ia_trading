...
from ai_trading.llm.predictions.parallel_processor import EnsembleParallelProcessor
...
if torch.cuda.is_available():
    ...
    if rtx_gpu_info:
        ...
        setup_rtx_environment()
        logger.info(
            f"Environnement GPU RTX {rtx_gpu_info['series']} configuré pour les modèles de prédiction"
        )
    else:
        logger.info(
            "Aucun GPU RTX détecté, exécution standard pour les modèles de prédiction"
        )
else:
    logger.info(
        "Aucun GPU détecté, exécution en mode CPU pour les modèles de prédiction"
    )
...
class PredictionModel:
    ...
    def __init__(self, custom_config: Optional[Dict[str, Any]] = None):
        ...
        self.inverse_direction_mapping = {
            v: k for k, v in self.direction_mapping.items()
        }
        ...
        if use_gpu and torch.cuda.is_available():
            ...
            if rtx_gpu_info:
                ...
                logger.info(
                    f"Accélération GPU RTX activée pour les modèles: {self.rtx_optimizer.get_optimization_info()}"
                )
            else:
        ...
        logger.info(
            "PredictionModel initialisé avec poids LLM: %s, ML: %s",
            self.llm_weight,
            self.ml_weight,
        )
    ...
    def train(
        ...
        self.ml_model = self.processor.train_models(
            models_config, X_train_scaled, y_train
        )
        ...
        logger.info(
            "Entraînement terminé. Accuracy: %.4f, F1: %.4f",
            metrics["accuracy"],
            metrics["f1"],
        )
    ...
    def predict(self, asset: str, timeframe: str) -> Dict[str, Any]:
        ...
        # 1. Prédiction du LLM
        llm_prediction = self.market_predictor.predict_market_direction(
            asset, timeframe
        )
        ...
        logger.info(
            f"Prédiction combinée pour {asset} ({timeframe}): {combined_prediction['direction']} "
        ...
        if self.torch_models:
            ...
            if self.rtx_optimizer:
                ...
                X_tensor = torch.tensor(
                    X, dtype=torch.float32, device=self.rtx_optimizer.device
                )
            else:
                from contextlib import nullcontext
    ...
    @profile(output_dir=str(config.DATA_DIR / "profiling" / "prediction_model"))
    def batch_predict(
        self, assets: List[str], timeframe: str = "24h"
        ...
        if hasattr(self.market_predictor, "batch_predict_directions"):
            logger.info(
                f"Utilisation de la méthode batch_predict_directions pour {len(assets)} actifs"
            )
    ...
    def preload_predictions(
        ...
        """
        logger.info(
            f"Préchargement des prédictions pour {len(assets)} actifs sur {len(timeframes)} timeframes"
        )
                ...
                for i in range(0, len(assets), batch_size):
                    ...
                    if assets_to_predict:
                        try:
                            ...
                            logger.error(
                                f"Erreur lors du préchargement du lot {i//batch_size+1}: {e}"
                            )

            duration = time.time() - start_time
            logger.info(
                f"Préchargement terminé en {duration:.2f}s. "
    ...
    def schedule_prediction_preloading(
        self,
        assets: List[str],
        timeframes: List[str] = ["24h"],
        ...
        """
        logger.info(
            f"Planification du préchargement toutes les {interval_hours} heures"
        )
        ...
        def scheduler_worker():
            while True:
                try:
                    ...
                    logger.error(
                        f"Erreur dans la tâche planifiée de préchargement: {e}"
                    )
    ...
    def optimize_prediction_pipeline(
        ...
        if len(assets) > 3:
            most_common_assets = assets[
                :3
            ]  # Les 3 premiers sont supposés être les plus communs
        ...
        optimization_stats = {
            ...
            "batch_per_asset_ms": (
                batch_per_asset * 1000 if batch_method_available else None
            ),
        ...
        logger.info(
            f"Optimisation terminée. Méthode: {optimal_method}, "
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        return {"market_predictor_cache": market_predictor_stats}
