...
class ModelEnsemble:
    ...
    def __init__(
        ...
        if fusion_strategy not in valid_strategies:
            raise ValueError(
                f"Stratégie de fusion non valide. Choisissez parmi: {valid_strategies}"
            )
        ...
        logger.info(
            f"Ensemble de modèles initialisé avec stratégie '{fusion_strategy}'"
        )
    ...
    def add_model(
        ...
        if any(m["name"] == name for m in self.models):
            raise ValueError(
                f"Un modèle avec le nom '{name}' existe déjà dans l'ensemble"
            )
    ...
    def _get_model_prediction(
        self, model_info: Dict[str, Any], X: Any
        ...
        if model_info["confidence_method"]:
            try:
                ...
                logger.warning(
                    f"Erreur lors de l'obtention de la confiance pour '{model_info['name']}': {e}"
                )
    ...
    def _fusion_predictions(
        self, predictions: List[Any], confidences: List[float], weights: List[float]
    ) -> Dict[str, Any]:
        """
        ...
        if isinstance(first_pred, (str, int, bool)) or (
            hasattr(first_pred, "__len__") and len(first_pred) == 1
        ):
    ...
    def _fusion_categorical(
        ...
        # Stratégie basée sur la confiance
            ...
            consensus_ratio = (
                confidence_votes[final_prediction] / total_confidence_weight
            )
    ...
    def _fusion_numerical(
        self, predictions: List[float], confidences: List[float], weights: List[float]
    ) -> Dict[str, Any]:
        """
        ...
        # Stratégie pondérée
            ...
            final_prediction = float(
                np.average(predictions_array, weights=weights_array)
            )

            return {
                "prediction": final_prediction,
                "weighted_std_dev": float(
                    np.sqrt(
                        np.average(
                            (predictions_array - final_prediction) ** 2,
        ...
        # Stratégie basée sur la confiance
            ...
            final_prediction = float(
                np.average(predictions_array, weights=combined_weights)
            )

            return {
                "prediction": final_prediction,
                "confidence_weighted_std_dev": float(
                    np.sqrt(
                        np.average(
                            (predictions_array - final_prediction) ** 2,
    ...
    def evaluate(self, X: Any, y_true: Any) -> Dict[str, Dict[str, float]]:
        ...
        for i, model_info in enumerate(self.models):
            ...
            try:
                ...
                if (
                    isinstance(y_true[0], (str, int, bool))
                    or len(np.unique(y_true)) < 10
                ):
                    ...
                    precision = precision_score(
                        y_true, prediction_array, average="weighted", zero_division=0
                    )
                    recall = recall_score(
                        y_true, prediction_array, average="weighted", zero_division=0
                    )
                ...
                # Pour la régression
                    ...
                    metrics[model_name] = {"mse": mse, "mae": mae, "rmse": np.sqrt(mse)}
                ...
                logger.warning(
                    f"Erreur lors de l'évaluation du modèle '{model_name}': {e}"
                )
        ...
        try:
            ...
            # Pour la régression
                ...
                metrics["ensemble"] = {"mse": mse, "mae": mae, "rmse": np.sqrt(mse)}
    ...
    def _adjust_weights(self, metrics: Dict[str, Dict[str, float]]) -> None:
        ...
        if "accuracy" in metrics[self.models[0]["name"]]:
            ...
            for model_name in metrics:
                if model_name != "ensemble" and metric_name in metrics[model_name]:
                    metrics[model_name][metric_name] = 1 / (
                        metrics[model_name][metric_name] + 1e-10
                    )
        else:
        ...
        for i, model_info in enumerate(self.models):
            ...
            if model_name in metrics and metric_name in metrics[model_name]:
                ...
                logger.info(
                    f"Poids du modèle '{model_name}' ajusté de {old_weight:.4f} à {self.models[i]['weight']:.4f}"
                )
