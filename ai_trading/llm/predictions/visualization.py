...
class PredictionVisualizer:
    ...
    def create_prediction_dashboard(
        ...
        pred_df["combined_signal"] = (
            pred_df["direction_value"] * pred_df["confidence_value"]
        )
        ...
        fig = make_subplots(
            ...
            cols=1,
            subplot_titles=[
                "Prédictions par horizon temporel",
        ...
        fig.add_trace(
            go.Scatter(
                ...
                mode="markers",
                marker=dict(
                    size=12, symbol="circle", line=dict(width=2, color="DarkSlateGrey")
                ),
                name="Confiance",
        ...
        if output_path:
            ...
            html_path = os.path.join(
                temp_dir,
                f"prediction_dashboard_{datetime.now().strftime('%Y%m%d_%H%M%S')}.html",
            )
    ...
    def create_explanation_dashboard(
        ...
        if "shap" in explanations:
            ...
            if (
                "feature_names" in shap_explanation
                and "shap_values" in shap_explanation
            ):
        ...
        if "lime" in explanations:
            ...
            if features and scores:
                # Créer un DataFrame
                lime_df = pd.DataFrame({"Feature": features, "Score": scores})
        ...
        if output_path:
            ...
            html_path = os.path.join(
                temp_dir,
                f"explanation_dashboard_{datetime.now().strftime('%Y%m%d_%H%M%S')}.html",
            )
    ...
    def create_consistency_visualization(
        self, consistency_analysis: Dict[str, Any], output_path: Optional[str] = None
    ) -> str:
        """
        ...
        direction_colors = {"bullish": "green", "neutral": "gray", "bearish": "red"}

        colors = [
            direction_colors.get(
                horizon_analysis.get(h, {}).get("overall_direction", "neutral"), "gray"
            )
            for h in horizons
        ]
        ...
        fig.add_trace(
            go.Bar(
                ...
                customdata=[
                    horizon_analysis.get(h, {}).get("overall_direction", "neutral")
                    for h in horizons
                ],
            ),
            row=1,
        ...
        if output_path:
            ...
            html_path = os.path.join(
                temp_dir,
                f"consistency_viz_{datetime.now().strftime('%Y%m%d_%H%M%S')}.html",
            )
    ...
    def create_detailed_report(
        ...
        if output_dir:
            ...
            report_dir = os.path.join(
                temp_dir,
                f"prediction_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
            )
        ...
        if consistency_analysis:
            consistency_path = os.path.join(
                report_dir, "consistency_visualization.html"
            )
            self.create_consistency_visualization(
                consistency_analysis, consistency_path
            )
        ...
        if market_data is not None:
            market_table = market_data.tail(10).to_html(
                classes="market-data-table", border=0
            )
            html_content += f"""
                <div class="card">
