...
class RealTimeAdapter:
    ...
    def __init__(
        ...
        logger.info(
            "RealTimeAdapter initialisé avec fréquence de mise à jour de %.2f secondes",
            update_frequency,
        )
    ...
    def add_data(self, data: Dict[str, Any], data_timestamp: Optional[datetime] = None):
        ...
        self.data_queue.put({"data": data, "timestamp": data_timestamp})
    ...
    def _process_new_data(self):
        ...
        try:
            ...
            while (
                ...
                and data_count < optimal_batch_size
            ):
                try:
                    ...
                    error_count += 1
                    logger.error(
                        f"Erreur lors de la récupération des données: {e} (erreur {error_count}/{max_errors})"
                    )
                    continue
            ...
            timestamps_unix = np.array(
                [item["timestamp"].timestamp() for item in collected_data]
            )
            ...
            filtered_data, outliers_count = self._filter_outliers_optimized(
                sorted_data, sorted_prices
            )
            ...
            if (
                "price" not in latest_data
                and self.current_market_state is not None
                and "price" in self.current_market_state
            ):
                latest_data["price"] = self.current_market_state["price"]

            # Vérification de cohérence avec détection adaptative des sauts
            if (
                self.current_market_state is not None
                and "price" in latest_data
                and "price" in self.current_market_state
            ):
                ...
                price_change_pct = abs(new_price - current_price) / max(
                    abs(current_price), 1e-10
                )

                if price_change_pct > max_change_pct:
                    direction = 1 if new_price > current_price else -1
                    latest_data["price"] = current_price * (
                        1 + direction * max_change_pct
                    )
                    latest_data["price_adjusted"] = True
                    latest_data["original_price"] = (
                        new_price  # Conserver l'original pour analyse
                    )
            ...
            if original_data_size > 0:
                batch_metrics["compression_ratio"] = 1 - (
                    compressed_size / original_data_size
                )
            ...
            ):  # Alerter si le traitement prend plus de 100ms
                logger.warning(
                    f"Traitement des données lent: {total_processing_time:.3f}s pour {data_count} items"
                )
            ...
            logger.error(
                f"Erreur lors du traitement des données: {e} (durée: {processing_time:.3f}s)"
            )
    ...
    def _filter_outliers_optimized(self, data_items, prices):
        ...
        try:
            ...
            filtered_items = [
                item for i, item in enumerate(data_items) if valid_mask[i]
            ]
    ...
    def _update_performance_metrics(self, batch_metrics):
        ...
        self.performance_stats.setdefault("data_processing_time", []).append(
        ...
        if len(self.performance_stats["data_processing_time"]) > max_history:
            self.performance_stats["data_processing_time"] = self.performance_stats[
                "data_processing_time"
            ][-max_history:]

        # Mettre à jour les statistiques agrégées
        self.performance_stats["total_data_processed"] = (
            self.performance_stats.get("total_data_processed", 0)
            + batch_metrics["data_processed"]
        )
        self.performance_stats["total_outliers_removed"] = (
            self.performance_stats.get("total_outliers_removed", 0)
            + batch_metrics["outliers_removed"]
        )

        # Calculer des moyennes glissantes
        self.performance_stats["avg_processing_time"] = np.mean(
            self.performance_stats["data_processing_time"]
        )
        ...
        for metric in ["validation_time", "outlier_detection_time", "compression_time"]:
            ...
            if len(self.performance_stats[metric]) > max_history:
                self.performance_stats[metric] = self.performance_stats[metric][
                    -max_history:
                ]
    ...
    def _validate_data_item(self, data_item: Dict[str, Any]) -> bool:
        ...
        if "data" not in data_item or "timestamp" not in data_item:
            logger.warning(
                "Donnée ignorée: champs obligatoires 'data' ou 'timestamp' manquants"
            )
            return False
        ...
        if not isinstance(data_item["data"]["price"], (int, float)):
            logger.warning(
                f"Donnée ignorée: 'price' n'est pas un nombre ({type(data_item['data']['price']).__name__})"
            )
            return False
    ...
    def _filter_outliers(
        ...
        # Extraire les prix
        prices = [
            item["data"]["price"] for item in data_items if "price" in item["data"]
        ]
        ...
        for item in data_items:
            ...
            if price is not None and lower_bound <= price <= upper_bound:
                ...
                logger.warning(
                    f"Donnée aberrante détectée: prix={price} (limites: {lower_bound:.2f}-{upper_bound:.2f})"
                )
        ...
        try:
            ...
            if isinstance(asset, list):
                ...
                if hasattr(self.prediction_model, "batch_predict_directions"):
                    predictions = self.prediction_model.batch_predict_directions(
                        asset, timeframe
                    )
                else:
                    # Fallback si la méthode batch n'est pas disponible
                    predictions = self._batch_predict_fallback(
                        asset, timeframe, market_state
                    )
    ...
    def _batch_predict_fallback(
        self, assets: List[str], timeframe: str, market_state: Dict[str, Any]
        ...
        with concurrent.futures.ThreadPoolExecutor(
            max_workers=min(10, len(assets))
        ) as executor:
            # Soumettre les tâches de prédiction
            for asset in assets:
                future = executor.submit(
                    self.prediction_model.predict, asset, timeframe
                )
            ...
            for asset, future in futures:
                try:
                    prediction = future.result(
                        timeout=60
                    )  # Timeout de 60 secondes par prédiction
    ...
    def _is_significant_change(self, new_prediction: Dict[str, Any]) -> bool:
        ...
        prev_timestamp = previous_prediction.get(
        ...
        if "direction" in new_prediction and "direction" in previous_prediction:
            ...
            direction_changed = (
                new_prediction["direction"] != previous_prediction["direction"]
            )
            ...
            if "confidence" in new_prediction and "confidence" in previous_prediction:
                confidence_threshold = (
                    0.6  # Ne considérer que les changements avec confiance suffisante
                )
                confidence_sufficient = (
                    new_prediction["confidence"] >= confidence_threshold
                )

                # Si la confiance est faible, exiger une confiance plus élevée que la précédente prédiction
                if (
                    not confidence_sufficient
                    and new_prediction["confidence"]
                    <= previous_prediction["confidence"]
                ):
                    return False
        ...
        # Pour les prédictions numériques (ex: prix cible)
            ...
            if isinstance(new_prediction["prediction"], (int, float)) and isinstance(
                previous_prediction["prediction"], (int, float)
            ):
                ...
                if (
                    "market_state" in new_prediction
                    and "volatility" in new_prediction["market_state"]
                    and new_prediction["market_state"]["volatility"] is not None
                ):
                    ...
                    adaptive_threshold = (
                        self.change_detection_threshold * volatility_factor
                    )
                ...
                if (
                    "market_state" in new_prediction
                    and "market_state" in previous_prediction
                    and "trend" in new_prediction["market_state"]
                    and "trend" in previous_prediction["market_state"]
                ):

                    trend_changed = (
                        new_prediction["market_state"]["trend"]
                ...
                return change_significant or (
                    trend_changed and relative_change > adaptive_threshold / 2
                )
        ...
        # Pour les prédictions d'ensemble avec métadonnées étendues
            ...
            return self._is_significant_ensemble_change(
    ...
    def _is_significant_ensemble_change(
        self, new_prediction: Dict[str, Any], previous_prediction: Dict[str, Any]
    ) -> bool:
        """
        ...
        if (
            "consensus_ratio" in new_prediction
            and "consensus_ratio" in previous_prediction
        ):
        ...
        if "prediction" in new_prediction and "prediction" in previous_prediction:

            # Pour les valeurs catégorielles
            if isinstance(
                new_prediction["prediction"], (str, int, bool)
            ) and isinstance(previous_prediction["prediction"], (str, int, bool)):
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            "avg_prediction_time": (
                np.mean(self.performance_stats["prediction_time"])
                if self.performance_stats["prediction_time"]
                else 0
            ),
            "max_prediction_time": (
                np.max(self.performance_stats["prediction_time"])
                if self.performance_stats["prediction_time"]
                else 0
            ),
            "avg_data_processing_time": (
                np.mean(self.performance_stats["data_processing_time"])
                if self.performance_stats["data_processing_time"]
                else 0
            ),
    ...
    def _update_prediction(self):
        ...
        try:
            ...
            if not self.current_market_state:
                logger.warning(
                    "Pas d'état de marché disponible pour la mise à jour de prédiction"
                )
                return
                    ...
                    self.last_prediction = {
                        ...
                        "consolidated": {
                            ...
                            "average_confidence": sum(
                                p.get("confidence", 0) for p in batch_results.values()
                            )
                            / len(batch_results),
                        },
                        "prediction_time": batch_prediction.get(
...
class RealTimeMarketMonitor:
    ...
    def __init__(
        ...
        logger.info(
            "RealTimeMarketMonitor initialisé avec fenêtre d'observation de %d points",
            observation_window,
        )
    ...
    def _update_indicators(self):
        ...
        try:
            ...
            price_returns = price_returns[
                ~np.isnan(price_returns) & ~np.isinf(price_returns)
            ]
            ...
            if len(price_returns) > 0:
                ...
                logger.warning(
                    "Pas assez de valeurs valides pour calculer la volatilité"
                )
        ...
        try:
            if (
                self.baseline_volatility is None
                and len(self.price_history) >= self.observation_window * 2
            ):
                baseline_start = max(
                    0, len(price_history_list) - self.observation_window * 2
                )
                baseline_end = max(0, len(price_history_list) - self.observation_window)
                baseline_prices = [
                    p[0] for p in price_history_list[baseline_start:baseline_end]
                ]
                ...
                baseline_returns = (
                    np.diff(filtered_baseline_prices) / filtered_baseline_prices[:-1]
                )

                # Filtrer les valeurs NaN ou infinies
                baseline_returns = baseline_returns[
                    ~np.isnan(baseline_returns) & ~np.isinf(baseline_returns)
                ]
        ...
        try:
            ...
            if (
                self.current_volatility is not None
                and self.baseline_volatility is not None
                and self.baseline_volatility > 0
            ):
                volatility_factor = min(
                    2.0, max(0.5, self.current_volatility / self.baseline_volatility)
                )

            # Ajuster les périodes en fonction de la volatilité
            short_span = max(2, int(self.observation_window / (4 * volatility_factor)))
            long_span = max(
                short_span + 1, int(self.observation_window * volatility_factor)
            )
            ...
            if (
                ema_short > ema_long * 1.005
            ):  # Seuil légèrement supérieur pour éviter les faux signaux
            ...
            trend_changed = (
                ...
                and prev_trend != self.current_trend
                and prev_trend != "neutral"
            )  # Ne pas considérer les changements depuis neutral
            ...
            if trend_changed and self.alert_callbacks["trend_change"]:
                ...
                try:
                    ...
                    logger.error(
                        f"Erreur lors de l'appel du callback trend_change: {e}"
                    )
    ...
    def _check_alerts(self):
        ...
        try:
            ...
            if (
                self.baseline_volatility is not None
                and self.baseline_volatility > 0
                and self.current_volatility
                > self.baseline_volatility * self.volatility_threshold
            ):
                ...
                if self.alert_callbacks["volatility_spike"]:
                    ...
                    try:
                        ...
                        logger.error(
                            f"Erreur lors de l'appel du callback volatility_spike: {e}"
                        )
            ...
            if abs(price_change) > adaptive_threshold:
                if self.alert_callbacks["significant_price_move"]:
                    alert_data = {
                        ...
                        "volatility_factor": (
                            volatility_factor
                            if "volatility_factor" in locals()
                            else 1.0
                        ),
                        "timestamp": datetime.now(),
                    }
                    try:
                        ...
                        logger.error(
                            f"Erreur lors de l'appel du callback significant_price_move: {e}"
                        )
            ...
            if len(self.volume_history) >= self.observation_window:
                ...
                recent_volumes = [
                    v[0] for v in volume_history_list[-self.observation_window :]
                ]
                    ...
                    if current_volume > volume_spike_threshold:
                        if self.alert_callbacks["volume_spike"]:
                            ...
                            try:
                                ...
                                logger.error(
                                    f"Erreur lors de l'appel du callback volume_spike: {e}"
                                )
