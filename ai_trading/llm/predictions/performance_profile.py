...
class PerformanceProfiler:
    ...
    def __init__(self, output_dir: str = None, enable_gpu_profiling: bool = True):
        ...
        try:
            ...
            logger.info(
                "NVIDIA Nsight non disponible, utilisant des méthodes de profilage standard"
            )
    ...
    def start_profile(self, name: str) -> None:
        ...
        if self.current_profile:
            logger.warning(
                f"Un profil est déjà en cours: {self.current_profile}. Arrêt du profil précédent."
            )
            self.end_profile()
    ...
    def mark_event(self, name: str, metadata: Dict[str, Any] = None) -> None:
        ...
        if not self.current_profile:
            logger.warning(
                f"Aucun profil en cours. Impossible de marquer l'événement: {name}"
            )
            return

        event = {"name": name, "timestamp": time.time(), "metadata": metadata or {}}
    ...
    def _calculate_stats(self, profile_data: Dict[str, Any]) -> None:
        ...
        if gpu_stats:
            ...
            profile_data["gpu_summary"] = {
                ...
                "avg_memory_allocated": sum(memory_allocated)
                / len(memory_allocated)
                / (1024 * 1024),  # MB
                "peak_memory_reserved": max(memory_reserved) / (1024 * 1024),  # MB
                "avg_utilization": (
                    sum(utilization) / len(utilization) if utilization[0] else 0
                ),
            }
        ...
        for name, profile in profiles_to_analyze.items():
            ...
            for i in range(len(events) - 1):
                if "duration" in events[i]:
                    slow_events.append(
                        {"name": events[i]["name"], "duration": events[i]["duration"]}
                    )
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                if "gpu_summary" in profile_data:
                    ...
                    logger.info(
                        f"Performance {func.__name__}: {profile_data['duration']:.4f}s, "
...
class PerformanceOptimizer:
    ...
    def get_optimal_batch_size(self, model, sample_input, min_batch=1, max_batch=32):
        ...
        if not self.use_gpu:
            logger.info(
                "GPU non disponible, utilisation de la taille de lot par défaut"
            )
            return self.batch_size

        logger.info(
            f"Recherche de la taille de lot optimale entre {min_batch} et {max_batch}..."
        )
        ...
        for batch_size in range(min_batch, max_batch + 1, 2):
            ...
            if isinstance(sample_input, torch.Tensor):
                batch_input = sample_input.repeat(
                    batch_size, *([1] * len(sample_input.shape[1:]))
                )
            else:
            ...
            logger.info(
                f"Batch size {batch_size}: {time_per_sample:.6f}s par échantillon, throughput: {throughput:.2f} échantillons/s"
            )
        ...
        logger.info(
            f"Taille de lot optimale: {optimal_batch['batch_size']} "
    ...
    def optimize_model(self, model):
        ...
        if (
            hasattr(torch, "nn")
            and hasattr(torch.nn, "utils")
            and hasattr(torch.nn.utils, "fusion")
        ):
            try:
        ...
        try:
            ...
            sample_input = torch.rand(
                1, 3, 224, 224
            ).cuda()  # Exemple d'entrée, à adapter
