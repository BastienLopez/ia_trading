...
class DataLoader:
    ...
    def __init__(self, data_path: str = None):
        ...
        self.data_path = data_path or os.path.join(
            "ai_trading", "info_retour", "data", "dashboard"
        )
    ...
    def get_market_data_for_visualization(
        self, portfolio_id: str = "main", max_points: int = 5000
    ) -> pd.DataFrame:
        """
        ...
        try:
            ...
            if data is None or data.empty:
                ...
                data = self._generate_synthetic_market_data()
                print(
                    "Utilisation de données de marché synthétiques pour la visualisation"
                )
            ...
            if result is None or result.empty:
                print(
                    "Échec de l'échantillonnage des données, utilisation des données complètes"
                )
                return data
            ...
            print(
                f"Erreur lors de la récupération des données pour visualisation: {str(e)}"
            )
    ...
    def _generate_synthetic_trade_data(self) -> pd.DataFrame:
        ...
        exit_times = [
            entry_time + timedelta(days=int(period))
        ...
        df = pd.DataFrame(
            {
                ...
                "profit": profits,
                "profit_pct": [
                    (exit_p / entry_p) - 1 if t == "achat" else (entry_p / exit_p) - 1
