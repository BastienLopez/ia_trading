...
class Visualizer3D:
    ...
    def create_multi_indicator_surface(self, df, x_col, y_col, z_col, colorscale='Viridis'):
        """
        ...
        if len(df_clean) < 10:
            raise ValueError(f"Pas assez de données valides après élimination des valeurs manquantes (restant: {len(df_clean)})")
        ...
        if len(points) > 3:
            try:
                ...
                grid_z = interpolate.griddata(points, values, (X, Y), method=method, fill_value=np.nan)
                
                # Combler les trous restants avec une méthode plus simple
                if np.isnan(grid_z).any():
                    mask = np.isnan(grid_z)
                    grid_z_nearest = interpolate.griddata(points, values, (X, Y), method='nearest')
    ...
    def create_portfolio_trajectory(self, df, window=30, n_components=3):
        ...
        if len(feature_cols) < n_components:
            raise ValueError(f"Pas assez de colonnes numériques ({len(feature_cols)}) pour {n_components} composantes")
        ...
        fig.add_trace(go.Scatter3d(
            ...
            marker=dict(
                size=5,
                color=markers_indices,
                colorscale='Viridis',
                opacity=0.8
            ),
            text=[df.index[i].strftime('%Y-%m-%d') if hasattr(df.index[i], 'strftime') else str(df.index[i]) 
    ...
    def create_trade_clusters_3d(self, trades_df, features=None, n_clusters=5):
        ...
        if len(X) < n_clusters:
            ...
            if len(X) < 3:
                raise ValueError(f"Pas assez de données valides après élimination des valeurs manquantes (restant: {len(X)})")
