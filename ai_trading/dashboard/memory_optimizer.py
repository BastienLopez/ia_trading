...
def downscale_for_visualization(df: pd.DataFrame, max_points: int = 5000) -> pd.DataFrame:
    """
    ...
    try:
        ...
        if result.empty:
            print(f"WARNING: Échantillonnage a résulté en DataFrame vide (step={step}, len original={len(df)})")
