...
def generate_large_df(rows=100000, cols=20):
    ...
    for i in range(int(cols * 0.2)):
        ...
        data[f"cat_col_{i}"] = np.random.choice(
            [f"category_{j}" for j in range(n_categories)], size=rows
        )
...
def test_optimize_dataframe():
    ...
    for col in df.columns:
        if pd.api.types.is_numeric_dtype(df[col]) and pd.api.types.is_numeric_dtype(
            ...
            if not np.allclose(
                df[col].fillna(0), optimized_df[col].fillna(0), rtol=1e-4, atol=1e-4
            ):
                print(
                    f"ATTENTION: Potentielle perte de précision dans la colonne {col}"
                )
        else:
...
def test_data_frame_pool():
    ...
    for i in range(0, 10, 2):
        ...
        if retrieved_df is not None:
            print(
                f"Récupéré {key}: {len(retrieved_df):,} lignes en {retrieval_time:.3f} secondes"
            )
        else:
        ...
        return pd.DataFrame(
            {
                ...
                "timestamp": pd.date_range(
                    start="2022-01-01", periods=size, freq="1min"
                ),
            }
        )
...
def print_memory_comparison(before_detail, after_detail):
    ...
    print("-" * 80)
    print(
        f"{'Colonne':<20} {'Avant (MB)':<15} {'Après (MB)':<15} {'Réduction (%)':<15}"
    )
    print("-" * 80)
...
def print_downscaling_results(results):
    ...
    print("-" * 80)
    print(
        f"{'Max Points':<15} {'Points Réduits':<15} {'Mémoire (MB)':<15} {'Réduction (%)':<15} {'Temps (s)':<15}"
    )
    print("-" * 80)

    for result in results:
        print(
            f"{result['max_points']:<15,} {result['actual_points']:<15,} {result['memory_mb']:<15.2f} {result['reduction_pct']:<15.1f} {result['time_sec']:<15.3f}"
        )
...
def run_all_tests():
    ...
    print("\n=== RÉSUMÉ DES TESTS ===")
    print(
        f"Optimisation DataFrame: {before_mem:.2f} MB → {after_mem:.2f} MB ({100*(1-after_mem/before_mem):.1f}% réduction)"
    )
    print(f"Temps d'optimisation: {optimization_time:.2f} secondes")
    print(
        f"Downscaling: Réduction mémoire moyenne: {np.mean([r['reduction_pct'] for r in downscaling_results]):.1f}%"
    )
