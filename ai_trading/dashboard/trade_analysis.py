...
class TradeAnalyzer:
    ...
    def prepare_trade_data(self, trades_df):
        ...
        required_cols = ['entry_time', 'exit_time', 'symbol', 'direction', 'entry_price', 'exit_price',
                        'quantity', 'profit', 'status']
        
        # Créer des colonnes dérivées
        if 'entry_time' in df.columns and 'exit_time' in df.columns:
            # Calculer la durée de détention
            df['holding_period'] = (pd.to_datetime(df['exit_time']) - 
                                   pd.to_datetime(df['entry_time'])).dt.total_seconds() / 3600  # en heures
        ...
        if 'profit' in df.columns:
            ...
            df['result_category'] = pd.cut(
                ...
                labels=['Perte majeure', 'Perte modérée', 'Perte mineure', 
                       'Gain mineur', 'Gain modéré', 'Gain majeur']
            )
    ...
    def create_win_loss_analysis(self, trades_df):
        ...
        fig.add_trace(
            go.Heatmap(
                ...
                showscale=False,
                text=[[f"Trade {i*cols_count+j+1}" for j in range(cols_count)] for i in range(rows_count)],
                hoverinfo='text+z'
            ),
            row=2, col=1
        )
        ...
        # 4. Courbe d'équité
        df_sorted = df.sort_values('entry_time' if 'entry_time' in df.columns else df.index)
        ...
        # Mise à jour du layout
        fig.update_layout(
            title='Analyse des gains et pertes',
            height=800,
            width=1000
        )
    ...
    def create_trade_attribution(self, trades_df, market_data=None):
        ...
        if 'direction' in df.columns and 'result_category' in df.columns:
            ...
            direction_result = df.groupby(['direction', 'result_category']).size().unstack(fill_value=0)
        ...
        if 'entry_volatility' in df.columns or (market_data is not None and 'volatility' in market_data.columns):
            ...
            if 'volatility_col' in locals():
                ...
                if len(volatility_data) > 2:
                    slope, intercept, r_value, p_value, std_err = stats.linregress(
                        volatility_data[volatility_col], 
                        volatility_data['profit_pct']
                    )
