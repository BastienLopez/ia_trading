...
def pytest_configure(config):
    ...
    warnings.filterwarnings("ignore", message="twisted.internet.defer.returnValue was deprecated")
    ...
    warnings.filterwarnings("ignore", message="The `keras.initializers.serialize()` API should only be used")
...
def pytest_addoption(parser):
    ...
    parser.addoption(
        "--run-slow", action="store_true", default=False, help="Exécuter les tests lents"
    )
    parser.addoption(
...
def pytest_collection_modifyitems(config, items):
    ...
    run_slow = config.getoption("--run-slow") or os.environ.get("RUN_SLOW_TESTS", "0") == "1"
    run_gpu = config.getoption("--run-gpu") or os.environ.get("RUN_GPU_TESTS", "0") == "1"
    
    skip_slow = pytest.mark.skip(reason="Test trop long - utilisez --run-slow pour l'exécuter")
