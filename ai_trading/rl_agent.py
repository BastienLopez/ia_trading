...
class TradingCallback(BaseCallback):
    ...
    def _on_step(self):
        ...
        if hasattr(self.model.env.envs[0].unwrapped, "portfolio_value"):
            self.portfolio_values.append(self.model.env.envs[0].unwrapped.portfolio_value)
        return True
...
class CryptoTradingEnv(gym.Env):
    ...
    def __init__(self, df, initial_balance=10000, transaction_fee=0.001):
        ...
        self.observation_space = spaces.Box(
            ...
            high=np.array([100, np.inf, np.inf, np.inf, np.inf, np.inf, np.inf, np.inf, np.inf, np.inf, np.inf, np.inf, 1, 1]),
            dtype=np.float32
        )
    ...
    def _take_action(self, action):
        ...
        if action == 1:  # Acheter
            ...
            if self.current_position == 1:  # Si on a une position
                ...
                sale_amount = self.crypto_held * current_price * (1 - self.transaction_fee)
...
class RLAgent:
    ...
    def predict(self, observation):
        ...
        if self.model is None:
            raise ValueError("Le modèle n'est pas chargé. Utilisez load() ou train() d'abord.")
    ...
    def backtest(self, df):
        ...
        initial_value = df.iloc[0]['close'] * (env.initial_balance / df.iloc[0]['close'])
        final_portfolio = env.portfolio_value
        bh_final_value = df.iloc[-1]['close'] * (env.initial_balance / df.iloc[0]['close'])
        
        profit_pct = (final_portfolio - env.initial_balance) / env.initial_balance * 100
        bh_profit_pct = (bh_final_value - env.initial_balance) / env.initial_balance * 100
