...
class TradingEnvironment:
    ...
    def _get_state(self):
        ...
        df_window = self.data.iloc[self.current_step - window + 1:self.current_step + 1]
        ...
        if not df_window.empty:
            ...
            normalized_closes = (close_prices - np.mean(close_prices)) / (np.std(close_prices) + 1e-8)
            ...
            if 'macd' in df_window.columns and 'signal_line' in df_window.columns:
                ...
                state.append((macd - signal) / (abs(macd) + abs(signal) + 1e-8))  # Normalisation
    ...
    def step(self, action):
        ...
        if action_type == "buy":
            ...
            adjusted_action = self.risk_manager.check_exposure(self.portfolio, (action_type, amount))
        ...
        reward = (new_balance - previous_balance) / previous_balance  # Rendement en pourcentage
        ...
        try:
            ...
            if self.agent is None:
                ...
                try:
                    ...
                    logger.info(f"Agent DQN créé avec state_size={state_size}, action_size={action_size}")
                except ImportError:
                    logger.error("Impossible d'importer DQNAgent, utilisation d'un agent fictif")
    ...
    def backtest(self, data):
        ...
        try:
            ...
            if trades:
                ...
                sharpe_ratio = np.mean(returns) / (np.std(returns) + 1e-8) * np.sqrt(252)  # Annualisé
            else:
                sharpe_ratio = 0
            ...
            logger.info(f"Backtest terminé. Profit: {profit_pct:.2f}%, BH: {bh_profit_pct:.2f}%, Trades: {len(trades)}")
            return results
        ...
        try:
            ...
            if "dqn" in model_path.lower():
                ...
                try:
                    ...
                    self.agent.load(model_path)
                    logger.info(f"Agent chargé par défaut en tant que DQN depuis {model_path}")
                except ImportError:
                    logger.error("Impossible d'importer DQNAgent")
                    raise
    ...
    def save(self, model_path):
        ...
        try:
            ...
            if hasattr(self.agent, 'save'):
                ...
                raise NotImplementedError("L'agent ne possède pas de méthode de sauvegarde")
