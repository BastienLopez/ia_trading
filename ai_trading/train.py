...
def plot_backtest_results(results, save_path=None):
    ...
    # Tracer la valeur du portefeuille
    ax2.plot(
        df.index, df["portfolio_value"], label="Valeur du portefeuille", color="orange"
    )
    ax2.set_ylabel("Valeur ($)")
    ax2.legend()
    ax2.grid(True)
...
def main():
    parser = argparse.ArgumentParser(
        description="Entraînement d'un agent RL pour le trading de crypto"
    )
    parser.add_argument(
        "--download", action="store_true", help="Télécharger de nouvelles données"
    )
    parser.add_argument(
        "--exchange", type=str, default="binance", help="Exchange à utiliser"
    )
    parser.add_argument(
        "--symbol", type=str, default="BTC/USDT", help="Paire de trading"
    )
    parser.add_argument(
        "--timeframe", type=str, default="1h", help="Intervalle de temps"
    )
    parser.add_argument(
        "--days", type=int, default=60, help="Nombre de jours de données historiques"
    )
    parser.add_argument(
        "--timesteps", type=int, default=50000, help="Nombre d'étapes d'entraînement"
    )
    parser.add_argument(
        "--backtest",
        action="store_true",
        help="Effectuer un backtest après l'entraînement",
    )
    ...
    if args.download:
        ...
        if not os.path.exists(data_path):
            print(
                f"Fichier de données {data_path} non trouvé. Utilisation de l'option --download pour télécharger des données."
            )
            return
    ...
    metrics = agent.train(
        train_data, total_timesteps=args.timesteps, save_path=save_path
    )

    # Tracer les résultats d'entraînement
    plot_training_results(
        metrics, save_path=f"ai_trading/models/{model_name}_training.png"
    )
    ...
    if args.backtest:
        ...
        plot_backtest_results(
            backtest_results, save_path=f"ai_trading/models/{model_name}_backtest.png"
        )
