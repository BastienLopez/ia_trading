...
from ai_trading.data.data_optimizers import convert_to_parquet, convert_to_hdf5
from ai_trading.utils.performance_tracker import log_memory_usage, track_time
...
def profile_dataset_creation(data, sequence_length=50, lazy_loading=False, chunk_size=None):
...
def profile_async_vs_sync_loading(data_path, sequence_length=50, batch_size=32):
    ...
    print(f"Chargement synchrone - Temps d'initialisation: {sync_load_time:.2f}s, Temps d'itération: {sync_iter_time:.2f}s")
    ...
    print(f"Chargement asynchrone - Temps d'initialisation: {async_load_time:.2f}s, Temps d'itération: {async_iter_time:.2f}s")
    ...
    return {
        'sync': {'init_time': sync_load_time, 'iter_time': sync_iter_time, 'total_time': total_sync_time},
        'async': {'init_time': async_load_time, 'iter_time': async_iter_time, 'total_time': total_async_time},
        'speedup': speedup
    }
...
def run_cpu_profiling():
    ...
    print("=== Profilage CPU pour l'optimisation des performances de chargement des données ===\n")
    ...
    if optimal_workers > 0:
        _, optimal_prefetch = profile_prefetch_factors(lazy_dataset, batch_size=32, num_workers=optimal_workers)
    else:
    ...
    print(f"1. Utilisation de lazy_loading=True pour réduire la charge mémoire et CPU à l'initialisation")
    ...
    if 'async_results' in locals():
        print(f"4. Utilisation du chargement asynchrone pour une accélération de {async_results['speedup']:.2f}x")
