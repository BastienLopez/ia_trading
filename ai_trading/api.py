...
class PredictionRequest(BaseModel):
    ...
    use_latest: bool = Field(default=True, description="Utiliser les dernières données disponibles")
    days: Optional[int] = Field(default=1, description="Nombre de jours de données à analyser")
...
class BacktestRequest(BaseModel):
    ...
    symbol: str = Field(default="BTC/USDT", description="Paire de trading")
    timeframe: str = Field(default="1h", description="Intervalle de temps")
    days: int = Field(default=30, description="Nombre de jours de données pour le backtest")
    ...
    try:
        ...
        if request.use_latest:
            ...
            if not os.path.exists(data_path):
                raise HTTPException(status_code=404, detail=f"Données non trouvées: {data_path}")
        ...
        model_name = f"{request.symbol.replace('/', '_')}_{request.timeframe}_rl_model.zip"
        model_path = f"ai_trading/models/{model_name}"
        
        if not os.path.exists(model_path):
            raise HTTPException(status_code=404, detail=f"Modèle non trouvé: {model_path}")
...
async def train(
    ...
    try:
        logger.info(f"Entraînement demandé pour {request.symbol} sur {request.exchange}")
        ...
        logger.info(f"Début de l'entraînement sur {len(train_data)} points de données...")
        metrics = agent.train(train_data, total_timesteps=request.timesteps, save_path=save_path)
...
async def backtest(
    ...
    try:
        ...
        return {
            ...
            "num_trades": len(backtest_results["trades"]),
            "win_rate": len([t for t in backtest_results["trades"] if t > 0]) / len(backtest_results["trades"]) if len(backtest_results["trades"]) > 0 else 0,
