...
class StatisticalTestSuite:
    ...
    def sharpe_ratio(
        ...
        return (
            np.sqrt(annualization) * excess_returns.mean() / excess_returns.std(ddof=1)
        )
    ...
    def jarque_bera_test(returns: pd.Series) -> Dict[str, float]:
        ...
        return {"statistic": jb_stat, "p_value": p_value, "is_normal": p_value > 0.05}
    ...
    def ljung_box_test(returns: pd.Series, lags: int = 10) -> Dict[str, float]:
        ...
        lb_stat = (
            ...
            * (n + 2)
            * np.sum(
                [(acf_val**2) / (n - k) for k, acf_val in enumerate(acf_values, 1)]
            )
        )
    ...
    def white_reality_check(
        ...
        if HAS_ARCH:
            try:
                ...
                simulated_diffs = StatisticalTestSuite._manual_bootstrap(
                    perf_diff, n_bootstrap
                )
        else:
    ...
    def stability_test(returns: pd.Series, window_size: int = 63) -> Dict[str, float]:
        ...
        rolling_sharpe = returns.rolling(window=window_size).apply(
            lambda x: StatisticalTestSuite.sharpe_ratio(x, annualization=window_size),
            raw=False,
        )
        ...
        return {
            ...
            "sharpe_stability": (
                rolling_sharpe.std() / abs(rolling_sharpe.mean())
