...
class MarketRegimeClassifier:
    ...
    def get_regime_characteristics(self, df: pd.DataFrame) -> Dict[int, Dict[str, float]]:
        """
...
class RobustnessEvaluator:
    ...
    def evaluate(self, df: pd.DataFrame, returns: pd.Series, 
                 metrics: List[Callable[[pd.Series], float]]) -> Dict[int, Dict[str, float]]:
        """
        ...
        regimes_df = pd.DataFrame({'regime': regimes}, index=self.classifier.extract_features(df).index)
    ...
    def stress_test(self, df: pd.DataFrame, strategy_func: Callable, 
                   scenarios: List[Dict[str, float]], 
                   metrics: List[Callable[[pd.Series], float]]) -> Dict[str, Dict[str, float]]:
        """
