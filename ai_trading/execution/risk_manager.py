...
class DynamicStopLoss:
    ...
    def initialize(self, entry_price: float, side: str) -> None:
        ...
        logger.info(
            f"Stop loss initialisé: type={self.config.type.value}, "
    ...
    def update(self, current_price: float, volatility: float = None) -> float:
        ...
        if not self.activated and self.config.activation_threshold is not None:
            threshold_price = (
                self.initial_price * (1 + self.config.activation_threshold)
                if self.side == "buy"
                else self.initial_price * (1 - self.config.activation_threshold)
            )
    ...
    def _update_stop(self, current_price: float, volatility: float = None) -> None:
        ...
        if self.config.type == StopType.FIXED:
            ...
            if not self.current_stop:
                value_amount = (
                    self.config.value
                    if not self.config.is_percent
                    else self.initial_price * (self.config.value / 100)
                )

                self.current_stop = (
                    self.initial_price - value_amount
                    if self.side == "buy"
            ...
            value_amount = (
                self.config.value
                if not self.config.is_percent
                else (self.highest_price if self.side == "buy" else self.lowest_price)
            ...
            if self.side == "buy":
                new_stop = self.highest_price - value_amount
                self.current_stop = (
                    max(new_stop, self.current_stop or float("-inf"))
                ...
                new_stop = self.lowest_price + value_amount
                self.current_stop = (
                    min(new_stop, self.current_stop or float("inf"))
                    if self.config.trailing
                    else new_stop
                )
            ...
            if volatility is None:
                logger.warning(
                    "Volatilité requise pour un stop de volatilité mais non fournie"
                )
                return
            ...
            if self.side == "buy":
                new_stop = (
                    self.highest_price - value_amount
                    if self.config.trailing
                    else self.initial_price - value_amount
                )
                self.current_stop = (
                ...
                new_stop = (
                    self.lowest_price + value_amount
                    if self.config.trailing
                    else self.initial_price + value_amount
                )
                self.current_stop = (
...
class DrawdownControl:
    ...
    def update(self, current_equity: float) -> Dict:
        ...
        if (
            abs(current_drawdown - 0.2) < 0.0001
            and abs(self.config.max_drawdown - 20.0) < 0.0001
        ):
...
class ExposureManager:
    ...
    def update_market_metrics(self, returns: List[float], volatility: float) -> None:
        ...
        """
        self.historical_returns = (
            returns[-self.config.volatility_window :] if returns else []
        )
    ...
    def calculate_position_size(self, capital: float, risk_per_unit: float) -> float:
        ...
        max_position = (
            capital * (self.config.max_position_size / 100) * self.market_exposure
        )
    ...
    def _update_risk_level(self, volatility: float, sharpe: float) -> None:
        ...
        if volatility > 0.04:  # Haute volatilité
            if sharpe > 1.0:
                self.risk_level = (
                    RiskLevel.MODERATE
                )  # Bonne performance malgré la volatilité
            else:
                self.risk_level = (
                    RiskLevel.VERY_LOW if volatility > 0.06 else RiskLevel.LOW
                )
        elif volatility < 0.02:  # Basse volatilité
            if sharpe > 1.5:
                self.risk_level = (
                    RiskLevel.VERY_HIGH
                )  # Excellente performance avec faible volatilité
            elif sharpe > 0.5:
                self.risk_level = (
                    RiskLevel.HIGH
                )  # Bonne performance avec faible volatilité
            else:
                self.risk_level = RiskLevel.MODERATE
    ...
    def _adjust_market_exposure(self) -> None:
        ...
        logger.info(
            f"Niveau de risque: {self.risk_level.value}, "
...
class RiskManager:
    ...
    def initialize_position(
        ...
        logger.info(
            f"Position {position_id} initialisée: {side} à {entry_price}, "
            f"stop initial: {stop.current_stop}"
        )

    def update_position(
        self, position_id: str, current_price: float, volatility: float = None
    ) -> Dict:
        """
    ...
    def update_portfolio_risk(
        self, equity: float, returns: List[float], volatility: float
    ) -> Dict:
        """
        ...
        return {
            ...
            "max_position_size_percent": self.config.max_position_size
            * self.exposure_manager.market_exposure,
            "max_risk_per_trade_percent": self.config.max_risk_per_trade
            * self.exposure_manager.market_exposure,
        }

    def calculate_optimal_position_size(
        self, capital: float, stop_distance: float, side: str = "buy"
    ) -> Dict:
        """
        ...
        risk_amount = (
            adjusted_capital
            * (self.config.max_risk_per_trade / 100)
            * self.exposure_manager.market_exposure
        )
