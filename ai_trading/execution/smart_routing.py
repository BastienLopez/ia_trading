...
class OrderRoutingStrategy(ABC):
    ...
    def select_venue(
        ...
        order_type: str,
        quantity: float,
        available_venues: List[ExchangeInfo],
    ) -> ExchangeVenue:
        """
...
class BestLiquidityStrategy(OrderRoutingStrategy):
    ...
    def select_venue(
        ...
        for venue in eligible_venues:
            ...
            liquidity = self.liquidity_provider.get_liquidity_data(
                symbol=symbol, venue=venue.venue
            )
    ...
    def _estimate_slippage(
        self, liquidity: Dict, quantity: float, is_buy: bool
    ) -> float:
        """
...
class SmartRoutingStrategy(OrderRoutingStrategy):
    ...
    def select_venue(
        ...
        for venue in eligible_venues:
            ...
            speed_score = (
                1 - (venue.avg_latency_ms / max_latency) if max_latency > 0 else 1
            )
...
class SmartRouter:
    ...
    def route_order(
        ...
        priority: ExchangePriority = ExchangePriority.SMART,
        exclude_venues: List[ExchangeVenue] = None,
    ) -> ExchangeVenue:
        """
        ...
        """
        logger.info(
            f"Routage d'ordre: {symbol} {side} {quantity} ({order_type}) avec priorité {priority.value}"
        )
        ...
        available_venues = [
            info
            for venue, info in self.exchange_info.items()
    ...
    def _estimate_market_impact(
        self, symbol: str, quantity: float, venue: ExchangeVenue, side: str
    ) -> float:
        """
