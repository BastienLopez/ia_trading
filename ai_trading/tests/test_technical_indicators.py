...
class TestTechnicalIndicators(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.test_data["close"] = (
            self.test_data["close"].rolling(5, min_periods=1).mean()
        )
    ...
    def test_macd(self):
        ...
        pd.testing.assert_series_equal(
            histogram, macd_line - signal_line, check_dtype=False, check_names=False
        )

        # Vérifier les croisements du MACD
        crossovers = (macd_line > signal_line) & (
    ...
    def test_adx(self):
        ...
        self.assertTrue(
            (plus_di.dropna() >= 0).all() and (plus_di.dropna() <= 100).all()
        )
        self.assertTrue(
            (minus_di.dropna() >= 0).all() and (minus_di.dropna() <= 100).all()
        )
    ...
    def test_volume_average(self):
        ...
        pd.testing.assert_series_equal(
            volume_avg[20:], sma[20:], check_dtype=False, check_names=False
        )
    ...
    def test_mfi(self):
        ...
        self.assertTrue(
            (mfi[14:].dropna() >= 0).all() and (mfi[14:].dropna() <= 100).all()
        )
    ...
    def test_normalize_indicator(self):
        ...
        self.assertTrue(
            (rsi_minmax[14:].dropna() >= 0).all()
            and (rsi_minmax[14:].dropna() <= 1).all()
        )
    ...
    def test_get_all_indicators(self):
        ...
        new_indicators = ["pivot_P", "ichimoku_tenkan", "donchian_upper", "vp_poc"]
    ...
    def test_volume_profile(self):
        ...
        self.assertTrue(
            all(col in vp.columns for col in ["vp_poc", "vp_vah", "vp_val"])
        )
    ...
    def test_ichimoku_cloud(self):
        ...
        tenkan, kijun, senkou_a, senkou_b, chikou = (
            self.indicators.calculate_ichimoku_cloud()
        )
