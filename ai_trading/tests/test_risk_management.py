...
def simulate_trade_with_risk_management():
    ...
    print(f"Taille de la position: {position_size:.6f} BTC (${position_size * entry_price:,.2f})")
    ...
    price_moves = [
        50500, 51200, 51800, 52500, 53000, 52800, 52400, 51500
    ]
    ...
    for i, current_price in enumerate(price_moves):
        ...
        if position_update["stop_triggered"]:
            print(f"Stop déclenché! Clôture de la position à ${position_update['stop_level']:,.2f}")
    ...
    print(f"Facteur de mise à l'échelle des positions: "
    ...
    new_position_sizing = risk_manager.calculate_optimal_position_size(
        ...
        stop_distance=1500,  # Distance du stop
        side="buy"
    )
    
    print(f"Nouvelle taille de position (ajustée): {new_position_sizing['position_size']:.6f} BTC")
    print(f"Nouvelle taille de position (montant): "
    ...
    print(f"Drawdown maximal observé: {recovery_metrics['drawdown']['max_drawdown_seen']:.2%}")
