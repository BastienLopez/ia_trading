...
class TestStatisticalTestSuite:
    ...
    def test_sharpe_ratio(self, sample_returns):
        ...
        expected_sharpe = np.sqrt(252) * sample_returns.mean() / sample_returns.std(ddof=1)
        assert np.isclose(sharpe, expected_sharpe)
        
        # Vérifier que les paramètres optionnels fonctionnent
        sharpe_with_rf = StatisticalTestSuite.sharpe_ratio(sample_returns, risk_free_rate=0.02)
    ...
    def test_stability_test(self, sample_returns):
        ...
        stability_results = StatisticalTestSuite.stability_test(sample_returns, window_size=50)
        ...
        assert stability_results['sharpe_min'] <= stability_results['sharpe_mean'] <= stability_results['sharpe_max']
