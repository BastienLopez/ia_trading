...
def test_smart_routing_integration():
    ...
    for priority in test_priorities:
        ...
        print(f"Priorité: {priority.value} -> Venue sélectionnée: {selected_venue.value}")
    ...
    for venue in [ExchangeVenue.BINANCE, ExchangeVenue.COINBASE, ExchangeVenue.FTX]:
        ...
        print(f"  - Slippage: {costs['slippage_pct']}% (${costs['slippage_amount']:,.2f})")
        print(f"  - Impact marché: {costs['impact_pct']}% (${costs['impact_amount']:,.2f})")
        print(f"  - Coût total: ${costs['total_cost']:,.2f} ({costs['total_cost_pct']:.4f}%)")
