...
class SimpleEncoder(nn.Module):
    ...
    def __init__(self, input_dim, output_dim=2):
        ...
        self.encoder = nn.Sequential(nn.Linear(input_dim, output_dim))
...
class TestEnhancedPrioritizedReplay(unittest.TestCase):
    ...
    def test_update_priorities(self):
        ...
        for i, idx in enumerate(indices):
            ...
            expected_priority = (
                abs(td_errors[i]) + self.buffer.epsilon
            ) ** self.buffer.alpha
    ...
    def test_n_step_returns(self):
        ...
        self.assertGreaterEqual(
            len(n_step_buffer), 1, "Le buffer devrait contenir au moins 1 expérience"
        )
        ...
        if len(n_step_buffer) > 0:
            ...
            self.assertTrue(
                abs(batch[0].reward - min_expected) < 0.2
                or abs(batch[0].reward - 7.94) < 0.2,
    ...
    def test_redundancy_detection(self):
        ...
        similar_reward = 1.2
        similar_next_state = np.array([2.0, 3.0, 4.0, 5.0]) + np.random.normal(
            0, 0.1, 4
        )
        similar_done = False
