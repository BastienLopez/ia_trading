...
class TestPolicyLag(unittest.TestCase):
    ...
    def test_model_copy(self):
        ...
        self.assertEqual(
            train_model.get_random_value(), collect_model.get_random_value()
        )
        self.assertEqual(
            train_model.get_random_value(), target_model.get_random_value()
        )
    ...
    def test_sync_update(self):
        ...
        policy_lag = PolicyLag(
            model=model, update_frequency=5, target_update_freq=10, async_update=False
        )
        ...
        self.assertEqual(
            policy_lag.get_collect_model().get_random_value(), initial_value
        )
        self.assertEqual(
            policy_lag.get_target_model().get_random_value(), initial_value
        )
    ...
    def test_async_update(self):
        ...
        policy_lag = PolicyLag(
            model=model, update_frequency=5, target_update_freq=10, async_update=True
        )
...
class TestDecoupledPolicyTrainer(unittest.TestCase):
    ...
    def test_collect(self):
        ...
        steps_needed = self.trainer.policy_lag.update_frequency - (
            current_steps % self.trainer.policy_lag.update_frequency
        )
        ...
        for _ in range(
            steps_needed - 1
        ):  # -1 car on va faire une dernière collecte après
