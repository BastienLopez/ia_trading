...
class TestMultiAssetTradingEnv(unittest.TestCase):
    ...
    def test_action_execution(self):
        ...
        self.assertIsInstance(obs, np.ndarray)
        self.assertGreater(reward, -1.0)  # La récompense ne devrait pas être trop négative
        ...
        self.assertLess(env.crypto_holdings["BTC"], env.crypto_holdings["BTC"] + env.crypto_holdings["ETH"] + env.crypto_holdings["LTC"])
    ...
    def test_different_allocation_methods(self):
        ...
        for method in methods:
            ...
            self.assertGreater(portfolio_value, 0.0)  # Simplement vérifier que la valeur est positive
    ...
    def test_portfolio_allocator(self):
        ...
        action_weights = np.array([0.1, 0.8, 0.9])  # LTC est clairement le meilleur, ETH le second
        symbols = ["BTC", "ETH", "LTC"]
        ...
        allocation = allocator.allocate(action_weights, symbols, volatilities=volatilities)
