...
class TestTransformerSACAgent(unittest.TestCase):
    ...
    def test_get_sequence_state(self):
        ...
        for i in range(3):
            np.testing.assert_array_equal(sequence[self.sequence_length - 3 + i], np.ones(self.state_dim) * i)
    ...
    def test_update_target_networks(self):
        ...
        for i in range(len(initial_weights_critic_1)):
            np.testing.assert_array_almost_equal(
                initial_weights_critic_1[i],
                updated_weights_target_critic_1[i]
            )
    ...
    def test_save_load_models(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.agent.checkpoints_dir = temp_dir  # Utiliser le dossier temporaire directement
            ...
            try:
                ...
                for i in range(len(original_actor_weights)):
                    if not np.array_equal(original_actor_weights[i], modified_actor_weights[i]):
                ...
                for i in range(len(original_actor_weights)):
                    np.testing.assert_array_almost_equal(
                        original_actor_weights[i],
                        loaded_actor_weights[i]
                    )
            finally:
    ...
    def test_integration(self):
        ...
        if len(self.agent.replay_buffer) >= 4:
            ...
            sequence_states = np.zeros((4, self.sequence_length, self.state_dim), dtype=np.float32)
            sequence_next_states = np.zeros((4, self.sequence_length, self.state_dim), dtype=np.float32)
            ...
            next_states_tf = tf.convert_to_tensor(sequence_next_states, dtype=tf.float32)
            ...
            with tf.GradientTape(persistent=True) as tape:
                ...
                repeated_actions = tf.repeat(next_actions[:, tf.newaxis, :], repeats=self.sequence_length, axis=1)
                
                # Concaténer les états et les actions pour l'entrée du critique
                # Adaptée pour assurer la compatibilité des formes
                next_state_actions_1 = tf.concat([next_states_tf, repeated_actions], axis=-1)
                
                # Évaluer les fonctions Q
                target_q1 = self.agent.target_critic_1(next_state_actions_1, training=False)
                target_q2 = self.agent.target_critic_2(next_state_actions_1, training=False)
