...
def patch_real_time_adapter(monkeypatch):
    ...
    def mock_add_data(self, data):
        ...
        # S'assurer que current_market_state existe
        if (
            not hasattr(self, "current_market_state")
            or self.current_market_state is None
        ):
    ...
    monkeypatch.setattr(
        RealTimeAdapter, "_generate_prediction", mock_generate_prediction
    )
    ...
    monkeypatch.setattr(
        RealTimeAdapter, "_is_significant_change", mock_is_significant_change
    )
