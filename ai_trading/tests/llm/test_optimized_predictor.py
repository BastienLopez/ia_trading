...
class TestOptimizedMarketPredictor(unittest.TestCase):
    ...
    def test_cache_performance(self):
        ...
        logger.info(
            f"Premier appel: {duration_1:.3f}s, Deuxième appel: {duration_2:.3f}s"
        )
    ...
    def test_retry_mechanism(self):
        ...
        try:
            ...
            self.assertEqual(
                call_count[0],
                2,
                "Le mécanisme de retry n'a pas fonctionné correctement",
            )
    ...
    def test_cached_sentiment_analysis(self):
        ...
        self.assertEqual(
            news_calls[0], 1, "L'analyseur de nouvelles a été appelé plusieurs fois"
        )
        self.assertEqual(
            social_calls[0], 1, "L'analyseur social a été appelé plusieurs fois"
        )
    ...
    def test_batch_predict(self):
        ...
        try:
            ...
            if hasattr(self.model.market_predictor, "batch_predict_directions"):
                original_batch_predict_directions = (
                    self.model.market_predictor.batch_predict_directions
                )

                def mock_batch_predict_directions(assets_list, timeframe):
                    print(
                        f"Mock batch_predict_directions appelé avec {len(assets_list)} actifs sur {timeframe}"
                    )
                    result = {}
                    for asset in assets_list:
                        result[asset] = {
                            ...
                            "direction": (
                                "bullish"
                                if asset == "BTC"
                                else "bearish" if asset == "ETH" else "neutral"
                            ),
                            "confidence": "medium",
            ...
            self.assertEqual(
                len(predictions),
                len(assets),
                f"Nombre de prédictions ({len(predictions)}) différent du nombre d'actifs ({len(assets)})",
            )
            ...
            print(
                f"Prédictions batch pour {len(assets)} actifs effectuées en {duration:.3f}s"
            )
    ...
    def test_cache_integration(self):
        ...
        try:
            ...
            def mock_predict(asset, timeframe):
                ...
                print(
                    f"Mock predict appelé {predict_call_count[0]} fois avec asset={asset}, timeframe={timeframe}"
                )
            ...
            def mock_get_cache_stats():
                ...
                return {
                    "hits": (
                        predict_call_count[0] - 1 if predict_call_count[0] > 0 else 0
                    ),
                    "misses": 1,
                    "total_entries": 1,
                    "memory_usage": 1024,
            ...
            print(
                f"Premier appel: direction={result_1['direction']}, durée={duration_1:.3f}s"
            )
            ...
            print(
                f"Deuxième appel: direction={result_2['direction']}, durée={duration_2:.3f}s"
            )
            ...
            self.assertGreaterEqual(
                cache_stats["hits"], 1, "Le cache n'a pas été utilisé"
            )
