...
class TestUncertaintyCalibration(unittest.TestCase):
        ...
        self.predictions = [
            {
                ...
                "asset": "BTC",
                "direction": (
                    "bullish" if i % 3 == 0 else "neutral" if i % 3 == 1 else "bearish"
                ),
                "confidence": "medium",
                "timeframe": "24h",
            }
    ...
    def test_probability_distribution(self):
        ...
        distribution = self.calibrator.estimate_probability_distribution(
            self.prediction
        )
    ...
    def test_outlier_detection(self):
        ...
        result_z = self.calibrator.detect_outliers(
            self.predictions, method="z_score", threshold=2.0
        )
        ...
        result_iqr = self.calibrator.detect_outliers(
            self.predictions, method="iqr", threshold=1.5
        )
    ...
    def test_prediction_calibration(self):
        ...
        for method in methods:
            calibrated = self.calibrator.calibrate_prediction(
                self.prediction, calibration_method=method
            )
