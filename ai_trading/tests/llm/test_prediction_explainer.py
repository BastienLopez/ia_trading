...
matplotlib.use(
    "Agg"
)  # Utiliser le backend Agg (non-interactif) avant d'importer pyplot
...
class TestPredictionExplainer(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_prediction_model.ml_model.predict_proba = Mock(
            return_value=np.array([[0.2, 0.1, 0.7]])
        )

        # Créer une instance de l'explainer pour les tests
        self.explainer = PredictionExplainer(
            self.mock_prediction_model, self.market_data
        )
    ...
    def test_explain_with_shap(self, mock_shap_explainer):
        ...
        explanation = self.explainer.explain_with_shap(
            self.prediction, self.market_data
        )
    ...
    def test_explain_with_lime(self, mock_lime_explainer):
        ...
        # Appeler la méthode à tester
        explanation = self.explainer.explain_with_lime(
            self.prediction, self.market_data
        )
    ...
    def test_generate_report(self):
        ...
        self.explainer.explain_with_shap = Mock(
            return_value={"shap_values": np.random.rand(10, 9)}
        )
        self.explainer.explain_with_lime = Mock(
            return_value={"features": ["rsi", "macd"], "scores": [0.2, 0.15]}
        )
        self.explainer._explain_llm_prediction = Mock(
            return_value={"factors": ["Price momentum"]}
        )

        # Appeler la méthode à tester
        report = self.explainer.generate_report(
            self.prediction, ["shap", "lime", "llm"]
        )
    ...
    def test_plotting_methods(self):
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                self.explainer.plot_factor_importance(
                    self.prediction, save_path=factor_path
                )
