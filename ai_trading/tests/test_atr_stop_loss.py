...
def test_atr_stop_loss(verbose=True, config=None):
    ...
    for i in range(14, len(data) - 1):
        ...
        if current_position is None:
            ...
            if next_price > current_price * 1.005:
                ...
                print(
                    f"Position ouverte à {entry_price:.2f}, stop-loss à {stop_loss:.2f}, take-profit à {take_profit:.2f}"
                )
        else:
            ...
            risk_manager.update_volatility_adjusted_stops(
                data.iloc[: i + 1], position_id
            )
            ...
            stop_result = risk_manager.check_stop_conditions(
                position_id=position_id, current_price=next_price, direction="long"
            )
            ...
            if stop_result["stop_triggered"]:
                ...
                trades.append(trade)
                print(
                    f"Position fermée à {exit_price:.2f} ({stop_result['stop_type']}), profit: {profit_pct:.2f}%"
                )
    ...
    if current_position is not None:
        ...
        trades.append(trade)
        print(
            f"Position fermée à la fin des données à {exit_price:.2f}, profit: {profit_pct:.2f}%"
        )
    ...
    avg_win = (
        sum(trade["profit_pct"] for trade in win_trades) / len(win_trades)
        if win_trades
        else 0
    )
    avg_loss = (
        sum(trade["profit_pct"] for trade in loss_trades) / len(loss_trades)
        if loss_trades
        else 0
    )
...
def visualize_trades(data, trades):
    ...
    for trade in trades:
        ...
        # Tracer l'entrée
        ax1.scatter(
            data.index[entry_index], entry_price, color=color, marker="^", s=100
        )
        ...
        ax1.scatter(
            data.index[exit_index], exit_price, color=color, marker=marker, s=100
        )

        # Tracer la ligne entre l'entrée et la sortie
        ax1.plot(
            [data.index[entry_index], data.index[exit_index]],
    ...
    ax2.axhline(
        y=0.20, color="orange", linestyle="--", label="Seuil de Volatilité (20%)"
    )
