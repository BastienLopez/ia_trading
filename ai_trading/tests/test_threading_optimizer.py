...
class TestThreadingOptimizer(unittest.TestCase):
    ...
    def test_dataloader_config(self):
        ...
        small_config = self.optimizer.get_dataloader_config(data_size=100, batch_size=32)
        ...
        large_config = self.optimizer.get_dataloader_config(data_size=100000, batch_size=32)
        
        # Le grand dataset devrait avoir plus ou le même nombre de workers que le petit
        self.assertGreaterEqual(large_config['num_workers'], small_config['num_workers'])
...
class TestParallelProcessing(unittest.TestCase):
    ...
    def test_parallel_map_threads(self):
        ...
        start_time = time.time()
        results_parallel = parallel_map(process_item, items, max_workers=4, use_processes=False)
        ...
        logger.info(f"Temps séquentiel: {seq_time:.4f}s, Temps parallèle: {parallel_time:.4f}s")
    ...
    def test_parallel_map_processes(self):
        ...
        results = parallel_map(factorial_test_function, items, use_processes=True, max_workers=2)
...
def run_performance_comparison(iterations=5):
    ...
    for config in configs:
        ...
        for _ in range(iterations):
            ...
            start_time = time.time()
            parallel_map(cpu_intensive_task, items, max_workers=workers, use_processes=True)
...
if __name__ == "__main__":
    ...
    if fastest == "1 worker (séquentiel)":
        ...
        print(f"Cette configuration offre une accélération de {results['1 worker (séquentiel)'] / results[fastest]:.2f}x par rapport au traitement séquentiel.") 