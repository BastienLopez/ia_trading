...
class TestCNNPatternDetector(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.window_size = 20
        self.detector = CNNPatternDetector(
            window_size=self.window_size, confidence_threshold=0.7
        )
    ...
    def test_merge_overlapping_patterns(self):
        ...
        merged = self.detector._merge_overlapping_patterns(
        ...
        self.assertEqual(
            len(merged), 2
        )  # pattern1 et pattern2 devraient être fusionnés
        ...
        expected_confidence = ((0.8 * 21) + (0.9 * 21)) / (
            21 + 21
        )  # (conf * durée) / durée_totale
