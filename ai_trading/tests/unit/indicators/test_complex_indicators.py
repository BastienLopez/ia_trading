...
class TestSpectralAnalyzer(unittest.TestCase):
    ...
    def test_find_dominant_cycles(self):
        ...
        expected_periods = [
            100 / p for p in expected_periods
    ...
    def test_filter_by_frequency(self):
        ...
        band_pass = self.analyzer.filter_by_frequency(
            self.data, low_cutoff=9, high_cutoff=11
        )
        ...
        freq_idx = np.where(
            (self.analyzer.periodogram(self.data)[0] >= 9)
            & (self.analyzer.periodogram(self.data)[0] <= 11)
        )[0]
...
class TestMultiTimeframeIndicator(unittest.TestCase):
    ...
    def setUp(self):
        ...
        for tf in self.timeframes:
            ...
            n_points = (
                100 if tf == TimeFrame.HOUR_1 else (25 if tf == TimeFrame.HOUR_4 else 7)
            )
    ...
    def test_get_alignment_score(self):
        ...
        alignment_score = self.indicator.get_alignment_score(
            self.indicator.calculate_rsi
        )
...
class TestDivergenceDetector(unittest.TestCase):
    ...
    def setUp(self):
        ...
        indicator[50:] = (
            50 + 8 * np.sin(np.linspace(0, np.pi, 50)) + 5
        )  # +5 pour que le second creux soit plus haut
        ...
        self.detector = DivergenceDetector(
            self.price_data, self.indicator_data, window_size=5
        )
    ...
    def test_find_extrema(self):
        ...
        for idx in maxima:
            ...
            self.assertTrue(
                self.price_data["close"].iloc[idx]
                > self.price_data["close"].iloc[idx - 1]
            )
            self.assertTrue(
                self.price_data["close"].iloc[idx]
                > self.price_data["close"].iloc[idx + 1]
