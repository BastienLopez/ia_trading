...
class TestZstandardParquetCompressor(unittest.TestCase):
    ...
    def test_save_and_load_parquet(self):
        ...
        ]  # Réordonner les colonnes pour correspondre
        ...
        pd.testing.assert_series_equal(
            self.test_data["id"], loaded_df["id"], check_dtype=False
        )
    ...
    def test_save_with_partitioning(self):
        ...
        saved_path = compressor.save_to_parquet(
            self.test_data, test_dir, partition_cols=["category_col"]
        )
        ...
        self.assertEqual(
            set(self.test_data["id"].unique()), set(loaded_df["id"].unique())
        )
    ...
    def test_train_zstd_dict(self):
        ...
        samples = [self.test_data.sample(n=100) for _ in range(3)]
...
class TestDataStreamProcessor(unittest.TestCase):
    ...
    def test_init(self):
        ...
        processor = DataStreamProcessor(
            chunk_size=10000, use_dask=True, n_workers=2, progress_bar=False
        )
    ...
    def test_process_in_chunks_csv(self):
        ...
        processor = DataStreamProcessor(
            chunk_size=1000, use_dask=False, progress_bar=False
        )
        ...
        result = processor.process_in_chunks(
            self.csv_path, process_func, file_type="csv"
        )
    ...
    def test_process_in_chunks_with_dask(self):
        ...
        processor = DataStreamProcessor(
            chunk_size=1000, use_dask=True, progress_bar=False
        )
        ...
        try:
            result = processor.process_in_chunks(
                self.parquet_path, process_func, file_type="parquet"
            )
            ...
            # Vérifier les valeurs calculées pour quelques échantillons
            self.assertEqual(
                result.iloc[0]["doubled_id"], self.test_data.iloc[0]["id"] * 2
            )
    ...
    def test_process_with_output(self):
        ...
        processor.process_in_chunks(
            self.csv_path, process_func, output_path=output_path, file_type="csv"
        )
...
class TestUtilityFunctions(unittest.TestCase):
    ...
    def test_quick_save_load_parquet(self):
        ...
        saved_path = quick_save_parquet(
            test_df, test_file, compression_level=5, optimize_types=True
        )
        ...
        pd.testing.assert_series_equal(
            test_df["id"], loaded_df["id"], check_dtype=False
        )
    ...
    def test_stream_process_file(self):
        ...
        result = stream_process_file(
            csv_path, process_func, chunk_size=200, use_dask=False
        )
        ...
        self.assertTrue(
            expected_processed.reset_index(drop=True).equals(actual_processed)
        )
