...
from ai_trading.llm.sentiment_analysis.advanced_llm_integrator import (
    AdvancedLLMIntegrator,
)

# Patch pour simuler la disponibilité de PEFT
patch(
    "ai_trading.llm.sentiment_analysis.advanced_llm_integrator.PEFT_AVAILABLE", True
).start()
...
class TestAdvancedLLMIntegrator(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.patcher = patch(
            "ai_trading.llm.sentiment_analysis.advanced_llm_integrator.AutoModelForSequenceClassification"
        )
        self.mock_model = self.patcher.start()

        self.tokenizer_patcher = patch(
            "ai_trading.llm.sentiment_analysis.advanced_llm_integrator.AutoTokenizer"
        )
        self.mock_tokenizer = self.tokenizer_patcher.start()

        self.pipeline_patcher = patch(
            "ai_trading.llm.sentiment_analysis.advanced_llm_integrator.pipeline"
        )
    ...
    def test_analyze_sentiment(self):
        ...
        result = self.integrator.analyze_sentiment("Bitcoin est en hausse aujourd'hui.")
    ...
    def test_process_sentiment_results_cryptobert(self):
        ...
        processed = self.integrator._process_sentiment_results(
            results, "ElKulako/cryptobert"
        )
    ...
    def test_process_sentiment_results_generic(self):
        ...
        results = [[{"label": "1", "score": 0.6}, {"label": "0", "score": 0.4}]]
    ...
    @patch(
        "ai_trading.llm.sentiment_analysis.advanced_llm_integrator.AutoModelForCausalLM"
    )
    ...
    def test_optimize_prompts(self):
        ...
        self.integrator.generate_crypto_analysis.side_effect = (
            lambda prompt, model_name: ("positif" if "expert" in prompt else "négatif")
        )
        ...
        # Cas de test
        test_cases = [{"input": "BTC", "expected": "positif"}]

        # Optimisation des prompts
        result = self.integrator.optimize_prompts(
            "Analyse", test_cases, model_name="test/model"
        )
