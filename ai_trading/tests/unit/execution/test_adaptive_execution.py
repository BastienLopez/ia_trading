...
def test_passive_execution_strategy():
    ...
    impact = strategy.calculate_impact_estimate(
        symbol="BTC/USD", quantity=1.0, side="buy"
    )
    assert impact > 0
...
def test_normal_execution_strategy():
    ...
    # Test d'estimation d'impact
    impact = strategy.calculate_impact_estimate(
        symbol="BTC/USD", quantity=1.0, side="sell"
    )
    assert impact > 0
...
def test_aggressive_execution_strategy():
    ...
    assert impact > 0
    assert impact > strategy.calculate_impact_estimate(
        symbol="BTC/USD", quantity=0.5, side="buy"
    )
...
def test_adaptive_execution_strategy():
    ...
    with mock.patch.object(strategy, "_analyze_market_conditions") as mock_analyze:
        ...
        result_passive = strategy.execute_order(
        ...
        result_aggressive = strategy.execute_order(
            symbol="BTC/USD", side="buy", quantity=1.0
        )
        ...
        result_normal = strategy.execute_order(
            symbol="BTC/USD", side="buy", quantity=1.0
        )
...
def test_adaptive_executor():
    ...
    result_passive = executor.execute(
        symbol="BTC/USD", side="buy", quantity=1.0, mode=ExecutionMode.PASSIVE
    )
    assert result_passive["strategy"] == "passive"

    result_normal = executor.execute(
        symbol="BTC/USD", side="buy", quantity=1.0, mode=ExecutionMode.NORMAL
    )
    assert result_normal["strategy"] == "normal"

    result_aggressive = executor.execute(
        symbol="BTC/USD", side="buy", quantity=1.0, mode=ExecutionMode.AGGRESSIVE
    )
    assert result_aggressive["strategy"] == "aggressive"

    # Test de la fonction d'estimation d'impact
    impact_passive = executor.estimate_impact(
        symbol="BTC/USD", quantity=1.0, side="buy", mode=ExecutionMode.PASSIVE
    )

    impact_aggressive = executor.estimate_impact(
        symbol="BTC/USD", quantity=1.0, side="buy", mode=ExecutionMode.AGGRESSIVE
    )
...
def test_suggest_execution_mode():
    ...
    with mock.patch.object(
        executor, "analyze_market_volatility"
    ) as mock_volatility, mock.patch.object(
        executor, "analyze_market_liquidity"
    ) as mock_liquidity:
        ...
        mode = executor.suggest_execution_mode(
            symbol="BTC/USD", quantity=1.0, side="buy", urgency=0.1  # Faible urgence
        )
        ...
        mode = executor.suggest_execution_mode(
            symbol="BTC/USD", quantity=1.0, side="buy", urgency=0.9  # Haute urgence
        )
        ...
        mode = executor.suggest_execution_mode(
            symbol="BTC/USD", quantity=1.0, side="buy", urgency=0.5  # Urgence moyenne
        )
