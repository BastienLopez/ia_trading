...
class TestOptimizedDataset(unittest.TestCase):
    ...
    def test_precompute_features(self):
        ...
        loaded_features = dataset.load_cached_data(features_name, format="numpy", tensor_dtype=torch.float32)
    ...
    def test_compress_raw_data(self):
        ...
        self.assertLess(compressed_path.stat().st_size, Path(self.csv_path).stat().st_size)
