...
from ai_trading.llm.sentiment_analysis import NewsAnalyzer, EnhancedNewsAnalyzer
...
class TestVisualizationPaths(unittest.TestCase):
    ...
    def setUp(self):
        ...
        for i, date in enumerate(dates):
            self.test_data.append({
                ...
                "global_sentiment": {"label": "positive" if sentiment_scores[i] > 0.5 else "negative", "score": sentiment_scores[i]}
            })
        
        self.df = pd.DataFrame(self.test_data)
        
        # Chemin vers le dossier de visualisations
        self.visualization_base_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'visualizations')
        self.sentiment_visualization_dir = os.path.join(self.visualization_base_dir, 'sentiment')
        self.examples_visualization_dir = os.path.join(self.visualization_base_dir, 'examples')
    ...
    def test_news_analyzer_visualization_path(self):
        ...
        self.assertTrue(os.path.exists(expected_path), f"Le fichier {expected_path} n'existe pas")
    ...
    def test_custom_visualization_path(self):
        ...
        # Vérifier que le fichier existe
        self.assertTrue(os.path.exists(output_path), f"Le fichier {output_path} n'existe pas")
    ...
    def test_no_visualization_in_root(self):
        ...
        root_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), filename)
        
        # Vérifier que le fichier n'existe pas à la racine
        self.assertFalse(os.path.exists(root_path), f"Le fichier {root_path} existe à la racine du projet")
