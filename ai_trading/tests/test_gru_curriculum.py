...
from ai_trading.rl.curriculum_learning import GRUCurriculumLearning, GRUCurriculumTrainer
...
class TestGRUCurriculumLearning(unittest.TestCase):
    ...
    def test_filter_data_by_volatility(self):
        ...
        self.assertGreaterEqual(len(filtered_data_low), 3 * self.curriculum.sequence_length)
        self.assertGreaterEqual(len(filtered_data_high), 3 * self.curriculum.sequence_length)
        ...
        self.assertEqual(len(filtered_data_max), len(self.df) - 20)  # -20 car window=20 par défaut
    ...
    def test_update_difficulty(self):
        ...
        self.assertEqual(self.curriculum.difficulty, 0.2)  # La difficulté devrait rester inchangée
        ...
        for i, perf in enumerate(high_performances):
            ...
            if i == len(high_performances) - 1:
                self.assertTrue(result)  # La difficulté devrait augmenter après la 3ème performance
            else:
