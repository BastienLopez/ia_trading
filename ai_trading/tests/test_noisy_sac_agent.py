...
class TestNoisySACAgent(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.actions = np.random.normal(0, 1, (100, self.action_size)).astype(np.float32)
        self.rewards = np.random.normal(0, 1, (100, 1)).astype(np.float32)
        self.next_states = np.random.normal(0, 1, (100, self.state_size)).astype(np.float32)
    ...
    def test_act_deterministic_vs_stochastic(self):
        ...
        deterministic_actions = [self.agent.act(test_state, deterministic=True) for _ in range(10)]
        stochastic_actions = [self.agent.act(test_state, deterministic=False) for _ in range(10)]
    ...
    def test_target_network_update(self):
        ...
        critic1_target_weights_after = [w.numpy() for w in self.agent.critic_1_target.weights]
        critic2_target_weights_after = [w.numpy() for w in self.agent.critic_2_target.weights]
        
        # Vérifier que les poids ont changé
        weights_changed = False
        for before, after in zip(critic1_target_weights_before, critic1_target_weights_after):
            if not np.allclose(before, after):
                weights_changed = True
                break
        
        if not weights_changed:
            for before, after in zip(critic2_target_weights_before, critic2_target_weights_after):
                if not np.allclose(before, after):
                    weights_changed = True
                    break
        
        self.assertTrue(weights_changed, "Les poids des réseaux cibles n'ont pas été mis à jour")
    ...
    def test_action_scaling(self):
        ...
        test_state = np.random.normal(0, 1, self.state_size)
        raw_action = self.agent.actor(np.array([test_state]))[0].numpy()[0]  # Prendre la moyenne
