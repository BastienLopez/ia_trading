...
class TestDataIntegration(unittest.TestCase):
    ...
    def test_preprocess_market_data(self):
        ...
        preprocessed_data = self.integrator.preprocess_market_data(self.market_data)
        
        # Vérifier que les données prétraitées ont les bonnes colonnes
        expected_columns = ['open', 'high', 'low', 'close', 'volume', 
                           'returns', 'ma_5', 'ma_10', 'rsi_14', 'bb_upper', 'bb_lower']
        
        for col in expected_columns:
            self.assertIn(col, preprocessed_data.columns)
        
        # Vérifier que les données sont normalisées
        for col in ['open', 'high', 'low', 'close']:
            self.assertTrue(0 <= preprocessed_data[col].min() <= preprocessed_data[col].max() <= 1)
    
    def test_integrate_data(self):
        """Teste l'intégration des données de marché et de sentiment."""
        # Prétraiter les données de marché
        preprocessed_market_data = self.integrator.preprocess_market_data(self.market_data)
        ...
        self.assertIn('compound_score', train_data.columns)
    ...
    def test_visualize_integrated_data(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.integrator.visualize_integrated_data(
                market_data=preprocessed_market_data,
                sentiment_data=self.sentiment_data,
                save_path=temp_dir
            )
            ...
            expected_files = ['price_vs_sentiment.png', 'correlation_heatmap.png']
