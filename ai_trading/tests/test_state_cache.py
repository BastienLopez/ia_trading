...
class TestStateCache(unittest.TestCase):
    ...
    def test_states_similar(self):
        ...
        # États identiques
        self.assertTrue(self.cache._states_similar(self.test_states[0], self.test_states[0]))
        
        # États similaires (en dessous du seuil)
        self.assertTrue(self.cache._states_similar(self.test_states[0], self.test_states[1]))
        
        # États différents
        self.assertFalse(self.cache._states_similar(self.test_states[0], self.test_states[2]))
        
        # Types différents
        self.assertFalse(self.cache._states_similar(self.test_states[0], self.test_states[3]))
        self.assertFalse(self.cache._states_similar(self.test_states[0], self.test_states[4]))
...
class TestMultiLevelCache(unittest.TestCase):
    ...
    def test_level_selection(self):
        ...
        self.assertEqual(self.multi_cache._select_level(self.test_states["frequent"]), "frequent")
        self.assertEqual(self.multi_cache._select_level(self.test_states["rare"]), "rare")
        self.assertEqual(self.multi_cache._select_level(self.test_states["precise"]), "precise")
    ...
    def test_get_metrics_multi_level(self):
        ...
        # Faire des requêtes
        for level, state in self.test_states.items():
            for _ in range(level.count('e')):  # 2 hits pour frequent, 1 pour rare, 1 pour precise
