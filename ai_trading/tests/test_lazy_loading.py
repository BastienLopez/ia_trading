...
class TestLazyFileReader(unittest.TestCase):
    ...
    def test_get_rows(self):
        ...
        self.assertAlmostEqual(rows.iloc[0]['close'], self.df.iloc[150]['close'], places=6)
        self.assertAlmostEqual(rows.iloc[99]['close'], self.df.iloc[249]['close'], places=6)
    ...
    @unittest.skipIf(not os.path.exists(os.path.expandvars("${TEMP}/test_data.parquet")), "Test parquet ignoré si pyarrow n'est pas installé")
...
class TestLazyDataset(unittest.TestCase):
    ...
    def setUp(self):
        ...
        np.random.seed(42)
        self.df = pd.DataFrame({
            'close': np.random.randn(1000).cumsum(),  # Prix cumulés pour simuler une série temporelle
            'open': np.random.randn(1000).cumsum(),
            'high': np.random.randn(1000).cumsum() + 1.0,  # Toujours plus haut que close
            'low': np.random.randn(1000).cumsum() - 1.0,   # Toujours plus bas que close
            'volume': np.abs(np.random.randn(1000)) * 1000
        })
