...
class TestCompleteAllocationSystem:
    ...
    def test_initialization(self, allocation_system, returns_data):
        ...
        assert allocation_system.market_regime == "normal"
    ...
    def test_detect_market_regime(self, allocation_system, returns_data):
        ...
        assert regime in ["normal", "high_vol", "bearish", "bullish"]
    ...
    def test_optimize_allocation(self, allocation_system):
        ...
        for asset, weight in result.weights.items():
            assert weight >= allocation_system.min_single_asset_weight
            assert weight <= allocation_system.max_single_asset_weight
    ...
    def test_optimize_different_regimes(self, allocation_system):
        ...
        regimes = ["normal", "high_vol", "bearish", "bullish"]
        ...
        for regime in regimes:
            ...
            # Les régimes plus risqués devraient avoir des poids plus concentrés
            if regime == "high_vol" or regime == "bearish":
                # Vérifier que le nombre d'actifs avec poids significatif est plus faible
                significant_weights = sum(
                    1 for w in result.weights.values() if w > 0.05
                )
                assert significant_weights <= len(allocation_system.assets)
    ...
    def test_rebalance_need(self, allocation_system):
        ...
        identical_weights = {asset: weight for asset, weight in result.weights.items()}
        assert not allocation_system.should_rebalance(identical_weights)

        # Cas où un actif dévie significativement
        deviated_weights = identical_weights.copy()
        assets = list(deviated_weights.keys())
        # Augmenter un poids de 10%
        deviated_weights[assets[0]] += 0.10
        # Réduire un autre poids pour compenser
        deviated_weights[assets[1]] -= 0.10

        assert allocation_system.should_rebalance(deviated_weights)
    ...
    def test_get_rebalance_plan(self, allocation_system):
        ...
        current_weights = {asset: weight for asset, weight in result.weights.items()}
        ...
        # Vérifier que les ajustements compensent les déviations
        assert abs(adjustments[assets[0]]) > 0
        assert abs(adjustments[assets[1]]) > 0
        assert adjustments[assets[0]] < 0  # Devrait réduire le poids du premier actif
        assert (
            adjustments[assets[1]] > 0
        )  # Devrait augmenter le poids du deuxième actif

        # Vérifier que la somme des ajustements est proche de zéro
        assert abs(sum(adjustments.values())) < 1.0
    ...
    def test_integrate_signals(self, allocation_system):
        ...
        # Vérifier que les poids reflètent les signaux
        assert (
            result.weights[assets[0]] > result.weights[assets[1]]
        )  # Plus de poids sur le signal positif
        assert (
            result.weights[assets[0]] > result.weights[assets[4]]
        )  # Plus de poids sur le signal positif
        assert (
            result.weights[assets[2]] > result.weights[assets[4]]
        )  # Plus de poids sur le signal positif que négatif
    ...
    def test_stress_test(self, allocation_system):
        ...
        # Effectuer un stress test de type crash de marché
        stress_result = allocation_system.stress_test(scenario="market_crash")

        # Vérifier les résultats
        assert "total_return" in stress_result
        assert "max_drawdown" in stress_result
        assert "volatility" in stress_result
        assert "var_95" in stress_result
        assert "es_95" in stress_result
        assert "portfolio_returns" in stress_result

        # Le crash devrait produire un rendement négatif
        assert stress_result["total_return"] < 0
