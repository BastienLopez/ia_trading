...
class TestOrderFlow(unittest.TestCase):
    ...
    def test_order_flow_collection(self):
        ...
        self.assertTrue(
            (df["buy_sell_ratio"] >= 0).all() & (df["buy_sell_ratio"] <= 1).all()
        )
    ...
    def test_order_flow_metrics(self):
        ...
        self.assertGreaterEqual(
            len(metrics_df.columns), len(df.columns) + len(new_columns) - 1
        )
    ...
    def test_data_integration(self):
        ...
        integrated_data = integrate_order_flow_and_market_data(
            market_data=market_data, order_flow_data=order_flow_data
        )
