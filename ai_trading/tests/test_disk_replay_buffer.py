...
class TestDiskReplayBuffer(unittest.TestCase):
    ...
    def test_n_step_returns(self):
        ...
        self.assertGreaterEqual(
            len(n_step_buffer), 1, "Le buffer devrait contenir au moins une entrée"
        )
        ...
        try:
            ...
            n_step_buffer._flush_cache(force=True)
            self.assertGreater(
                len(n_step_buffer), 0, "Le buffer devrait contenir des données"
            )
    ...
    def test_performance_metrics(self):
        ...
        )  # Peut être 0 si l'échantillonnage a échoué
