...
class TestNStepSACAgent(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.actions = np.random.normal(0, 0.5, (50, self.action_size)).astype(
            np.float32
        )
        self.rewards = np.random.normal(0, 1, 50).astype(np.float32)
        self.next_states = np.random.normal(0, 1, (50, self.state_size)).astype(
            np.float32
        )
    ...
    def test_initialization(self):
        ...
        self.assertAlmostEqual(
            self.agent.n_step_discount_factor, self.discount_factor**self.n_steps
        )
    ...
    def test_n_step_return_calculation_through_remember(self):
        ...
        for i in range(self.n_steps + 1):
            self.agent.remember(
                self.states[i], self.actions[i], rewards[i], self.next_states[i], False
            )
        ...
        states, actions, rewards_sampled, next_states, dones = (
            self.agent.replay_buffer.sample(2)
        )
        ...
        expected_reward_1 = (
            rewards[0]
            + self.discount_factor * rewards[1]
            + (self.discount_factor**2) * rewards[2]
        )
        ...
        expected_reward_2 = (
            rewards[1]
            + self.discount_factor * rewards[2]
            + (self.discount_factor**2) * rewards[3]
        )
        ...
        self.assertAlmostEqual(
            sorted_rewards[0], min(expected_reward_1, expected_reward_2), places=2
        )
        self.assertAlmostEqual(
            sorted_rewards[1], max(expected_reward_1, expected_reward_2), places=2
        )
    ...
    def test_save_load(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            self.assertAlmostEqual(
                self.agent.n_step_discount_factor, original_discount_factor
            )
    ...
    def test_n_step_factor_in_train_step(self):
        ...
        self.assertAlmostEqual(
            original_n_step_discount_factor, original_discount_factor**self.n_steps
        )
