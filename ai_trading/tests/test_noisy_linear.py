...
class TestNoisyLinear(unittest.TestCase):
    ...
    def test_initialization(self):
        ...
        self.assertEqual(
            self.layer.weight_mu.shape, (self.out_features, self.in_features)
        )
        self.assertEqual(self.layer.bias_mu.shape, (self.out_features,))
        self.assertEqual(
            self.layer.weight_sigma.shape, (self.out_features, self.in_features)
        )
        ...
        bound_sigma = (
            self.sigma_init / np.sqrt(self.in_features) / np.sqrt(self.out_features)
        )
        self.assertTrue(
            torch.allclose(
                self.layer.weight_sigma,
                torch.full_like(self.layer.weight_sigma, bound_sigma),
            )
        )
        ...
        self.assertTrue(
            torch.allclose(
                self.layer.bias_sigma,
                torch.full_like(self.layer.bias_sigma, bound_sigma_bias),
            )
        )
    ...
    def test_backpropagation(self):
        ...
        optimizer = torch.optim.SGD([{"params": self.layer.parameters()}], lr=0.01)
