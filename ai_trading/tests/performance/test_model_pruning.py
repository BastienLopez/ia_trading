...
def test_apply_global_unstructured_pruning(model):
    ...
    pruned_model = apply_global_unstructured_pruning(
        model, pruning_method=prune.L1Unstructured, amount=pruning_amount
    )
...
def test_apply_layerwise_pruning(model):
    ...
    pruning_amount = 0.4
    pruned_model = apply_layerwise_pruning(model, method="l1", amount=pruning_amount)
...
def test_iterative_pruning(model):
    ...
    assert (
        sparsity_stats["global_sparsity"] > 0.1
    ), "La sparsité finale devrait être significative"

    # Nous notons l'écart pour référence dans les logs
    logger.info(
        f"Sparsité obtenue: {sparsity_stats['global_sparsity']:.4f}, attendue théoriquement: {expected_sparsity:.4f}"
    )
    logger.info(
        f"Écart de sparsité: {abs(sparsity_stats['global_sparsity'] - expected_sparsity):.4f}"
    )
...
def test_sensitivity_analysis(model):
    ...
    sensitivity_results = sensitivity_analysis(
        model, eval_fn=mock_eval_fn, prune_amounts=prune_amounts, method="l1"
    )
...
def test_pruning_scheduler(model):
    ...
    assert (
        abs(final_sparsity - 0.5) < 0.2
    ), f"Sparsité finale inattendue: {final_sparsity}"
...
def test_pruning_with_exclude_layers(model):
    ...
    pruned_model = apply_layerwise_pruning(
        model, method="l1", amount=pruning_amount, exclude_layers=exclude_layers
    )
