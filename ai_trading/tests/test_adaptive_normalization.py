...
class TestAdaptiveNormalization(unittest.TestCase):
    ...
    def test_normalize_features_minmax(self):
        ...
        pd.testing.assert_series_equal(
            normalized_df["non_numeric"], self.data["non_numeric"]
        )
    ...
    def test_normalize_features_adaptive(self):
        ...
        normalized_df = self.normalizer.normalize_features(
            self.data, method="adaptive", window_size=20
        )
    ...
    def test_normalize_features_with_missing_values(self):
        ...
        normalized_df = self.normalizer.normalize_features(
            data_with_missing, method="adaptive"
        )
    ...
    def test_normalize_features_with_constant_values(self):
        ...
        constant_data = pd.DataFrame({"constant1": [5] * 100, "constant2": [10] * 100})

        # Normaliser les features
        normalized_df = self.normalizer.normalize_features(
            constant_data, method="adaptive"
        )
