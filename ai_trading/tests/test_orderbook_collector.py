...
class TestOrderBookCollector(unittest.TestCase):
    ...
    def test_orderbook_features_with_pressure(self):
        ...
        features_buy = self.collector.calculate_orderbook_features(
            self.mock_orderbook_buy_pressure
        )
        self.assertGreater(features_buy["volume_imbalance"], 0)

        # Test avec pression de vente
        features_sell = self.collector.calculate_orderbook_features(
            self.mock_orderbook_sell_pressure
        )
    ...
    def test_slippage_with_market_pressure(self):
        ...
        self.assertGreater(
            slippage_buy_pressure,
            self.collector.calculate_slippage(
                self.mock_orderbook_buy_pressure, "sell", 10.0
            ),
        )
        self.assertGreater(
            slippage_sell_pressure,
            self.collector.calculate_slippage(
                self.mock_orderbook_sell_pressure, "buy", 10.0
            ),
        )
    ...
    def test_market_impact_scenarios(self):
        ...
        small_impact = self.collector.estimate_market_impact(
            self.mock_orderbook, "buy", 0.1
        )

        # Impact avec gros ordre
        large_impact = self.collector.estimate_market_impact(
            self.mock_orderbook, "buy", 10.0
        )

        # L'impact devrait être plus important pour les gros ordres
        self.assertGreater(
            large_impact["immediate_impact_pct"], small_impact["immediate_impact_pct"]
        )
        self.assertGreater(
            large_impact["estimated_recovery_time"],
            small_impact["estimated_recovery_time"],
        )
    ...
    def test_execution_delay_edge_cases(self):
        ...
        large_volume_delay = self.collector.simulate_execution_delay(
            1000.0, current_load=0.5
        )  # Volume augmenté à 1000

        # Vérifications
        self.assertGreater(
            max_delay,
            min_delay,
            "Le délai maximal devrait être plus grand que le délai minimal",
        )
        self.assertGreater(
            large_volume_delay,
            max_delay,
            "Le délai pour un gros volume devrait être plus grand que le délai maximal",
        )
    ...
    def test_get_vwap_levels(self):
        ...
        vwap_bid, vwap_ask = self.collector.get_vwap_levels(
            self.mock_orderbook, volume_threshold
        )
    ...
    def test_collect_orderbook_data(self, mock_binance, mock_sleep):
        ...
        data_dict = collector.collect_orderbook_data(
            duration_minutes=0.1, interval_seconds=1
        )
    ...
    def test_calculate_slippage(self):
        ...
        slippage_buy = self.collector.calculate_slippage(
            self.mock_orderbook, "buy", 2.0
        )
        self.assertIsInstance(slippage_buy, float)
        self.assertGreaterEqual(slippage_buy, 0.0)

        # Test avec un ordre de vente
        slippage_sell = self.collector.calculate_slippage(
            self.mock_orderbook, "sell", 2.0
        )
        self.assertIsInstance(slippage_sell, float)
        self.assertGreaterEqual(slippage_sell, 0.0)

        # Test avec un volume nul
        slippage_zero = self.collector.calculate_slippage(
            self.mock_orderbook, "buy", 0.0
        )
        ...
        historical_data = pd.DataFrame(
            {
                "timestamp": pd.date_range(
                    start="2024-01-01", periods=100, freq="1min"
                ),
    ...
    def test_calibration_impact(self):
        ...
        prices_after = prices_before * (
            1 + 0.001 * volumes
        )  # Impact de 0.1% par unité de volume
