...
class SupportResistanceDetector:
    ...
    def calculate_strength(
        self, price: float, df: pd.DataFrame, tolerance: float = 0.002
    ) -> float:
        ...
        for _, row in df.iterrows():
            if (
                abs(row["high"] - price) <= price_range
                or abs(row["low"] - price) <= price_range
            ):
                touches += 1
    ...
    def detect_breaks_retests(
        self, df: pd.DataFrame, level: SRLevel, tolerance: float = 0.002
    ):
