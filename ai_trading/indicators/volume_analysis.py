...
class VolumeAnalyzer:
    ...
    def create_volume_profile(self, 
                            ...
                            profile_type: VolumeProfileType = VolumeProfileType.PRICE) -> Dict:
        """
        ...
        if profile_type == VolumeProfileType.PRICE:
            ...
            result = {
                ...
                'max_volume_price': (price_bins[np.argmax(volume_by_price)] + price_bins[np.argmax(volume_by_price)+1])/2,
    ...
    def find_control_points(self, 
                         ...
                         min_volume_percentile: float = 0.85) -> List[Dict]:
        """
        ...
        for i in range(lookback_periods, len(self.data)):
            ...
            for j, volume in enumerate(volumes):
                if volume >= volume_threshold:
                    ...
                    if not any(abs(cp['price'] - price_level) / price_level < 0.005 for cp in control_points):
    ...
    def detect_volume_anomalies(self, 
        ...
        self.data['volume_zscore'] = (self.data['volume'] - self.data['volume_ma']) / self.data['volume_std']
        ...
        anomalies['price_change_pct'] = (anomalies['close'] - anomalies['open']) / anomalies['open'] * 100
        
        # Calculer un score d'importance
        anomalies['importance'] = abs(anomalies['volume_zscore']) * abs(anomalies['price_change_pct'])
        
        # Déterminer si c'est un volume de pression acheteur ou vendeur
        anomalies['buying_pressure'] = np.where(
            anomalies['close'] > anomalies['open'],
            True,
            False
        )
    ...
    def validate_signal(self, 
        ...
        volume_price_corr = signal_data['volume'].corr(signal_data['close'] - signal_data['open'])
        ...
        is_confirming = ((volume_ratio > 1.2) and  # Volume significativement plus élevé
                        ((price_direction > 0 and volume_price_corr > 0.3) or  # Hausse du prix, corr positive
                         (price_direction < 0 and volume_price_corr < -0.3)))  # Baisse du prix, corr négative
        ...
        return {
            ...
            'recommendation': 'buy' if is_confirming and price_direction > 0 else
                              'sell' if is_confirming and price_direction < 0 else
                              'neutral'
        }
    ...
    def plot_volume_profile(self, 
        ...
        if profile_type == VolumeProfileType.PRICE:
            if horizontal:
                ...
                ax.text(profile['volumes'][max_vol_idx] * 0.5, max_vol_price, 
                        f'POC: {max_vol_price:.2f}', verticalalignment='bottom')
            else:
                ...
                ax.text(max_vol_price, profile['volumes'][max_vol_idx] * 0.5, 
                        f'POC: {max_vol_price:.2f}', horizontalalignment='left')
            ...
            ax.plot(profile['timestamps'], profile['upper_band_1'], '--', color='red', alpha=0.5, label='+1σ')
            ax.plot(profile['timestamps'], profile['lower_band_1'], '--', color='green', alpha=0.5, label='-1σ')
            ax.plot(profile['timestamps'], profile['upper_band_2'], ':', color='red', alpha=0.3, label='+2σ')
            ax.plot(profile['timestamps'], profile['lower_band_2'], ':', color='green', alpha=0.3, label='-2σ')
            
            # Tracer le prix de clôture
            ax.plot(period_data.index, period_data['close'], color='black', alpha=0.7, label='Close')
...
def accelerating_volume(ohlcv_data: pd.DataFrame, window: int = 5) -> pd.DataFrame:
    ...
    df['volume_acceleration_ma'] = df['volume_acceleration'].rolling(window=window).mean()
