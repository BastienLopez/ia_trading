...
class SpectralAnalyzer:
    ...
    def find_dominant_cycles(self, data: np.ndarray, n_cycles: int = 3) -> List[float]:
        ...
        sorted_peaks = sorted(
            zip(peak_indices, psd[peak_indices]), key=lambda x: x[1], reverse=True
        )
    ...
    def filter_by_frequency(
        ...
        if low_cutoff is None and high_cutoff is not None:
            ...
            b, a = signal.butter(
                4,
                [
                    low_cutoff / (self.sampling_freq / 2),
                    high_cutoff / (self.sampling_freq / 2),
                ],
                btype="band",
            )
        else:
...
class MultiTimeframeIndicator:
    ...
    def add_data(self, timeframe: TimeFrame, data: pd.DataFrame):
        ...
        if timeframe not in self.timeframes:
            raise ValueError(
                f"Le timeframe {timeframe} n'est pas dans la liste des timeframes configurés"
            )
    ...
    ) -> Dict[TimeFrame, Dict[str, pd.Series]]:
        """
...
class DivergenceDetector:
    ...
    def __init__(
        self, price_data: pd.DataFrame, indicator_data: pd.Series, window_size: int = 10
    ):
        """
        ...
        if "close" not in price_data.columns:
            raise ValueError(
                "Le DataFrame price_data doit contenir une colonne 'close'"
            )
    ...
    def detect_divergences(self) -> Dict[DivergenceType, List[Tuple[int, int]]]:
        ...
        for i in range(len(price_minima) - 1):
            ...
            ind_min1_candidates = [
                idx for idx in ind_minima if abs(idx - idx1) <= self.window_size
            ]
            ind_min2_candidates = [
                idx for idx in ind_minima if abs(idx - idx2) <= self.window_size
            ]
            ...
            if (
                self.price.iloc[idx2] < self.price.iloc[idx1]
                and self.indicator.iloc[ind_idx2] > self.indicator.iloc[ind_idx1]
            ):
        ...
        for i in range(len(price_maxima) - 1):
            ...
            ind_max1_candidates = [
                idx for idx in ind_maxima if abs(idx - idx1) <= self.window_size
            ]
            ind_max2_candidates = [
                idx for idx in ind_maxima if abs(idx - idx2) <= self.window_size
            ]
