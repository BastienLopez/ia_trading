...
def reliability_score(pattern: PatternDetection, 
                    volume_confirmation: bool = False,
                    momentum_confirmation: bool = False,
                    timeframe_alignment: bool = False) -> float:
    """
...
class ClassicalPatternDetector:
    ...
    def _find_peaks_and_troughs(self, data: pd.Series, 
                              window_size: int = 5) -> Tuple[List[int], List[int]]:
        """
    ...
    def detect_double_top(self, data: pd.DataFrame, 
                         tolerance: float = 0.02) -> List[PatternDetection]:
        """
        ...
        for i in range(len(peaks) - 1):
            ...
            pattern = PatternDetection(
                ...
                price_target=data['close'].iloc[valley_idx] - valley_depth,  # Projection vers le bas
    ...
    def detect_patterns(self, data: pd.DataFrame) -> Dict[PatternType, List[PatternDetection]]:
        """
        ...
        patterns = {
            PatternType.DOUBLE_TOP: self.detect_double_top(data)
        }
