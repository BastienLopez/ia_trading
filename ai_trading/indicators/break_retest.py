...
class BreakRetestDetector:
    ...
    def is_valid_break(self, 
        ...
        if candle['volume'] < avg_volume * self.volume_threshold:
            print(f"Volume insuffisant: {candle['volume']} < {avg_volume * self.volume_threshold}")
            return False, ""
            
        # Cassure haussière
        if (prev_candle['close'] < level and 
            candle['close'] > level + tolerance):
            print(f"Cassure haussière détectée: prev_close={prev_candle['close']}, close={candle['close']}, level={level}")
            return True, "up"
            
        # Cassure baissière
        if (prev_candle['close'] > level and 
            candle['close'] < level - tolerance):
            print(f"Cassure baissière détectée: prev_close={prev_candle['close']}, close={candle['close']}, level={level}")
            return True, "down"
            
        print(f"Pas de cassure: prev_close={prev_candle['close']}, close={candle['close']}, level={level}, tolerance={tolerance}")
        return False, ""
        
    def calculate_break_strength(self, 
                               candle: pd.Series, 
                               level: float,
                               avg_volume: float,
        ...
        volume_factor = min(candle['volume'] / (avg_volume * self.volume_threshold), 2.0)
    ...
    def detect_break(self, 
                    df: pd.DataFrame, 
                    level: float) -> Optional[BreakEvent]:
        ...
        for i in range(20, len(df)):  # Commence après la période de calcul des moyennes
            ...
            if ((prev_candle['close'] <= level and candle['close'] > level) or  # Cassure haussière
                (prev_candle['close'] >= level and candle['close'] < level)):   # Cassure baissière
    ...
    def is_valid_retest(self, 
        ...
        if break_direction == "up":
            return (candle['low'] >= level - tolerance and 
                    candle['close'] > level)
        else:
            return (candle['high'] <= level + tolerance and 
                    candle['close'] < level)
    
    def calculate_retest_strength(self, 
                                candle: pd.Series, 
                                level: float,
                                volatility: float) -> float:
    ...
    def detect_retest(self, 
                     df: pd.DataFrame, 
                     level: float,
                     break_event: BreakEvent) -> Optional[RetestEvent]:
        ...
        for i, (idx, candle) in enumerate(post_break_df.iterrows()):
            ...
            print(f"Prix: close={candle['close']}, high={candle['high']}, low={candle['low']}")
