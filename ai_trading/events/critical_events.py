...
class CriticalEventDetector:
    ...
    def detect_regulatory_events(self, 
                               news_data: List[Dict],
        ...
        logger.info(f"Détection d'événements réglementaires: {len(regulatory_events)} trouvés")
        return regulatory_events
    
    def detect_fundamental_events(self, 
                                news_data: List[Dict],
        ...
        for news in news_data:
            ...
            # Normaliser le score
            max_possible_score = sum(w * 1.5 for w in self.fundamental_keywords.values())
            normalized_score = score / max_possible_score if max_possible_score > 0 else 0
        ...
        logger.info(f"Détection d'événements fondamentaux: {len(fundamental_events)} trouvés")
        return fundamental_events
    
    def detect_narrative_changes(self, 
                               news_data: List[Dict],
        ...
        for pattern, mentions in pattern_mentions.items():
            if len(mentions) >= min_mentions:
                ...
                if (datetime.now().date() - last_date).days <= timeframe_days:
                    ...
                    diversity_score = min(len(sources) / 3, 1.0)  # Normaliser à 3 sources différentes
                    ...
                    event = {
                        ...
                        'example_news': all_news_with_pattern[0] if all_news_with_pattern else None,
                        'detected_at': datetime.now().isoformat()
                    }
        ...
        logger.info(f"Détection de changements de narratif: {len(narrative_events)} trouvés")
    ...
    def analyze_all_events(self, 
                         ...
                         narrative_min_mentions: int = 3) -> Dict[str, List[Dict]]:
        """
        ...
        narrative = self.detect_narrative_changes(news_data, min_mentions=narrative_min_mentions)
    ...
    def _extract_entities(self, text: str) -> List[str]:
        ...
        common_words = {'le', 'la', 'les', 'des', 'du', 'au', 'aux', 'un', 'une', 'et', 'ou', 'a'}
        filtered_entities = [e for e in entities if len(e) > 2 and e.lower() not in common_words]
            ...
            for past_event in self.detected_events:
                if past_event['type'] == event['type']:
                    ...
                    if self._similarity(event_title, past_title) > 0.8:
                        ...
                        try:
                            past_date = datetime.fromisoformat(past_event.get('detected_at', ''))
                            hours_diff = (datetime.now() - past_date).total_seconds() / 3600
            ...
            for past_event in self.detected_events:
                if past_event['type'] == EventType.NARRATIVE.value:
                    if past_event.get('pattern', '').lower() == event_pattern:
                        # Vérifier si l'événement est récent
                        try:
                            past_latest = datetime.fromisoformat(past_event.get('latest_mention', ''))
                            event_latest = datetime.fromisoformat(event.get('latest_mention', ''))
...
def filter_news_by_relevance(news_data: List[Dict], 
    ...
    return sorted(relevant_news, key=lambda x: x.get('relevance_score', 0), reverse=True) 