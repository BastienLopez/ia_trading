...
def collect_news(coins: list, days: int = 7, limit_per_coin: int = 10) -> pd.DataFrame:
    ...
    """
    logger.info(f"Collecte des actualités pour {len(coins)} cryptomonnaies sur {days} jours")
...
def analyze_news(news_df: pd.DataFrame, use_gpu: bool = False, enable_cache: bool = True) -> pd.DataFrame:
    """
    ...
    if report['sentiment_by_crypto']:
        ...
        for crypto, data in report['sentiment_by_crypto'].items():
            logger.info(f"- {crypto}: {data['sentiment_label']} ({data['average_sentiment']:.2f}) - {data['article_count']} articles")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Analyse de sentiment des actualités crypto")
    parser.add_argument("--coins", type=str, nargs="+", default=["bitcoin", "ethereum", "solana", "cardano", "ripple"],
                        help="Liste des cryptomonnaies à analyser")
    parser.add_argument("--days", type=int, default=7, help="Nombre de jours d'historique")
    parser.add_argument("--limit", type=int, default=10, help="Nombre maximum d'actualités par cryptomonnaie")
    parser.add_argument("--gpu", action="store_true", help="Utiliser le GPU pour l'inférence")
    ...
    enriched_df = analyze_news(news_df, use_gpu=args.gpu, enable_cache=not args.no_cache)
    ...
    logger.info("Les visualisations ont été sauvegardées dans data/sentiment/visualizations/")
