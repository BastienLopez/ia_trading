...
def example_with_default_config():
    ...
    print(f"CPU: {system_info['physical_cpu']} cœurs physiques, {system_info['cpu_count']} threads")
...
def example_with_custom_config():
    ...
    try:
        ...
        print(f"  Variables d'environnement: {len(status['optimizations'].get('environment_variables', {}))}")
        
        if 'system_limits' in status['optimizations']:
            print(f"  Limites système: {status['optimizations']['system_limits']}")
        
        if 'disk_optimization' in status['optimizations'] and 'temp_dir' in status['optimizations']['disk_optimization']:
            print(f"  Répertoire temporaire: {status['optimizations']['disk_optimization']['temp_dir']}")
        
        if 'memory_optimization' in status['optimizations']:
            print(f"  Optimisations mémoire: {status['optimizations']['memory_optimization']}")
