...
def generate_sample_frames(n_frames: int, frame_size: Tuple[int, int, int] = (84, 84, 3)) -> List[np.ndarray]:
    """
...
def test_compression_performance(frames: List[np.ndarray], 
                               compressor: FrameCompressor) -> Tuple[float, float, float]:
    """
...
def test_frame_stack_performance(frames: List[np.ndarray], 
                               wrapper: FrameStackWrapper) -> Tuple[float, List[np.ndarray]]:
    """
...
def plot_compression_results(sizes: List[float], 
                           times: List[float], 
                           labels: List[str],
    ...
    for i, value in enumerate(sizes):
        if i > 0:
            ...
            ax1.text(i, value + 0.1, f"{reduction:.1f}% de réduction", 
    ...
    for i, value in enumerate(times):
        ax2.text(i, value + 0.01, f"{value:.4f}s", 
                ha='center', va='bottom', fontsize=9)
...
def plot_sample_frames(orig_frames: List[np.ndarray], 
    ...
    indices = np.linspace(0, min(len(orig_frames), len(comp_frames))-1, n_samples, dtype=int)
...
def main(args):
    ...
    frames = generate_sample_frames(args.n_frames, (args.frame_size, args.frame_size, 3))
    ...
    basic_size, basic_comp_size, basic_time = test_compression_performance(frames, basic_compressor)
    ...
    opti_size, opti_comp_size, opti_time = test_compression_performance(frames, optimized_compressor)
    ...
    basic_wrapper_time, basic_stacked = test_frame_stack_performance(frames, basic_wrapper)
    ...
    compress_wrapper_time, compress_stacked = test_frame_stack_performance(frames, compress_wrapper)
    ...
    plot_compression_results(sizes, times, labels, "Performance de la compression de frames")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Démonstration de la compression de frames RL")
    parser.add_argument("--n_frames", type=int, default=1000, 
                      help="Nombre de frames à générer")
    parser.add_argument("--frame_size", type=int, default=84, 
                      help="Taille des frames (carré)")
    parser.add_argument("--stack_size", type=int, default=4, 
                      help="Nombre de frames à empiler")
    parser.add_argument("--compression_level", type=int, default=6, 
                      help="Niveau de compression (0-9)")
    parser.add_argument("--grayscale", action="store_true", 
                      help="Convertir en niveaux de gris")
    parser.add_argument("--quantize", action="store_true", 
                      help="Quantifier les valeurs")
    parser.add_argument("--delta", action="store_true", 
                      help="Utiliser l'encodage delta")
