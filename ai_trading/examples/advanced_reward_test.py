...
def parse_args():
    ...
    """
    parser = argparse.ArgumentParser(description='Test de fonctions de récompense avancées pour le trading RL')
    ...
    parser.add_argument('--interval', type=str, default='1d', help='Intervalle des données (1h, 1d, etc.)')
...
def fetch_market_data(symbol, days, interval='1d'):
    ...
    """
    logger.info(f"Récupération des données pour {symbol} sur {days} jours avec intervalle {interval}...")
...
def generate_synthetic_data(symbol, days):
    ...
    df = pd.DataFrame({
        ...
        'fear_greed_classification': np.random.choice(['Extreme Fear', 'Fear', 'Neutral', 'Greed', 'Extreme Greed'], len(prices))
    }, index=date_range[:len(prices)])
    
    # Normaliser les scores pour qu'ils soient cohérents
    for i in range(len(df)):
        total = df.iloc[i]['positive_score'] + df.iloc[i]['negative_score'] + df.iloc[i]['neutral_score']
...
def preprocess_data(df):
    ...
    if len(data) >= 20:  # Vérifier qu'il y a suffisamment de données
        ...
        logger.info(f"Indicateurs techniques ajoutés: {len(data.columns) - len(df.columns)} nouveaux indicateurs")
    else:
        logger.warning(f"Pas assez de données pour calculer les indicateurs techniques ({len(data)} points de données)")
    ...
    logger.info(f"Données prétraitées: {len(data)} entrées avec {data.columns.tolist()[:5]}... et {len(data.columns) - 5} autres colonnes")
    return data

def train_agent_with_reward(data, reward_function, episodes, window_size=20, initial_balance=10000,
                          transaction_fee=0.001, action_type="discrete", n_discrete_actions=5):
    """
    ...
    for episode in range(episodes):
        ...
        if (episode + 1) % max(1, episodes // 10) == 0:
            logger.info(f"Épisode {episode + 1}/{episodes}, Récompense: {total_reward:.2f}, "
...
def compare_reward_functions(data, args):
    ...
    logger.info("-" * 80)
    logger.info(f"{'Fonction':20} | {'Portfolio':12} | {'ROI':8} | {'Sharpe':8} | {'Drawdown':10}")
    logger.info("-" * 80)
    
    for reward_function, result in results.items():
        logger.info(f"{reward_function:20} | ${result['final_portfolio']:<11.2f} | {result['roi']:<7.2f}% | "
...
def visualize_comparison(results, symbol):
    ...
    ax.bar([i - 0.2 for i in range(len(reward_functions))], sharpe_values, width=0.4, color='green', alpha=0.7, label='Sharpe')
    ax2.bar([i + 0.2 for i in range(len(reward_functions))], drawdown_values, width=0.4, color='red', alpha=0.7, label='Drawdown')
