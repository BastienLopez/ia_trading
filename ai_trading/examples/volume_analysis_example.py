...
def main():
    ...
    analyzer.plot_volume_profile(
        start_idx=50, end_idx=150, profile_type=VolumeProfileType.VWAP, ax=ax2
    )
    ...
    control_points = analyzer.find_control_points(
        lookback_periods=30, min_volume_percentile=0.75
    )
    ...
    for cp in control_points[:5]:  # 5 points les plus importants
        ax3.axhline(
            cp["price"], color="r", linestyle="--", alpha=0.3 + 0.5 * cp["importance"]
        )
        ax3.text(
            data.index[0],
            cp["price"],
    ...
    volume_anomalies = analyzer.detect_volume_anomalies(
        window_size=20, threshold_sigma=2.0
    )
    ...
    if len(volume_anomalies) > 0:
        high_vol_anomalies = volume_anomalies[
            volume_anomalies["anomaly_type"] == "high_volume"
        ]
        low_vol_anomalies = volume_anomalies[
            volume_anomalies["anomaly_type"] == "low_volume"
        ]

        if len(high_vol_anomalies) > 0:
            ax4.bar(
                high_vol_anomalies.index,
                high_vol_anomalies["volume"],
                color="red",
        ...
        if len(low_vol_anomalies) > 0:
            ax4.bar(
                low_vol_anomalies.index,
                low_vol_anomalies["volume"],
                color="blue",
                label="Volume anormalement bas",
            )
    ...
    ax6.plot(
        delta_analysis.index,
        delta_analysis["cumulative_delta"],
        label="Delta Cumulatif",
    )
    ax6.plot(
        delta_analysis.index,
        delta_analysis["volume_delta"],
        label="Delta de Volume",
        alpha=0.5,
    )
    ...
    for idx in [40, 80, 120, 160]:
        ...
        print(
            f"  Corrélation volume-prix: {validation['volume_price_correlation']:.2f}"
        )
