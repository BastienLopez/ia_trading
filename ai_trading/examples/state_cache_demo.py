...
from rl.state_cache import StateCache, MultiLevelCache
    ...
    def next_state(self):
        ...
        if self.distribution == "uniform":
            ...
            if self.zipf_counter % 10 == 0:  # 10% d'états aléatoires
                ...
                index = min(x - 1, self.n_common_states - 1)  # Ajuster à l'indice 0-based
            ...
            if torch.norm(self.current_state) > 5.0:
                self.current_state = self.current_state * 5.0 / torch.norm(self.current_state)
...
def benchmark_with_cache(model, state_generator, n_samples=1000, device="cpu", 
                         cache_config=None, multi_level=False):
    """
    ...
    if multi_level:
        ...
        levels = {
            "frequent": {"capacity": cache_config["capacity"], "similarity_threshold": 0.01},
            "rare": {"capacity": cache_config["capacity"] // 2, "similarity_threshold": 0.001}
        }
    ...
    if cache_config["enable_disk_cache"] and hasattr(cache, 'cache_dir') and cache.cache_dir:
...
def compare_cache_configs(model, state_generator, n_samples=1000, device="cpu"):
    ...
    for config_info in configs:
        ...
        cache_results = benchmark_with_cache(
            model, state_generator, n_samples, device, 
            cache_config=config, multi_level=multi_level
        )
...
def plot_distribution_comparison(results, title="Comparaison des distributions d'états"):
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument("--n-samples", type=int, default=1000, help="Nombre d'échantillons pour les benchmarks")
    
    # Différentes démos
    parser.add_argument("--basic-demo", action="store_true", help="Démonstration basique du cache")
    parser.add_argument("--distributions", action="store_true", help="Comparaison des distributions d'états")
    parser.add_argument("--configs", action="store_true", help="Comparaison des configurations de cache")
    parser.add_argument("--multi-level", action="store_true", help="Démonstration du cache multi-niveaux")
