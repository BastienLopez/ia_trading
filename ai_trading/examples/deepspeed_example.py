...
from ai_trading.utils.deepspeed_optimizer import optimize_model_with_deepspeed
...
class LargeRLModel(nn.Module):
    ...
    def __init__(
        self, input_dim=128, hidden_dims=[512, 1024, 2048, 1024, 512], output_dim=64
    ):
        super().__init__()
    ...
    for epoch in range(epochs):
        ...
        for i in range(len(inputs)):
            ...
            loss = ds_optimizer.train_step(
                batch=batch_inputs, labels=(batch_actions, batch_returns)
            )
        ...
        print(
            f"Epoch {epoch+1}/{epochs} | Loss: {epoch_loss/len(inputs):.4f} | Time: {epoch_time:.2f}s"
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Exemple d'utilisation de DeepSpeed pour l'optimisation mémoire"
    )
    parser.add_argument("--input_dim", type=int, default=128, help="Dimension d'entrée")
    parser.add_argument(
        "--hidden_dim", type=int, default=2048, help="Dimension des couches cachées"
    )
    parser.add_argument(
        "--num_layers", type=int, default=5, help="Nombre de couches cachées"
    )
    parser.add_argument(
        "--output_dim", type=int, default=64, help="Dimension de sortie"
    )
    ...
    parser.add_argument(
        "--zero_stage", type=int, default=2, help="Stage ZeRO (0, 1, 2, ou 3)"
    )
    parser.add_argument(
        "--fp16", action="store_true", help="Utiliser la précision mixte (FP16)"
    )
    parser.add_argument(
        "--offload_optimizer",
        action="store_true",
        help="Décharger l'optimiseur sur CPU",
    )
    parser.add_argument(
        "--offload_parameters",
        action="store_true",
        help="Décharger les paramètres sur CPU",
    )
    parser.add_argument(
        "--run_baseline",
        action="store_true",
        help="Exécuter l'entraînement sans DeepSpeed pour comparaison",
    )
