    ...
    try:
        ...
        for symbol, df in data.items():
            if not df.empty:
                ...
                print(f"Volume total moyen: {(df['total_bid_volume'].mean() + df['total_ask_volume'].mean()):.4f}")
    ...
    try:
        ...
        for volume in volumes:
            try:
                ...
                print(f"{volume:6.1f} | {metrics['slippage_pct']:11.2f} | "
...
def exemple_calibration():
    ...
    try:
        ...
        historical_data['price_after'] = historical_data['price_before'] * (1 + 0.0001 * historical_data['volume'])
...
def exemple_complet():
    ...
    try:
        ...
        if orderbook:
            ...
            # 4. Calcul VWAP
            vwap_bid, vwap_ask = collector.get_vwap_levels(orderbook, volume_threshold=1.0)
