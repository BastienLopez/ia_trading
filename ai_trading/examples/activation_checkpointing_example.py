...
class DeepTradingModel(nn.Module):
    ...
    def __init__(
        self, input_dim=64, hidden_dim=256, num_blocks=8, use_checkpointing=False
    ):
        """
...
def benchmark_training_memory(
    ...
    memory_with = analyze_memory_usage(
        model_with_checkpointing,
    ...
    memory_without = analyze_memory_usage(
        model_without_checkpointing,
        sample_batch.shape[1:],
        batch_size=train_loader.batch_size,
    )
    ...
    for epoch in range(num_epochs):
        ...
        train_loss = train_epoch(
            model_with_checkpointing, train_loader, optimizer_with, criterion, device
        )
        val_loss, accuracy = validate(
            model_with_checkpointing, val_loader, criterion, device
        )
        ...
        if torch.cuda.is_available():
            ...
            logger.info(
                f"Époque {epoch+1}/{num_epochs} - Loss: {train_loss:.4f}, Val Loss: {val_loss:.4f}, "
    ...
    try:
        ...
        for epoch in range(num_epochs):
            ...
            val_loss, accuracy = validate(
                model_without_checkpointing, val_loader, criterion, device
            )
        ...
        if torch.cuda.is_available() and peak_memory_without and peak_memory_with:
            memory_reduction = (
                (max(peak_memory_without) - max(peak_memory_with))
                / max(peak_memory_without)
                * 100
            )
        ...
        if "CUDA out of memory" in str(e):
            logger.warning(
                "OOM ERROR: Entraînement sans checkpointing impossible - CUDA out of memory!"
            )
            oom_without = True
            ...
            time_without = None
            memory_reduction = (
                100  # Considéré comme 100% car le modèle ne tient pas en mémoire
            )
            speed_overhead = None
        else:
            raise e
...
def save_benchmark_results(results):
    ...
    with open(results_file, "w") as f:
        ...
        f.write(
            f"  Avec checkpointing: {results['memory_with_checkpointing_mb']:.2f} MB\n"
        )
        f.write(
            f"  Sans checkpointing: {results['memory_without_checkpointing_mb']:.2f} MB\n"
        )

        if results["memory_reduction_percent"] is not None:
            f.write(
                f"  Réduction de mémoire: {results['memory_reduction_percent']:.2f}%\n"
            )

        f.write("\nTemps d'entraînement:\n")
        f.write(
            f"  Avec checkpointing: {results['training_time_with_checkpointing_s']:.2f}s\n"
        )

        if results["training_time_without_checkpointing_s"] is not None:
            f.write(
                f"  Sans checkpointing: {results['training_time_without_checkpointing_s']:.2f}s\n"
            )

            if results["speed_overhead_percent"] is not None:
                f.write(
                    f"  Surcoût de calcul: {results['speed_overhead_percent']:.2f}%\n"
                )
        else:
        ...
        if results["peak_memory_with_mb"]:
            ...
            if results["peak_memory_without_mb"]:
                f.write(
                    "  Sans checkpointing: "
                    + ", ".join(
                        [f"{mem:.2f}" for mem in results["peak_memory_without_mb"]]
                    )
                    + "\n"
                )
    ...
    try:
        ...
        if results["train_losses_with"] and results["val_losses_with"]:
            ...
            plt.plot(
                epochs,
                results["train_losses_with"],
                "b-",
                label="Train avec checkpointing",
            )
            plt.plot(
                epochs,
                results["val_losses_with"],
                "b--",
                label="Val avec checkpointing",
            )

            if results["train_losses_without"] and results["val_losses_without"]:
                plt.plot(
                    epochs,
                    results["train_losses_without"],
                    "r-",
                    label="Train sans checkpointing",
                )
                plt.plot(
                    epochs,
                    results["val_losses_without"],
                    "r--",
                    label="Val sans checkpointing",
                )
            ...
            logger.info(
                f"Graphique de perte sauvegardé: {RESULTS_DIR / 'loss_comparison.png'}"
            )
        ...
        if results["peak_memory_with_mb"]:
            ...
            plt.plot(
                epochs, results["peak_memory_with_mb"], "b-", label="Avec checkpointing"
            )

            if results["peak_memory_without_mb"]:
                plt.plot(
                    epochs,
                    results["peak_memory_without_mb"],
                    "r-",
                    label="Sans checkpointing",
                )
            ...
            logger.info(
                f"Graphique de mémoire sauvegardé: {RESULTS_DIR / 'memory_comparison.png'}"
            )
...
def main(args):
    ...
    # Créer les modèles
    logger.info(
        f"Création des modèles avec {num_blocks} blocs et dimension cachée {hidden_dim}..."
    )
    ...
    train_loader, val_loader = generate_trading_data(
        num_samples=args.num_samples, input_dim=input_dim, batch_size=args.batch_size
    )
    ...
    if results["out_of_memory_without_checkpointing"]:
        logger.info(
            "Le modèle sans checkpointing ne peut pas être entraîné en raison de contraintes de mémoire."
        )
        logger.info(
            "L'Activation Checkpointing a permis d'entraîner le modèle avec la VRAM disponible."
        )
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input_dim", type=int, default=64, help="Dimension d'entrée")
    parser.add_argument(
        "--hidden_dim", type=int, default=256, help="Dimension cachée des couches"
    )
    parser.add_argument(
        "--num_blocks", type=int, default=8, help="Nombre de blocs résiduels"
    )
    parser.add_argument(
        "--num_samples",
        type=int,
