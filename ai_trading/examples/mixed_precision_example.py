...
def generate_synthetic_data(
    num_samples=1000, image_size=32, channels=3, num_classes=10
):
...
def compare_precision_modes(batch_size=64, epochs=5):
    ...
    if not torch.cuda.is_available():
        logger.warning(
            "CUDA n'est pas disponible, impossible d'utiliser la précision mixte"
        )
        return None, None
...
def plot_comparison(history_fp32, history_fp16, save_path):
    ...
    bars = axes[1, 1].bar(
        ["FP32", "FP16"], [total_time_fp32, total_time_fp16], color=["blue", "orange"]
    )
...
def run_performance_test():
    ...
    ax2.set_title(
        f"Utilisation mémoire (Réduction: {results['memory_reduction']:.2f}x)"
    )
    ...
    logger.info(
        f"Graphique de performance sauvegardé dans {RESULTS_DIR / 'performance_comparison.png'}"
    )
