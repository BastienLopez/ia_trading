...
class SimpleTradingEnv(gym.Env):
    ...
    def _get_observation(self):
        ...
        return np.concatenate(
            [normalized_prices, position_feature, balance_feature]
        ).astype(np.float32)
    ...
    def step(self, action):
        ...
        if len(self.history) > 1:
            reward = (
                portfolio_value / self.history[-2] - 1
            ) * 100  # Rendement en pourcentage
        else:
            reward = 0
...
def custom_search_space():
    ...
    return {
        ...
        "fcnet_hiddens": tune.choice(
            [[64, 64], [128, 128], [64, 64, 64], [128, 64, 32]]
        ),
...
def main(args):
    ...
    if args.method == "optimize_function":
        ...
        print(
            f"Nombre d'échantillons: {args.num_samples}, itérations: {args.iterations}"
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Exemple d'optimisation d'hyperparamètres avec Ray Tune"
    )
    ...
    parser.add_argument(
        ...
        choices=["ppo", "sac", "dqn"],
        default="ppo",
        help="Algorithme d'apprentissage par renforcement",
    )
    parser.add_argument(
        "--search_alg",
        type=str,
    ...
    parser.add_argument(
        "--num_samples", type=int, default=10, help="Nombre d'échantillons à évaluer"
    )
    parser.add_argument(
        "--iterations",
        type=int,
        default=50,
        help="Nombre maximal d'itérations par essai",
    )
    parser.add_argument(
        "--custom_space",
        action="store_true",
    ...
    parser.add_argument(
        "--num_cpus", type=int, default=1, help="Nombre de CPUs à utiliser par essai"
    )
    parser.add_argument(
        "--num_gpus", type=float, default=0, help="Nombre de GPUs à utiliser par essai"
    )
    parser.add_argument(
        "--checkpoint_dir",
        type=str,
        default="ai_trading/info_retour/optimisation/ray_tune_checkpoints",
