...
def main():
    ...
    print(
        f"  Données générées: {market_data.shape[0]} lignes, {market_data.shape[1]} colonnes"
    )
    ...
    print(
        f"  Prédiction: {sample_prediction['asset']} - {sample_prediction['direction']} ({sample_prediction['confidence']})"
    )
    ...
    for method in methods:
        ...
        print(
            f"  Méthode {method}: [{confidence_interval['lower_direction']} à {confidence_interval['upper_direction']}]"
        )
        print(
            f"    Intervalle numérique: [{confidence_interval['lower_bound']:.2f}, {confidence_interval['upper_bound']:.2f}]"
        )
    ...
    print(
        f"  Probabilités: Baissier={probability_dist['probabilities']['bearish']:.2f}, "
    ...
    outlier_results = calibrator.detect_outliers(
        predictions, method="z_score", threshold=2.5
    )

    print(f"  Total prédictions: {outlier_results['total_predictions']}")
    print(
        f"  Outliers détectés: {outlier_results['outliers_detected']} "
    ...
    if outlier_results["outliers"]:
        ...
        for outlier in outlier_results["outliers"]:
            print(
                f"    ID: {outlier['id']}, Direction: {outlier['direction_value']}, "
    ...
    for prediction in predictions[:3]:
        calibrated = calibrator.calibrate_prediction(
            prediction, calibration_method="platt"
        )
        print(
            f"  Prédiction {prediction['id']}: {prediction['direction']} avec confiance {prediction['confidence']}"
        )
        print(
            f"    Après calibration: confiance {calibrated['confidence']} (valeur: {calibrated['confidence_value']:.2f})"
        )
    ...
    output_dir = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "uncertainty_calibration_outputs"
    )
    os.makedirs(output_dir, exist_ok=True)

    # Visualiser la distribution de probabilité
    dist_path = os.path.join(output_dir, "probability_distribution.png")
    calibrator.plot_probability_distribution(
        sample_prediction, distribution=probability_dist, save_path=dist_path
    )
