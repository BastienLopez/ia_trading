...
def run_compile_example():
    ...
    with torch.no_grad():
        ...
        logger.info(
            f"Temps d'exécution moyen du modèle standard: {standard_time:.6f} secondes"
        )
    ...
    with torch.no_grad():
        ...
        logger.info(
            f"Temps d'exécution moyen du modèle compilé: {compiled_time:.6f} secondes"
        )
...
def run_cudnn_benchmark_example():
    ...
    with torch.no_grad():
        ...
        logger.info(
            f"Temps d'exécution moyen sans cudnn.benchmark: {no_benchmark_time:.3f} ms"
        )
    ...
    with torch.no_grad():
        ...
        logger.info(
            f"Temps d'exécution moyen avec cudnn.benchmark: {benchmark_time:.3f} ms"
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Démonstration des optimisations des opérations critiques"
    )
    parser.add_argument("--all", action="store_true", help="Exécuter tous les exemples")
    parser.add_argument(
        "--jit", action="store_true", help="Exécuter l'exemple de torch.jit.script"
    )
    parser.add_argument(
        "--vmap", action="store_true", help="Exécuter l'exemple de torch.vmap"
    )
    parser.add_argument(
        "--compile", action="store_true", help="Exécuter l'exemple de torch.compile"
    )
    parser.add_argument(
        "--cudnn", action="store_true", help="Exécuter l'exemple de cudnn.benchmark"
    )
