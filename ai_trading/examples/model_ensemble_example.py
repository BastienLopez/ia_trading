...
def create_demo_data():
    ...
    print(f"Données créées : {X_train.shape[0]} échantillons d'entraînement, {X_test.shape[0]} échantillons de test")
...
def main():
    ...
    ensemble.add_model(rf_model, "RandomForest", weight=1.2, 
                      predict_method="predict", 
                      confidence_method="predict_proba")
    
    ensemble.add_model(gb_model, "GradientBoosting", weight=1.0,
                      predict_method="predict", 
                      confidence_method="predict_proba")
    
    ensemble.add_model(lr_model, "LogisticRegression", weight=0.8,
                      predict_method="predict", 
                      confidence_method="predict_proba")
    ...
    for model_name in metrics:
        if model_name != 'ensemble':
            print(f"- {model_name}: Accuracy = {metrics[model_name]['accuracy']:.4f}, "
    ...
    for i in range(5):
        ...
        print(f"- Ensemble: {ensemble_pred} (Ratio de consensus: {ensemble_result.get('consensus_ratio', 'N/A'):.2f})")
    ...
    for strategy in strategies:
        ...
        strat_ensemble.add_model(rf_model, "RandomForest", weight=1.0, 
                                predict_method="predict", 
                                confidence_method="predict_proba")
        
        strat_ensemble.add_model(gb_model, "GradientBoosting", weight=1.0,
                                predict_method="predict", 
                                confidence_method="predict_proba")
        
        strat_ensemble.add_model(lr_model, "LogisticRegression", weight=1.0,
                                predict_method="predict", 
                                confidence_method="predict_proba")
    ...
    for strategy in strategies:
        if 'error' in results[strategy]:
            ...
            print(f"- Stratégie '{strategy}': Accuracy = {results[strategy]['accuracy']:.4f}, "
