...
def visualize_training_results(history, save_path=None):
    ...
    if save_path is None:
        ...
        save_path = f"visualizations/transformer_sac_{timestamp}"
    ...
    if len(history["portfolio_values"]) <= 10:
        ...
        selected_episodes = [0, len(history["portfolio_values"])//2, len(history["portfolio_values"])-1]
        plt.figure(figsize=(15, 8))
...
def train_transformer_sac_agent(
    ...
    for episode in range(episodes):
        ...
        while not done and step < max_steps_per_episode:
            ...
            if step % 10 == 0:  # Enregistrer périodiquement pour économiser de la mémoire
                ...
                history["q_values"].append(train_info["q_value"])
            
            # Log périodique
            if step % 20 == 0:
                portfolio_value = info["portfolio_value"]
                portfolio_weights = info["portfolio_weights"]
                logger.info(f"Épisode {episode+1}/{episodes}, Étape {step}, "
        ...
        if (episode + 1) % eval_every == 0:
            evaluate_agent(agent, data_dict, model_type=model_type, sequence_length=sequence_length)
    ...
    for episode in range(episodes):
        ...
        while not done:
            ...
            if env.current_step % 20 == 0:
                portfolio_value = info["portfolio_value"]
                portfolio_weights = info["portfolio_weights"]
                logger.info(f"Évaluation - Épisode {episode+1}/{episodes}, Étape {env.current_step}, "
                           f"Portefeuille: ${portfolio_value:.2f}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Exemple d'entraînement avec TransformerSAC")
    parser.add_argument("--symbols", nargs="+", default=["BTC", "ETH", "LTC", "ADA", "SOL"],
    ...
    parser.add_argument("--batch_size", type=int, default=64, help="Taille du batch pour l'entraînement")
    ...
    parser.add_argument("--embed_dim", type=int, default=64, help="Dimension d'embedding pour le Transformer")
    parser.add_argument("--num_heads", type=int, default=4, help="Nombre de têtes d'attention")
    parser.add_argument("--seq_length", type=int, default=20, help="Longueur de la séquence")
    parser.add_argument("--transformer_blocks", type=int, default=2, help="Nombre de blocs Transformer")
    parser.add_argument("--days", type=int, default=200, help="Nombre de jours de données synthétiques")
