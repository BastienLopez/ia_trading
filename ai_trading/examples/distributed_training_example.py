...
def generate_synthetic_data(num_samples=10000, seq_length=20, input_dim=128, batch_size=64):
    """
...
def benchmark_distributed_vs_single(
    ...
    if num_gpus < 2:
        logger.warning("Moins de 2 GPUs disponibles, impossible de comparer l'entraînement distribué")
    ...
    for gpu in gpu_info:
        logger.info(f"GPU {gpu['device']}: {gpu['name']}, Mémoire libre: {gpu['free_gb']:.2f} GB")
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (inputs, targets) in enumerate(train_loader):
            ...
            if (batch_idx + 1) % 10 == 0:
                logger.info(f"Époque {epoch+1}/{num_epochs} - Batch {batch_idx+1}/{len(train_loader)} - Loss: {loss.item():.4f}")
        ...
        logger.info(f"Époque {epoch+1}/{num_epochs} - Train Loss: {train_loss:.4f}, Val Loss: {val_loss:.4f}")
    
    single_gpu_training_time = time.time() - single_gpu_start_time
    logger.info(f"Temps d'entraînement Single-GPU: {single_gpu_training_time:.2f} secondes")
    
    # Calculer le gain de performance
    speedup = single_gpu_training_time / ddp_training_time if ddp_training_time > 0 else 0
...
def save_benchmark_results(results):
    ...
    with open(results_file, 'w') as f:
        ...
        f.write(f"Précision mixte: {'Activée' if results['mixed_precision'] else 'Désactivée'}\n\n")
    ...
    plt.bar(['Single-GPU', f'DDP ({results["num_gpus"]} GPUs)'], 
    ...
    for i, v in enumerate([results['single_gpu_training_time'], results['ddp_training_time']]):
    ...
    plt.axhline(y=ideal_time, color='r', linestyle='--', label=f'Accélération idéale ({results["num_gpus"]}x)')
    
    plt.text(1.1, ideal_time - 5, f"Idéal: {ideal_time:.1f}s", color='red')
    plt.legend()
    
    plt.savefig(RESULTS_DIR / "training_time_comparison.png")
    logger.info(f"Graphique de comparaison sauvegardé dans {RESULTS_DIR / 'training_time_comparison.png'}")
...
def main(args):
    ...
    if results:
        # Résumé final
        logger.info("\n=== Résumé du benchmark ===")
        logger.info(f"Entraînement distribué sur {results['num_gpus']} GPUs: {results['ddp_training_time']:.2f} secondes")
        logger.info(f"Entraînement sur un seul GPU: {results['single_gpu_training_time']:.2f} secondes")
        logger.info(f"Accélération: {results['speedup']:.2f}x (théorique idéale: {results['num_gpus']:.1f}x)")
        
        # Efficacité parallèle
        efficiency = (results['speedup'] / results['num_gpus']) * 100
        logger.info(f"Efficacité parallèle: {efficiency:.1f}%")
    
    logger.info(f"Exemple d'entraînement distribué terminé! Voir les résultats dans: {RESULTS_DIR}")


if __name__ == "__main__":
    # Analyser les arguments de la ligne de commande
    parser = argparse.ArgumentParser(description="Exemple d'entraînement distribué multi-GPU")
