...
def plot_inference_comparison(
    ...
    ax.text(
        0.5,
        0.9,
        f"Accélération: {results['speedup']:.2f}x",
...
def main(args):
    ...
    if args.method == "direct":
        ...
        test_results = exporter.test_onnx_model(
            model_path=onnx_model_path, input_shape=(input_dim,), num_samples=5
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Exemple d'exportation de modèle PyTorch vers ONNX"
    )
    parser.add_argument(
        "--method",
        type=str,
    ...
    parser.add_argument(
        "--input_dim", type=int, default=20, help="Dimension d'entrée du modèle"
    )
    parser.add_argument(
        "--hidden_dims",
        type=str,
    ...
    parser.add_argument(
        "--output_dim", type=int, default=1, help="Dimension de sortie du modèle"
    )
    parser.add_argument(
        "--model_name", type=str, default="policy_model", help="Nom du modèle exporté"
    )
    parser.add_argument(
        "--output_dir",
        type=str,
    ...
    parser.add_argument(
        "--benchmark", action="store_true", help="Exécuter un benchmark d'inférence"
    )
    parser.add_argument(
        "--benchmark_runs",
        type=int,
