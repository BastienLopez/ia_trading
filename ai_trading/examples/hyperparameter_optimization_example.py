...
from ai_trading.rl.hyperparameter_optimizer import (
    optimize_gru_sac_agent,
    optimize_sac_agent,
)
...
def main():
    ...
    """
    logger.info(
        "Génération de données synthétiques pour l'optimisation d'hyperparamètres..."
    )
    ...
    logger.info(
        "Démarrage de l'optimisation d'hyperparamètres pour l'agent SAC standard..."
    )
    ...
    logger.info(
        f"Meilleurs hyperparamètres pour l'agent SAC standard: {sac_best_params}"
    )

    # 2. Optimiser les hyperparamètres d'un agent SAC avec GRU
    logger.info(
        "Démarrage de l'optimisation d'hyperparamètres pour l'agent SAC avec GRU..."
    )
    ...
    logger.info(
        f"Meilleurs hyperparamètres pour l'agent SAC avec GRU: {gru_sac_best_params}"
    )

    logger.info(
        "Optimisation d'hyperparamètres terminée. Les résultats sont disponibles dans les répertoires results/"
    )
