...
from ai_trading.utils.ray_rllib_optimizer import RayRLlibOptimizer
...
class SimpleTradingEnv(gym.Env):
    ...
    def __init__(self, config=None):
        ...
        self.obs_dim = (
            self.window_size * self.obs_features + 2
        )  # +2 pour le solde et la position
    ...
    def _generate_dummy_data(self, n_steps=1000, volatility=0.01):
        ...
        df["high"] = df[["open", "close"]].max(axis=1) * (
            1 + np.random.uniform(0, 0.003, n_steps + 1)
        )
    ...
    def _calculate_reward(self, action):
        ...
        if (action == 1 and self.prev_action == 2) or (
            action == 2 and self.prev_action == 1
        ):
    ...
    def _get_observation(self):
        ...
        normalized_prices = (
            frame.iloc[:, 0:4].values - self.price_scaler["mean"]
        ) / self.price_scaler["std"]
        normalized_volumes = (
            frame["volume"].values - self.volume_scaler["mean"]
        ) / self.volume_scaler["std"]
        ...
        observation = np.concatenate(
            [prices_feature, volume_feature, balance_feature, position_feature]
        )
    ...
    def render(self, mode="human"):
        ...
        if mode == "human":
            ...
            print(
                f"Return: {(self.nav_history[-1] / self.initial_balance - 1) * 100:.2f}%"
            )
            print("-" * 50)
...
def train_agent(
    ...
    num_gpus=0,
    train_iterations=50,
    save_dir="ai_trading/info_retour/optimisation/ray_checkpoints",
):
    ...
    return (
        optimizer,
        train_results,
        {"message": "Évaluation désactivée pour éviter les erreurs avec Ray 2.45.0"},
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Exemple d'utilisation de Ray RLlib pour l'entraînement distribué"
    )
    parser.add_argument(
        "--algorithm",
        type=str,
    ...
    parser.add_argument(
        "--num_workers", type=int, default=2, help="Nombre de workers parallèles"
    )
    parser.add_argument(
        "--num_gpus", type=float, default=0, help="Nombre de GPUs à utiliser"
    )
    parser.add_argument(
        "--iterations", type=int, default=50, help="Nombre d'itérations d'entraînement"
    )
    parser.add_argument(
        "--save_dir",
        type=str,
        default="ai_trading/info_retour/optimisation/ray_checkpoints",
        help="Répertoire pour les checkpoints",
    )
