...
from ai_trading.data.order_flow import OrderFlowCollector, OrderBookDepthCollector
from ai_trading.data.enhanced_market_data import EnhancedMarketDataFetcher
from ai_trading.data.synthetic_data_generator import generate_market_with_indicators

def parse_args():
    """Analyse les arguments de ligne de commande."""
    parser = argparse.ArgumentParser(description='Exemple d\'intégration des données de flux d\'ordres')
    
    parser.add_argument('--symbol', type=str, default='BTC',
                      help='Symbole de la cryptomonnaie (ex: BTC, ETH)')
    parser.add_argument('--days', type=int, default=30,
                      help='Nombre de jours de données historiques')
    parser.add_argument('--interval', type=str, default='1d', choices=['1d', '1h', '1m'],
                      help='Intervalle des données (1d, 1h, 1m)')
    parser.add_argument('--depth_levels', type=int, default=5,
                      help='Nombre de niveaux de profondeur du carnet')
    parser.add_argument('--plot', action='store_true',
                      help='Afficher des graphiques')
...
def analyze_order_flow(data):
    ...
    required_cols = ['close', 'buy_volume', 'sell_volume', 'buy_sell_ratio', 'order_book_imbalance']
    ...
    if 'normalized_pressure' in data.columns and len(data) > 5:
        ...
        for idx in buy_pressure_zones:
            if idx < data.index[-5]:  # Vérifier qu'il reste au moins 5 périodes
                ...
                future_return = (data.iloc[position+5]['close'] / data.loc[idx]['close']) - 1
                future_returns.append(future_return)
        
        avg_return_after_buy_pressure = np.mean(future_returns) if future_returns else None
    else:
        buy_pressure_zones = []
        sell_pressure_zones = []
    ...
    return {
        ...
        'buy_volume_proportion': data['buy_volume'].sum() / (data['buy_volume'].sum() + data['sell_volume'].sum())
    }
...
def plot_data(data, symbol):
    ...
    ax2_twin = ax2.twinx()
    ax2_twin.plot(data.index, data['order_book_imbalance'], 'r-', label='Déséquilibre du carnet')
    ...
    if 'normalized_pressure' in data.columns:
        ...
        ax3.plot(data.index, data['normalized_pressure'], 'purple', label='Pression normalisée')
        ax3.axhline(y=0, color='gray', linestyle='-', alpha=0.3)
        ax3.fill_between(data.index, 0, data['normalized_pressure'], 
    ...
    if 'ask_price_1' in data.columns and 'bid_price_1' in data.columns:
        ...
        plt.barh(ask_prices, ask_volumes, color='red', alpha=0.6, label='Offres de vente')
        plt.barh(bid_prices, [-v for v in bid_volumes], color='green', alpha=0.6, label='Offres d\'achat')
        
        plt.axhline(y=last_entry['mid_price'], color='blue', linestyle='--', label='Prix du marché')
        plt.xlabel('Volume')
        plt.ylabel('Prix')
    ...
    logger.info(f"Graphiques sauvegardés: {symbol}_order_flow_analysis.png et {symbol}_order_book_depth.png")
...
def main():
    ...
    for key, value in flow_insights.items():
        if key != 'price_correlation' and key not in ('buy_pressure_zones', 'sell_pressure_zones'):
