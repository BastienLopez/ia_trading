...
def example_pytorch_compilation():
    ...
    compiler = TorchScriptCompiler(save_dir="./compiled_models")
    
    # Tracer le modèle
    traced_model = compiler.trace_model(
        model=model,
        example_inputs=input_data,
        name="example_traced_model"
    )
    ...
    print(f"Temps moyen par inférence (original): {benchmark_results['original_avg_ms']:.3f} ms")
    print(f"Temps moyen par inférence (compilé): {benchmark_results['scripted_avg_ms']:.3f} ms")
    ...
    def complex_function(x, y):
        ...
        for i in range(x.size(0)):
            result[i] = torch.sin(x[i]) * torch.cos(y[i]) + torch.sqrt(torch.abs(x[i] * y[i]))
        return result
...
def example_tensorflow_xla():
    ...
    if not XLA_AVAILABLE:
        print("XLA n'est pas disponible dans votre installation TensorFlow, exemple ignoré.")
        return
