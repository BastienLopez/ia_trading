...
def demo_parallel_operations():
    ...
    # Calculer le speedup
    speedup = (
        elapsed_sequential / elapsed_parallel if elapsed_parallel > 0 else float("inf")
    )
...
def main(args):
    ...
    if args.all or args.batch:
        ...
        speedups["batch"] = (
            slowest_time / fastest_time if fastest_time > 0 else float("inf")
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Démonstration des optimisations de réduction des temps d'opération"
    )
    parser.add_argument(
        "--all", action="store_true", help="Exécuter toutes les démonstrations"
    )
    parser.add_argument(
        "--cache",
        action="store_true",
        help="Exécuter la démo de mise en cache des résultats",
    )
    parser.add_argument(
        "--batch",
        action="store_true",
        help="Exécuter la démo de taille de batch optimale",
    )
    parser.add_argument(
        "--prediction",
        action="store_true",
        help="Exécuter la démo de cache de prédictions",
    )
    parser.add_argument(
        "--parallel", action="store_true", help="Exécuter la démo de parallélisation"
    )
