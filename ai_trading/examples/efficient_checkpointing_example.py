...
RESULTS_DIR = Path("ai_trading/info_retour/examples/efficient_checkpointing")
...
def benchmark_checkpoint_methods(model, optimizer, data_size_mb):
    ...
    async_saver.save_checkpoint(
        model, async_path, optimizer=optimizer, weights_only=False
    )
    ...
    logger.info(
        f"Poids uniquement    : {weights_time:.4f}s, {weights_size:.2f} MB "
    ...
    logger.info(
        f"Estimation pour {num_epochs} époques avec {checkpoints_per_epoch} checkpoints/époque:"
    )
    logger.info(f"Taille standard      : {standard_total:.2f} MB")
    logger.info(f"Taille optimisée     : {optimized_total:.2f} MB")
    logger.info(
        f"Espace économisé     : {standard_total - optimized_total:.2f} MB "
...
def train_with_efficient_checkpointing(
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % checkpoint_interval == 0 or epoch == num_epochs - 1:
            ...
            checkpoint_mgr.save(
                model, optimizer=optimizer, epoch=epoch + 1, extra_data=metadata
            )
    ...
    with open(RESULTS_DIR / "training_history.txt", "w") as f:
        ...
        f.write(
            f"{'Époque':<10}{'Train Loss':<15}{'Val Loss':<15}{'Val Acc':<15}"
            f"{'Temps (s)':<15}{'Checkpoint (s)':<15}\n"
        )

        for i in range(num_epochs):
            f.write(
                f"{i+1:<10}{training_history['train_loss'][i]:<15.4f}"
    ...
    logger.info(
        f"Historique d'entraînement sauvegardé dans: {RESULTS_DIR / 'training_history.txt'}"
    )
...
def main():
    ...
    data_size_bytes = X.element_size() * X.nelement() + y.element_size() * y.nelement()
