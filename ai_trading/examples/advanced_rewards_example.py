...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Démonstration des récompenses avancées pour RL en trading"
    )
    parser.add_argument(
        "--symbol",
        type=str,
        default="BTC",
        help="Symbole de la crypto-monnaie (par défaut: BTC)",
    )
    parser.add_argument(
        "--days",
        type=int,
        default=90,
        help="Nombre de jours de données à récupérer (par défaut: 90)",
    )
    parser.add_argument(
        "--interval",
        type=str,
    ...
    parser.add_argument(
        "--plot", action="store_true", help="Affiche les graphiques des métriques"
    )
...
def run_simulation_with_reward(env, reward_calculator, num_episodes, reward_name):
    ...
    for episode in range(num_episodes):
        ...
        while not done:
            ...
            if isinstance(reward_calculator, AdvancedRewardCalculator):
                ...
                if len(episode_values) > 0:
                    current_return = (
                        env.get_portfolio_value() - episode_values[-1]
                    ) / episode_values[-1]
                else:
                    current_return = 0
        ...
        print(
            f"Épisode {episode+1}/{num_episodes} avec {reward_name} - "
...
def advanced_rewards_demo():
    ...
    env = TradingEnvironment(
        data=df, initial_balance=10000, transaction_fee=0.001, window_size=10
    )
    ...
    sharpe_calculator = SharpeRatioReward(
        risk_free_rate=0.02, window_size=args.window, annualize=False
    )

    transaction_calculator = TransactionCostReward(
        base_cost=0.001, frequency_penalty=0.0005, window_size=args.window
    )

    drawdown_calculator = DrawdownReward(penalty_factor=2.0, window_size=args.window)

    # Exécuter des simulations avec chaque type de récompense
    print("\n=== Simulation avec récompense combinée ===")
    portfolio_values_combined, rewards_combined, actions_combined = (
        run_simulation_with_reward(env, advanced_calculator, args.episodes, "Combinée")
    )

    print("\n=== Simulation avec récompense basée sur le ratio de Sharpe ===")
    portfolio_values_sharpe, rewards_sharpe, actions_sharpe = (
        run_simulation_with_reward(env, sharpe_calculator, args.episodes, "Sharpe")
    )

    print("\n=== Simulation avec récompense incluant les coûts de transaction ===")
    portfolio_values_transaction, rewards_transaction, actions_transaction = (
        run_simulation_with_reward(
            env, transaction_calculator, args.episodes, "Transaction"
        )
    )

    print("\n=== Simulation avec récompense basée sur le drawdown ===")
    portfolio_values_drawdown, rewards_drawdown, actions_drawdown = (
        run_simulation_with_reward(env, drawdown_calculator, args.episodes, "Drawdown")
    )
    ...
    for name, values in portfolio_values_dict.items():
        ...
        print(
            f"Stratégie {name}: Valeur finale moyenne = {avg_final_value:.2f}, "
