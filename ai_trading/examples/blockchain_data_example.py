...
def analyze_ethereum_transactions():
    ...
    if 'timeStamp' in transactions.columns:
        ...
        txs_by_month = transactions.groupby(transactions['timeStamp'].dt.to_period('M')).size()
...
def analyze_defi_pools():
    ...
    for chain in chains_to_analyze:
        ...
        if 'project' in pools_data.columns and 'tvlUsd' in pools_data.columns:
            tvl_by_project = pools_data.groupby('project')['tvlUsd'].sum().sort_values(ascending=False).head(10)
...
def get_combined_blockchain_analysis():
    ...
    for blockchain in blockchains:
        ...
        combined_data = collector.get_combined_blockchain_data(blockchain=blockchain, days=30)
...
def gas_usage_analysis():
    ...
    for idx, row in gas_data.iterrows():
        print(f"  {row['contract']}: {row['gas_used_24h']:,} gas ({row['txs_24h']:,} transactions)")
