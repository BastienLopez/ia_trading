    ...
    for event in events:
        ...
        if event['type'] == EventType.REGULATORY.value:
            ...
            event_info = {
                ...
                'Dernière mention': event['latest_mention'].split('T')[0] if 'T' in event['latest_mention'] else event['latest_mention']
            }
        else:
            event_info = {
                'Type': "INCONNU",
...
def display_alerts(alerts):
    ...
    for i, alert in enumerate(alerts, 1):
        ...
        print(f"  Horodatage: {alert['timestamp'].split('T')[0] if 'T' in alert['timestamp'] else alert['timestamp']}")
        print()
...
def plot_event_distribution(all_events):
    ...
    print("Un graphique aurait montré la distribution des événements par type et sévérité.")
...
def main():
    ...
    bitcoin_news = filter_news_by_relevance(preprocessed_news, bitcoin_keywords, threshold=0.05)
