...
from ai_trading.utils.performance_tracker import PerformanceTracker, track_time
...
def preprocessing_sequential(data):
    ...
    for i in range(len(data)):
        ...
        processed = {
            ...
            "momentum": (
                row["close"] - data.iloc[max(0, i - 10) : i + 1]["close"].mean()
                if i > 0
                else 0
            ),
            "volume_ratio": (
                row["volume"] / data.iloc[max(0, i - 5) : i + 1]["volume"].mean()
                if i > 0
                else 1
            ),
        }
...
def process_row(row_data):
    ...
    processed = {
        ...
        "volatility": np.sqrt(np.square(row["high"] - row["low"])),
        "momentum": (
            row["close"] - data_window["close"].mean() if len(data_window) > 0 else 0
        ),
        "volume_ratio": (
            row["volume"] / data_window["volume"].mean() if len(data_window) > 0 else 1
        ),
    }
    return processed
...
def compare_dataloader_configs(dataset, configs):
    ...
    for name, config in configs.items():
        ...
        tracker.log_summary()
        logger.info(
            f"Performance {name}: {batches_per_second:.2f} batches/s, temps total: {elapsed_time:.2f}s"
        )

    # Trouver la meilleure configuration
    best_config = max(results.items(), key=lambda x: x[1]["batches_per_second"])
    logger.info(
        f"Meilleure configuration: {best_config[0]} avec {best_config[1]['batches_per_second']:.2f} batches/s"
    )
...
def main():
    ...
    logger.info(
        "=== Exemple d'optimisation multithreading/multiprocessing pour le trading ==="
    )
    ...
    logger.info(
        f"- CPU: {threading_optimizer.cpu_count} cœurs logiques, {threading_optimizer.physical_cores} cœurs physiques"
    )
    logger.info(
        f"- Hyperthreading: {'Présent' if threading_optimizer.has_hyperthreading else 'Absent'}"
    )
    ...
    parallel_result = preprocessing_parallel(
        data, num_workers=optimal_workers["preprocessing"]
    )
    ...
    dataset = FinancialDataset(
        data=data, sequence_length=50, is_train=True, lazy_loading=True, chunk_size=5000
    )

    # Différentes configurations à tester
    dataloader_configs = {
        "Séquentiel": {"batch_size": 32, "num_workers": 0},
        "Workers_2": {
            "batch_size": 32,
            "num_workers": 2,
            "prefetch_factor": 2,
    ...
    tracker.log_summary()
    logger.info(
        f"Performance auto-optimisée: {batches_per_second:.2f} batches/s, temps total: {elapsed_time:.2f}s"
    )
    ...
    logger.info(
        f"Accélération avec la meilleure configuration manuelle ({best_config}): {best_speedup:.2f}x"
    )
    logger.info(
        f"Accélération avec la configuration auto-optimisée: {auto_speedup:.2f}x"
    )

    logger.info("\nRecommandations:")
    if auto_speedup > best_speedup:
        logger.info(
            "- Utiliser l'auto-optimisation (num_workers=-1, auto_threading=True)"
        )
    else:
        logger.info(f"- Utiliser la configuration manuelle {best_config}")

    logger.info(
        f"- Pour le prétraitement des données, utiliser {optimal_workers['preprocessing']} workers"
    )
    logger.info(
        f"- Pour les bibliothèques numériques (NumPy/PyTorch), limiter à {optimal_workers['training']} threads"
    )
