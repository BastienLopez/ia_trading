...
from rl.policy_lag import DecoupledPolicyTrainer
...
class SimpleEnv:
    ...
    def _get_state(self):
        ...
        return torch.tensor(
            [self.position, self.target - self.position], dtype=torch.float32
        )
...
def run_episode(env, trainer, epsilon=0.1, render=False):
    ...
    while not done:
        ...
        if render:
            print(
                f"Step {step}: Position {env.position:.1f}, Action {action}, Reward {reward:.2f}"
            )
...
def train_agent(args):
    ...
    for episode in range(args.episodes):
        ...
        if episode % args.log_freq == 0 or episode == args.episodes - 1:
            avg_reward = sum(rewards[-args.log_freq :]) / min(
                args.log_freq, len(rewards[-args.log_freq :])
            )
            avg_steps = sum(steps[-args.log_freq :]) / min(
                args.log_freq, len(steps[-args.log_freq :])
            )
            print(
...
def main():
    ...
    parser.add_argument(
        "--max-steps", type=int, default=100, help="Nombre maximum d'étapes par épisode"
    )
    parser.add_argument(
        "--learning-rate", type=float, default=0.001, help="Taux d'apprentissage"
    )
    parser.add_argument(
        "--epsilon", type=float, default=0.3, help="Epsilon initial pour l'exploration"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Fréquence de mise à jour du modèle de collecte",
    )
    parser.add_argument(
        "--target-update-freq",
        type=int,
        default=100,
        help="Fréquence de mise à jour du modèle cible",
    )

    # Configuration de l'entraînement
    parser.add_argument(
        "--batch-size", type=int, default=32, help="Taille des batchs d'entraînement"
    )
    parser.add_argument(
        "--log-freq", type=int, default=10, help="Fréquence d'affichage des logs"
    )
    parser.add_argument(
        "--render-freq",
        type=int,
        default=20,
        help="Fréquence d'affichage de l'environnement",
    )

    # Configuration de l'expérience
    parser.add_argument(
        "--compare", action="store_true", help="Comparer différentes configurations"
    )
    parser.add_argument(
        "--show-plots", action="store_true", help="Afficher les graphiques"
    )
