...
from ai_trading.rl.replay_buffer import ReplayBuffer, PrioritizedReplayBuffer
...
def train_model_with_buffer(buffer, model, target_model, num_updates, batch_size, gamma=0.99):
    """
    ...
    for update in range(num_updates):
        ...
        if (update + 1) % 10 == 0:
            logger.info(f"Update {update+1}/{num_updates}, Loss: {loss.item():.4f}, "
                       f"Buffer size: {len(buffer)}")
    
    return losses, sampling_times, buffer_sizes


def compare_buffers(state_dim, action_dim, num_transitions, buffer_size, 
                   num_updates, batch_size, output_dir="outputs"):
    """
    ...
    buffers = [
        ...
        ("Enhanced", EnhancedPrioritizedReplay(
