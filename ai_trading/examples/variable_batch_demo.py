...
from rl.variable_batch import BatchOptimizer, VariableBatchSampler
...
def benchmark_batch_sizes(
    model, buffer, device, min_size=32, max_size=1024, n_iterations=100
):
    """
    ...
    for batch_size in batch_sizes:
        ...
        print(
            f"Taille de batch {batch_size}: {elapsed:.4f}s, "
...
def main(args):
    ...
    if args.strategies:
        ...
        strategy_results = compare_strategies(
            buffer, strategies, n_samples=args.adaptations * 10, check_interval=10
        )
    ...
    if args.optimize:
        ...
        print("\n=== Recherche de la taille de batch optimale ===")
        optimal_size, optimizer_results = find_optimal_batch_size_demo(
            model, buffer, device
        )
        ...
        try:
            for i in range(args.demo_steps):
                ...
                if i % 10 == 0:
                    ...
                    gpu_util = (
                        resources.get("gpu", 0) * 100
                        if resources.get("gpu") is not None
                        else "N/A"
                    )

                    print(
                        f"Itération {i}: Taille={info['batch_size']}, "
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Démonstration de Variable Batch Sampler"
    )

    # Paramètres généraux
    parser.add_argument("--state-dim", type=int, default=84, help="Dimension des états")
    parser.add_argument(
        "--buffer-size", type=int, default=100000, help="Taille du buffer"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--benchmark",
        action="store_true",
        help="Exécuter le benchmark de tailles de batch",
    )
    parser.add_argument(
        "--min-batch",
        type=int,
        default=32,
        help="Taille de batch minimale pour le benchmark",
    )
    parser.add_argument(
        "--max-batch",
        type=int,
        default=1024,
        help="Taille de batch maximale pour le benchmark",
    )
    parser.add_argument(
        "--iterations",
        type=int,
        default=100,
        help="Nombre d'itérations pour chaque taille de batch",
    )

    # Paramètres de comparaison des stratégies
    parser.add_argument(
        "--strategies", action="store_true", help="Comparer les stratégies d'adaptation"
    )
    parser.add_argument(
        "--adaptations", type=int, default=10, help="Nombre d'adaptations à effectuer"
    )

    # Recherche de la taille optimale
    parser.add_argument(
        "--optimize", action="store_true", help="Trouver la taille de batch optimale"
    )

    # Démonstration en temps réel
    parser.add_argument(
        "--demo", action="store_true", help="Exécuter la démonstration en temps réel"
    )
    parser.add_argument(
        "--demo-steps",
        type=int,
        default=200,
        help="Nombre d'étapes pour la démonstration",
    )
