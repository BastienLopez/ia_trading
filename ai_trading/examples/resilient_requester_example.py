...
from ai_trading.utils.resilient_requester import (
    ResilientRequester, MultiSourceRequester, resilient,
    CircuitBreakerError, AllSourcesFailedError
)
...
def simulate_api_call(api_name: str, success_rate: float = 0.8, timeout_rate: float = 0.1, delay: float = 0.2) -> Dict[str, Any]:
    """
...
def demo_resilient_decorator():
    ...
    @resilient(
        ...
        base_delay=0.3,
        timeout=0.8,
        fallback_function=price_fallback
    )
    ...
    for i in range(10):
        ...
        logger.info(f"Prix pour {symbol}: {result['price']} (source: {result['source']})")
