...
def parse_args():
    ...
    parser.add_argument('--symbol', type=str, default='BTC', help='Symbole de la crypto-monnaie')
    ...
    parser.add_argument('--initial_balance', type=float, default=10000.0, help='Solde initial du portefeuille')
...
def preprocess_data(data):
    ...
    if len(data) < 30:
        logger.info("Données insuffisantes, génération de données synthétiques supplémentaires.")
    ...
    if 'open' not in data_prepared.columns and price_col in data_prepared.columns:
        logger.info("Colonne 'open' manquante, utilisation de la colonne de prix avec variation.")
        data_prepared['open'] = data_prepared[price_col] * np.random.uniform(0.995, 1.005, len(data_prepared))
    
    if 'high' not in data_prepared.columns and price_col in data_prepared.columns:
        logger.info("Colonne 'high' manquante, utilisation de la colonne de prix avec augmentation.")
        data_prepared['high'] = data_prepared[price_col] * np.random.uniform(1.001, 1.01, len(data_prepared))
    
    if 'low' not in data_prepared.columns and price_col in data_prepared.columns:
        logger.info("Colonne 'low' manquante, utilisation de la colonne de prix avec diminution.")
        data_prepared['low'] = data_prepared[price_col] * np.random.uniform(0.99, 0.999, len(data_prepared))
    ...
    if 'volume' not in data_prepared.columns:
        ...
        data_prepared['volume'] = np.random.uniform(1000, 10000, len(data_prepared)) * base_price / 100
    ...
    for col in data_filled.columns:
        if data_filled[col].isna().any():
            if 'volume' in col.lower():
                ...
                data_filled[col] = data_filled[col].fillna(data_filled[col].mean() if not data_filled[col].isna().all() else 0)
...
def generate_synthetic_data(days=60, symbol='BTC'):
    ...
    base_price = 30000 if symbol.upper() == 'BTC' else (2000 if symbol.upper() == 'ETH' else 100)
    ...
    df = pd.DataFrame({
        ...
        'close': prices,
        'price': prices,
        'volume': np.random.uniform(base_price * 10, base_price * 100, len(date_range)),
        'market_cap': prices * np.random.uniform(1e9, 1e10, len(date_range)) / base_price,
...
def train_agent_with_reward(env, reward_function, episodes, visualize=False):
    ...
    max_drawdown = (max_portfolio_value - min(env.portfolio_value_history)) / max_portfolio_value
...
def compare_reward_functions(data, args):
    ...
    logger.info(f"{'Fonction':<20} | {'Valeur finale':<15} | {'Rendement':<15} | {'Max Drawdown':<15} | {'Transactions':<10}")
    logger.info("-" * 80)
    ...
    for result in results:
        ...
        logger.info(f"{reward_function:<20} | {final_value:<15.2f} | {return_pct:<15.2f}% | {max_drawdown:<15.2f}% | {transactions:<10}")
...
def visualize_comparison(results, initial_balance):
    ...
    filename = os.path.join(visualization_dir, f'reward_functions_comparison_{timestamp}.png')
...
def main():
    ...
    try:
        ...
        logger.info(f"Données récupérées: {len(data)} entrées avec colonnes: {data.columns.tolist()}")
        
        # Prétraiter les données
        logger.info("Prétraitement des données...")
        data_clean = preprocess_data(data)
        logger.info(f"Données prétraitées: {len(data_clean)} entrées avec colonnes: {data_clean.columns.tolist()}")
