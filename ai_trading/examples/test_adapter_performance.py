...
def generate_market_data(num_points=1000, with_outliers=True, with_trends=True):
    ...
    if with_outliers:
        ...
        outlier_indices = np.random.choice(num_points, size=int(num_points * 0.05), replace=False)
...
def run_performance_test(data_size=1000, batch_sizes=[10, 50, 100, 250]):
    ...
    for batch_size in batch_sizes:
        ...
        print(f"  Temps moyen de traitement par lot: {perf_stats.get('avg_processing_time', 0):.6f}s")
        print(f"  Valeurs aberrantes filtrées: {perf_stats.get('total_outliers_removed', 0)}")
