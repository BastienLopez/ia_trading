...
def train_model_with_lazy_loading(data_path, sequence_length=50, batch_size=32, epochs=5):
    """
    ...
    for epoch in range(epochs):
        ...
        logger.info(f"Epoch {epoch+1}/{epochs} terminée en {epoch_time:.2f}s, Loss moyenne: {avg_loss:.6f}")
...
def test_lazy_file_reader(data_path):
    ...
    for start, end in ranges:
        if end <= reader.get_length():
            ...
            elapsed = time.time() - start_time
            logger.info(f"Lignes {start}-{end}: {len(rows)} lignes chargées en {elapsed:.6f}s")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Exemple de lazy loading et optimisations associées")
    parser.add_argument("--data-size", type=int, default=100000, help="Nombre de points de données à générer")
    parser.add_argument("--sequence-length", type=int, default=50, help="Longueur des séquences pour le modèle")
    parser.add_argument("--batch-size", type=int, default=32, help="Taille des batchs pour l'entraînement")
    ...
    if data_path:
        ...
        result_df = test_batch_inference(
            ...
            data_path,
            sequence_length=args.sequence_length,
            batch_size=args.batch_size * 2  # Utiliser des batchs plus grands pour l'inférence
        )
