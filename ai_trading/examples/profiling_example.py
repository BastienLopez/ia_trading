...
from ai_trading.utils.profiling import (
    ProfilingManager,
    profile_function,
    profile_block
)
...
def process_data_optimized(size=1000000):
    ...
    final_result = np.sum(result1) + np.sum(result3) + np.sum(min_vals) + np.sum(max_vals)
...
def main():
    ...
    # Créer un gestionnaire de profilage
    profiler = ProfilingManager(output_dir="./profiling_results")
    ...
    for i, func in enumerate(result['top_functions'][:5], 1):
        print(f"{i}. {func['function']} - {func['cumtime']:.4f}s cumulé, {func['tottime']:.4f}s total")
    ...
    speedup = result1['top_functions'][0]['cumtime'] / result2['top_functions'][0]['cumtime'] if result2['top_functions'] else 0
