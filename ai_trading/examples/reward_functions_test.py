...
def preprocess_data(df, window_size=20):
    ...
    if len(df_clean) < window_size:
        raise ValueError(f"Après nettoyage, il ne reste que {len(df_clean)} points de données, ce qui est inférieur à window_size={window_size}")
...
def plot_results(results):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(visualization_dir, 'reward_functions_portfolio_comparison.png'))
    ...
    for i, rf in enumerate(reward_functions):
        ...
        truncated_values = [values[:min_length] for values in results[rf]["portfolio_values"]]
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(visualization_dir, 'reward_functions_portfolio_evolution.png'))
    
    # 3. Distribution des récompenses
    plt.figure(figsize=(12, 6))
    
    for i, rf in enumerate(reward_functions):
        all_rewards = [r for episode_rewards in results[rf]["rewards"] for r in episode_rewards]
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(visualization_dir, 'reward_functions_rewards_distribution.png'))
    ...
    for i, v in enumerate(cumulative_rewards):
        plt.text(i, v + (0.1 * max(cumulative_rewards) if v > 0 else -0.1 * max(cumulative_rewards)), 
                f"{v:.2f}", ha='center')
    
    plt.tight_layout()
    plt.savefig(os.path.join(visualization_dir, 'reward_functions_cumulative_rewards.png'))
    
    print(f"Graphiques sauvegardés dans le répertoire {visualization_dir}")


def main():
    parser = argparse.ArgumentParser(description='Test des fonctions de récompense pour l\'environnement de trading')
    parser.add_argument('--episodes', type=int, default=3, help='Nombre d\'épisodes à exécuter')
    parser.add_argument('--days', type=int, default=60, help='Nombre de jours de données à utiliser')
    parser.add_argument('--symbol', type=str, default='BTC', help='Symbole de la cryptomonnaie')
    parser.add_argument('--use_real_data', action='store_true', help='Utiliser des données réelles au lieu de données synthétiques')
    ...
    try:
        if args.use_real_data:
            ...
            if df is None or len(df) == 0:
                print("Aucune donnée réelle disponible. Utilisation de données synthétiques à la place.")
        ...
        best_reward_function = max(results.keys(), key=lambda rf: results[rf]["avg_final_value"])
        ...
        print(f"Récompense cumulative: {results[best_reward_function]['cumulative_reward']:.4f}")
        print("==============================")
