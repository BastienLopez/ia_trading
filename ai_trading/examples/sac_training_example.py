...
def parse_arguments():
    ...
    parser.add_argument('--interval', type=str, default='1h', help='Intervalle de temps (1h, 4h, 1d)')
    parser.add_argument('--initial_balance', type=float, default=10000.0, help='Solde initial')
    parser.add_argument('--save', action='store_true', help='Sauvegarder le modèle entraîné')
    ...
    parser.add_argument('--reward_function', type=str, default='sharpe', 
...
def create_environment(df, args):
    ...
    logger.info(f"Environnement créé avec {len(df)} points de données et espace d'action continu")
...
def evaluate_agent(env, agent, num_episodes=5):
    ...
    logger.info(f"Résultat moyen sur {num_episodes} épisodes: "
...
def main():
    ...
    if not args.load or args.episodes > 0:
        ...
        if args.save:
            save_dir = f"models/sac_{args.symbol}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
