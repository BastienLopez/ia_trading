    ...
    predictions = {
        "short_term": {
            **base_prediction,
            ...
            "contradictions": ["Support proche testé récemment"],
            "volatility": "low",
        ...
        "long_term": {
            **base_prediction,
            ...
            "contradictions": ["Résistance historique à 13000 USD"],
            "volatility": "medium",
...
def main():
    ...
    print(
        f"  Prédiction pour {prediction['asset']}: {prediction['direction']} ({prediction['confidence']})"
    )
    ...
    print(
        f"  Prédictions générées pour {len(multi_horizon_predictions)} horizons temporels"
    )
    ...
    print(
        f"  Signal de trading: {consistency_analysis['trading_signals']['signal']} ({consistency_analysis['trading_signals']['strength']})"
    )
    ...
    output_dir = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "output_visualizations"
    )
    ...
    explanation_path = visualizer.create_explanation_dashboard(
        prediction,
        {
            ...
            "llm": explainer._explain_llm_prediction(prediction),
        },
        os.path.join(output_dir, "explanation_dashboard.html"),
    )

    consistency_path = visualizer.create_consistency_visualization(
        consistency_analysis, os.path.join(output_dir, "consistency_visualization.html")
    )
    ...
    single_report_path = reporter.generate_prediction_report(
        prediction, market_data, os.path.join(output_dir, "single_prediction"), "html"
    )
