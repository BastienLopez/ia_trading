...
def generate_synthetic_data(symbols, days=100, seed=42):
    ...
    for symbol in symbols:
        ...
        for i in range(1, days):
            ...
            daily_return = np.random.normal(params["trend"] / days, params["volatility"] / np.sqrt(days))
    ...
    for episode in range(episodes):
        ...
        while not done:
            ...
            if env.current_step % 20 == 0:
                ...
                logger.info(f"Étape {env.current_step}, Portefeuille: ${portfolio_value:.2f}")
...
def visualize_results(results, symbols):
    """
    Visualise les résultats de plusieurs épisodes.
    ...
    plt.savefig("visualizations/episode_returns.png")
    plt.close()
    ...
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.savefig("visualizations/portfolio_evolution.png")
    plt.close()
    ...
    if results["final_allocations"]:
        avg_allocation = {}
        for symbol in symbols:
            avg_allocation[symbol] = np.mean([alloc.get(symbol, 0) for alloc in results["final_allocations"]])
        ...
        plt.savefig("visualizations/average_allocation.png")
        plt.close()
...
def main():
    ...
    parser.add_argument("--episodes", type=int, default=5, help="Nombre d'épisodes à exécuter")
    parser.add_argument("--visualize", action="store_true", help="Visualiser les résultats")
    parser.add_argument("--days", type=int, default=100, help="Nombre de jours de données")
    ...
    test_multi_asset_environment(data_dict, episodes=args.episodes, visualize=args.visualize)
