...
def fetch_crypto_data(symbol: str, start_date: datetime, end_date: datetime, interval: str = "1d") -> pd.DataFrame:
    """
    ...
    try:
        ...
        logger.info(f"Récupération des données pour {symbol} de {start_date} à {end_date}")
        time.sleep(1)  # Simuler la latence réseau
        
        # Générer des données fictives pour la démo
        date_range = pd.date_range(start=start_date, end=end_date, freq='D')
        seed = sum(ord(c) for c in symbol)  # Seed basée sur le symbole pour la cohérence
        np.random.seed(seed)
...
def fetch_stock_data(symbol: str, start_date: datetime, end_date: datetime, interval: str = "1d") -> pd.DataFrame:
    """
...
def demo_basic_cache():
    ...
    logger.info(f"Données complexes récupérées, taille: {len(str(retrieved_data))} caractères")
...
def demo_data_cache():
    ...
    logger.info(f"Première requête: {len(btc_data)} points en {first_request_time:.2f} secondes")
    ...
    logger.info(f"Deuxième requête (cachée): {len(btc_data_cached)} points en {second_request_time:.2f} secondes")
    ...
    logger.info(f"Données AAPL récupérées: {len(aapl_data)} points en {time.time() - start_time:.2f} secondes")
    
    # Au lieu de visualiser les données avec matplotlib, afficher quelques statistiques
    logger.info(f"Statistiques BTC: min={btc_data['close'].min():.2f}, max={btc_data['close'].max():.2f}, moyenne={btc_data['close'].mean():.2f}")
    logger.info(f"Statistiques AAPL: min={aapl_data['close'].min():.2f}, max={aapl_data['close'].max():.2f}, moyenne={aapl_data['close'].mean():.2f}")
...
def demo_preloading():
    ...
    preload_thread = threading.Thread(
        target=lambda: cache.preload("slow_data", slow_data_loader),
        daemon=True
    )
    ...
    logger.info(f"Données préchargées récupérées en {time.time() - start_time:.4f} secondes")
...
def demo_compression():
    ...
    logger.info(f"Cache avec compression faible: {stats_low['memory_usage_estimate'] / 1024 / 1024:.2f} Mo")
    logger.info(f"Cache avec compression élevée: {stats_high['memory_usage_estimate'] / 1024 / 1024:.2f} Mo")
