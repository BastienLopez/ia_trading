...
def plot_comparison(results, save_path):
    ...
    for steps, history in results.items():
        plt.plot(history["train_loss"], label=f"{steps} étapes (batch effectif={steps*32})")
    ...
    plt.subplot(2, 1, 2)
    for steps, history in results.items():
        plt.plot(history["val_loss"], label=f"{steps} étapes (batch effectif={steps*32})")
...
def demo_gradient_accumulator_class():
    ...
    for epoch in range(3):
        ...
        for i, (inputs, targets) in enumerate(dataloader):
            ...
            if accumulator.step():
                logger.info(f"Mise à jour des poids effectuée après {accumulator.accumulation_steps} batchs")
