...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description="Exemple de validation croisée temporelle pour l'optimisation d'agents de trading")
    
    parser.add_argument('--symbol', type=str, default='BTC', help='Symbole de la crypto-monnaie (ex: BTC, ETH)')
    parser.add_argument('--days', type=int, default=180, help='Nombre de jours de données historiques à utiliser')
    parser.add_argument('--interval', type=str, default='1d', help='Intervalle des données (ex: 1h, 4h, 1d)')
    parser.add_argument('--initial_train_size', type=float, default=0.6, help='Proportion initiale pour l\'entraînement')
    parser.add_argument('--n_splits', type=int, default=3, help='Nombre de divisions temporelles')
    parser.add_argument('--gap', type=int, default=5, help='Écart entre l\'entraînement et la validation (en périodes)')
    parser.add_argument('--visualize', action='store_true', help='Visualiser les résultats')
    parser.add_argument('--optimize', action='store_true', help='Exécuter l\'optimisation des hyperparamètres')
    
    return parser.parse_args()

def collect_data(symbol, days, interval):
    """Collecte des données historiques pour une crypto-monnaie."""
    logger.info(f"Collecte des données pour {symbol} sur {days} jours avec un intervalle de {interval}")
    
    try:
        # Utiliser notre collecteur de données amélioré
        collector = EnhancedDataCollector()
        data = collector.get_historical_market_data(symbol=symbol, days=days, interval=interval)
        
        # Si aucune donnée n'est disponible, utiliser des données synthétiques
        if data is None or len(data) == 0:
            logger.warning("Aucune donnée n'a pu être récupérée. Génération de données synthétiques.")
...
def evaluate_agent(agent, env, max_steps=None):
    ...
    logger.info(f"Évaluation - Récompense: {total_reward:.2f}, Profit: {profit_pct:.2f}%, "
...
def visualize_optimization_results(results):
    ...
    plt.yticks(y_pos, [label[:30] + "..." if len(label) > 30 else label for label in sorted_labels])
