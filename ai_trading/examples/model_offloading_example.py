...
def test_auto_offloading(model_class):
    ...
    offloader = ModelOffloader(model=model, offload_strategy="auto")
    ...
    logger.info(
        f"Pas d'entraînement terminé en {train_time:.4f}s avec stratégie '{offloader.strategy}'"
    )
    offloader.optimize_memory()
...
def compare_strategies():
    ...
    results = {
        ...
        "accelerate": (
            accelerate_results["inference_time"] if accelerate_results else None
        ),
    ...
    with open(RESULTS_DIR / "strategy_comparison.txt", "w") as f:
        ...
        f.write(
            f"Offloading auto ({results['auto_strategy']}): {results['auto']:.4f}s\n"
        )
        ...
        if base_time > 0:
            ...
            f.write(
                f"Offloading standard: {(results['standard'] / base_time) * 100:.1f}%\n"
            )
            if results["accelerate"]:
                f.write(
                    f"Offloading Accelerate: {(results['accelerate'] / base_time) * 100:.1f}%\n"
                )
            f.write(f"Offloading auto: {(results['auto'] / base_time) * 100:.1f}%\n")

    logger.info(
        f"Résultats de comparaison sauvegardés dans {RESULTS_DIR / 'strategy_comparison.txt'}"
    )
...
def main():
    ...
    if is_accelerate_available():
        ...
        logger.warning(
            "Huggingface Accelerate n'est pas disponible (pip install accelerate)"
        )
