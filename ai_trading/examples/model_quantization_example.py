...
def train_model(model, train_data, train_targets, val_data, val_targets, 
    ...
    for epoch in range(epochs):
        ...
        logger.info(f"Époque {epoch+1}/{epochs} - Perte entraînement: {epoch_loss:.4f} - Perte validation: {val_loss:.4f}")
...
def plot_results(original_model, quantized_model, test_data, test_targets, save_path=None):
    """
    ...
    plt.scatter(indices, original_preds[indices], color='green', alpha=0.5, label='Prédictions modèle original')
    plt.scatter(indices, quantized_preds[indices], color='red', alpha=0.5, label='Prédictions modèle quantifié')
...
def demonstration_mlp():
    ...
    val_data, val_targets = X[train_size:train_size+val_size], y[train_size:train_size+val_size]
    test_data, test_targets = X[train_size+val_size:], y[train_size+val_size:]
    
    # Créer et entraîner le modèle
    logger.info("Création et entraînement du modèle...")
    model = TradingModel(input_dim=input_dim, hidden_dim=hidden_dim, output_dim=output_dim)
    ...
    logger.info(f"Performance du modèle quantifié dynamiquement - MSE: {dynamic_loss:.4f}")
    
    # Comparer les performances (taille et vitesse)
    logger.info("\nComparaison des performances...")
    benchmark_results = benchmark_inference_speed(
        model, dynamic_quantized_model, test_data, num_iterations=100, warmup_iterations=10
    )
    
    logger.info(f"Temps d'inférence modèle original: {benchmark_results['original_inference_time_ms']:.2f} ms")
    logger.info(f"Temps d'inférence modèle quantifié: {benchmark_results['quantized_inference_time_ms']:.2f} ms")
    ...
    try:
        ...
        if not backends:
            logger.warning("Aucun backend de quantification statique n'est disponible sur ce système")
        else:
            ...
            logger.info(f"Performance du modèle quantifié statiquement - MSE: {static_loss:.4f}")
            ...
            export_quantized_model(static_quantized_model, str(export_path), input_sample=test_data[:1])
            logger.info(f"Modèle quantifié exporté dans {export_path}")
            
            # Tracer les résultats
            plot_path = export_dir / "quantization_comparison.png"
            plot_results(model, static_quantized_model, test_data[:100], test_targets[:100], str(plot_path))
...
def demonstration_cnn():
    ...
    model = ConvTradingModel(input_channels=1, sequence_length=sequence_length, output_dim=1)
    model, history = train_model(
        model, train_data, train_targets, val_data, val_targets,
        epochs=5, batch_size=64, learning_rate=0.001
    )
    ...
    def test_fn(m):
        ...
        return {
            'inference_time': (end_time - start_time) * 1000 / 10,  # ms
            'mse': mse
        }
    ...
    logger.info(f"Taille originale: {comparison.get('original_size')/1024/1024:.2f} Mo")
    logger.info(f"Taille quantifiée: {comparison.get('quantized_size')/1024/1024:.2f} Mo")
    logger.info(f"Réduction de taille: {comparison.get('size_reduction_percent', 0):.2f}%")
    ...
    if 'original_metrics' in comparison and 'quantized_metrics' in comparison:
        ...
        if 'inference_time' in orig_metrics and 'inference_time' in quant_metrics:
            ...
            logger.info(f"Temps d'inférence original: {orig_metrics['inference_time']:.2f} ms")
            logger.info(f"Temps d'inférence quantifié: {quant_metrics['inference_time']:.2f} ms")
...
def main():
    """Fonction principale."""
    parser = argparse.ArgumentParser(description="Démonstration de quantification de modèles")
    parser.add_argument("--model", type=str, choices=["mlp", "cnn", "both"], default="both",
