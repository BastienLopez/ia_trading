...
from ai_trading.utils.performance_tracker import PerformanceTracker, TimeTracker, log_memory_usage
...
def test_multiprocessing_configurations(data, sequence_length=50, batch_size=32):
    ...
    configurations = [
        {"name": "Synchrone (1 process)", "workers": 0, "pin_memory": False, "prefetch": None},
        ...
        {"name": "4 workers + pin memory", "workers": 4, "pin_memory": True, "prefetch": 2},
        {"name": "Auto workers", "workers": -1, "pin_memory": True, "prefetch": 2},  # Auto-détection du nombre de workers
    ]
    ...
    for config in configurations:
        ...
        logger.info(f"  Mémoire max: {perf_summary['memory']['max_rss'] / (1024 * 1024):.1f} MB")
...
def test_prefetch_configurations(data, sequence_length=50, batch_size=32, num_workers=4):
    """
...
def test_persistent_workers(data, sequence_length=50, batch_size=32, num_workers=4):
    ...
    if num_workers == 0:
        logger.warning("persistent_workers est ignoré quand num_workers=0. Test ignoré.")
        return pd.DataFrame()
...
def visualize_results(results_df, result_type="workers"):
    ...
    logger.info(f"Graphique enregistré dans {output_dir / f'multiprocessing_{result_type}_results.png'}")
...
def main():
    ...
    """
    logger.info("=== Test des optimisations multiprocessing pour le système de trading ===")
    ...
    if not workers_results.empty:
        ...
        best_config = workers_results.loc[workers_results["batches_per_second"].idxmax()]
    ...
    if optimal_workers > 0:
        ...
        if not prefetch_results.empty:
            ...
            best_prefetch = prefetch_results.loc[prefetch_results["batches_per_second"].idxmax()]
            logger.info(f"Facteur de préchargement optimal: {best_prefetch['prefetch_factor']}")
            logger.info(f"Performance: {best_prefetch['batches_per_second']:.2f} batches/s")
    ...
    if optimal_workers > 0 and not prefetch_results.empty:
        best_prefetch = prefetch_results.loc[prefetch_results["batches_per_second"].idxmax()]
        logger.info(f"2. Facteur de préchargement optimal: {best_prefetch['prefetch_factor']}")
        logger.info(f"   Performance: {best_prefetch['batches_per_second']:.2f} batches/s")
        
    if optimal_workers > 0 and not persistent_results.empty:
        best_persistent = persistent_results.loc[persistent_results["batches_per_second"].idxmax()]
        persistent_value = best_persistent["persistent_workers"]
        logger.info(f"3. Workers persistants: {'Activés' if persistent_value else 'Désactivés'}")
        logger.info(f"   Performance: {best_persistent['batches_per_second']:.2f} batches/s")
