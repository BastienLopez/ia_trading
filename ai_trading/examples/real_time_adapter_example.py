...
class SimpleMarketPredictor:
    ...
    def predict(self, market_data):
        ...
        if price_change_1h is not None:
            trend_score += (
                price_change_1h * 5
            )  # Amplifier l'effet du changement horaire
...
def simulate_market_data(duration_seconds=60, interval_seconds=5):
    ...
    while elapsed_seconds < duration_seconds:
        ...
        random_change = (
            current_price * np.random.normal(0, volatility) * np.sqrt(interval_seconds)
        )
...
def prediction_update_callback(prediction):
    ...
    print(
        f"[{time_str}] Nouvelle prédiction: {direction} (confiance: {confidence:.2f})"
    )
...
def significant_change_callback(prediction):
    ...
    if market_state:
        ...
        print(
            f"  État du marché: Prix={price:.2f}, Tendance={trend}, Volatilité={volatility:.4f}"
        )
...
def main():
    ...
    market_monitor = RealTimeMarketMonitor(
        observation_window=10, volatility_threshold=1.5, price_move_threshold=0.02
    )
    ...
    try:
        ...
        for price, volume, timestamp in simulate_market_data(
            duration_seconds=120, interval_seconds=2
        ):
        ...
        for i, pred in enumerate(history):
            ...
            print(
                f"  {i+1}. [{timestamp}] Direction: {direction} (confiance: {confidence:.2f})"
            )
